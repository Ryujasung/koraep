		...
		if(!isNaN(chkLength)){
			...
			$(this).change(function(){
				...
				if(nCurLength > nChkLength){
					...
					alert("한글 "+parseInt(chkLength/3)+"자(영문 "+chkLength+"자) 초과 입력할 수 없습니다.");
					$(this).val("");
					$(this).focus();
				}
			});
		...
		if(!isNaN(chkLength)){
			...
			$(this).change(function(){
				...
				if(nCurLength > nChkLength){
					...
					alert("한글 "+parseInt(chkLength/2)+"자(영문 "+chkLength+"자) 초과 입력할 수 없습니다.");
...
//form 데이터용 ajax
 function fileajaxPost(url,dataBody,func,pAsync){
		...
		if (typeof dataBody[key] == "string"){
			...
			for(var i=0;i<dataBody[key].length;i++){
				...
				if(typeof dataBody[key][i] == "object"){
					for(var k=0;k<dataBody[key][i].length;k++){
						if(dataBody[key][i][k] == undefined) continue;
						fileData[key][i][k] = dataBody[key][i][k];
					}
					continue;
				}
				
				fileData[key][i] = dataBody[key][i];
			}
		}
	}
	*/
 ...
 * @param mobile_yn - 모바일여부
 * 폼안에 CRF_NAME(리포트 파일명)이 반드시 있어야 함.
 */
kora.common.gfn_viewReport = function(formName, mobile_yn){
	...
	var prtForm = $(document.forms[formName]).serializeArray();
	var data = {};
	var params = "";

	$.each(prtForm, function(i,v){
	    data[v.name] = v.value;
	    
	    params += "&"+ v.name + "=" + v.value;
	});
	
	//var url = "https:/" + location.host + "/ClipReport4/viewReport.jsp?_csrf=" + gtoken;
	var url = "https:/" + location.host + "/ClipReport4/viewReport_m.jsp?_csrf=" + gtoken;
	    url += params;

	//kora.common.iWebAction('6000', {_url : url, _params : JSON.stringify(data)} );
...
kora.common.fn_validDate_ck =function(sel_val, data, upd_std){
	...
	if(rtc_dt_list !=undefined && rtc_dt_list != null){
				...
				if(Number(st_dt)>Number(inpDate)){//시작날짜가 등록날짜보다 큰경우
					alert(kora.common.formatter.datetime(st_dt,"yyyy-mm-dd")+" ~ "+kora.common.formatter.datetime(end_dt,"yyyy-mm-dd") +" 내의 일자만 등록 가능합니다.");
					return false;
...
kora.common.fnCheckDay = function(date1, date2, chkDate)
{
	...
	if(day>chkDate){
		alert("조회기간이 " + chkDate + "일을 넘을 수 없습니다.");
		return false;
	}
...
kora.common.gfn_bizNoCheck = function(bizNo) {
	...
	if(bizNo == null || bizNo == undefined) return false
...
kora.common.gfn_idPwChkValid = function (str){
	...
	for(var i=0; i < str.length; i++) {
		...
		if(chr_pass_0 == chr_pass_1){
			SamePass_0++;
			if(SamePass_0 > 1) {
				alert("동일한 문자를 연속으로 3번 이상 사용할 수 없습니다.");
				return false;
			}
		}else{
			SamePass_0 = 0;
		...
		if(chr_pass_0.charCodeAt(0) - chr_pass_1.charCodeAt(0) == -1){
			...
			if(SamePass_1 > 1)  {
				...
				alert("연속된 문자열(123, abc 등)을 3자 이상 사용 할 수 없습니다.");
				return false;
			}
		}else{
			SamePass_1 = 0;
...
kora.common.cfrmDivChkValid = function(divId){
	...
	$("#"+divId).find(".i_notnull").each(function(){
		if($(this).css("display") != "none"){
			...
			if($(this).val() == null || $(this).val() == ""){
				alert($(this).attr("alt")+" 을(를) 확인하세요.", "kora.common.cfrmDivChkValid_focus");
				chkTarget = $(this);
				chkDiv = false;
				return false;
	...
	$("#"+divId).find("input[format=number]").each(function(){
		...
		if($(this).val() !=""){
				if($(this).css("display") != "none"){
					if(isNaN($(this).val()) ){
						alert($(this).attr("alt")+" 은 숫자만 입력가능합니다.", "kora.common.cfrmDivChkValid_focus");
						chkTarget = $(this);
						chkDiv = false;
						return false;
					}
				...
				if(!/^[0-9]{1,}$/.test($(this).val()) ){
					alert($(this).attr("alt")+' 은 정수만 입력 가능합니다.', "kora.common.cfrmDivChkValid_focus");
					chkTarget = $(this);
					chkDiv = false;
					return false;
				}
...
kora.common.goPage = function(url, jsonInp){
...
kora.common.selAcctNoByBnk = function(pBnkCd){
	...
	//alert(JSON.stringify(acctValRec));
	return acctValRec;
...
function gfn_callInfoMsg(){
	alert("지급관리시스템 통합문의 콜센터 1522-0082 \n운영시간 : 평일(월~금) 9시~21시 \n토,일요일과 공휴일은 운영하지 않습니다.");
}
...
//						result = jsonObject(request.responseText);
			...
			alert("XMLHttpRequest not found.");
		}
	}
	catch(Exception){
		alert(Exception);
    		...
    		if($(this).css("display") != "none"){
    			if($(this).val() == null || $(this).val() == ""){
    				...
    				if ( kora.common.null2void($(this).attr("fieldName")) != "") {
    					alert($(this).attr("fieldName")+" 을(를) 확인하세요.");
    				}
    				else {
    					alert($(this).parents("td").prev().text()+" 을(를) 확인하세요.");
    				}
 ...
 * 모바일 로그아웃
 */
function fn_mblLogOut(){
    var url = "/MBL_USER_LOGOUT.do"
    var input = {};
	
    ajaxPost(url, input, function(rtnData) {
   	    
	if( window.BrowserBridge != undefined ){
