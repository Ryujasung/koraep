    ...
    UI.addClass({
        ...
        return function () {
            ...
            var ctrlKeys = {
                ...
                kora.common.fn_autoCompleteSelected(item.selected,item.target.id); //ㅇㄷ
                    ...
                    var autocompleteEvent = {
                        ...
                        'keyUp': function keyUp(queIdx, e) {  
                            ...
                            if (ctrlKeys[e.which]) {
                                ...
                                // backspace 감지 하여 input 값이 없으면 스탑이벤트 처리 할 것  ㅇㄷ
                                if (e.which == ax5.info.eventKeys.BACKSPACE && this.queue[queIdx].$displayLabelInput.val() == "") {
                                	if(valCnt !=1){  
	                                    ...
	                                    U.stopEvent(e);
	                                    //맨 마지막꺼지울경우 ㅇㄷ
	                                    if(this.queue[0].selected.length==0){
	                                    	kora.common.fn_autoCompleteSelected(this.queue[0].selected);
	                                    }
			                                   
                                	}else if(this.queue[queIdx].$displayLabelInput.val().length ==0){
                                		valCnt=0;
                                	}
                                } else {
                                	valCnt =this.queue[queIdx].$displayLabelInput.val().length;
                                    debouncedFocusWord.call(this, queIdx);
                                }
                            
                              //  test = displayLabelInput.val() .legngwserg
