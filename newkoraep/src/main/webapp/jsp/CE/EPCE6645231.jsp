...
<%@include file="/jsp/include/common_page.jsp" %>
	...
	<script type="text/javaScript" language="javascript" defer="defer">
		...
		$(document).ready(function(){
			...
			mfcSeCdList = jsonObject($("#mfcSeCdList_list").val());
		    ...
		    $('#DRCT_RTRVL_DT').YJcalendar({  
		  	...
		  	//text 셋팅
			$('#drct_rtrvl_mfc_sel').text(parent.fn_text('drct_rtrvl')+parent.fn_text('mfc_cho'));    //직접회수 생산자 선택
			...
			//생산자구분
			kora.common.setEtcCmBx2(mfcSeCdList, "","", $("#MFC_BIZRNM"), "BIZRID_NO", "BIZRNM", "N" ,'S');
			//직접회수대상 직매장/공장
			kora.common.setEtcCmBx2([], "","", $("#MFC_BRCH_NM"), "MFC_BIZRID_NO", "MFC_BIZRNM", "N" ,'S'); 
			//빈용기명
	    	...
	    	var err_msg2 = "";
	    	 ...
	    	 var mfc_bizrno = mfc_bizr_val[1];
	     	 ...
	     	 for(var i=0; i<rtnData.length ;i++) {
	     		...
	     		if(!kora.common.fn_validDate_ck( "R", input["DRCT_RTRVL_DT"])){ //등록일자제한 체크
					return;
				}
	   ...
	   function fn_mfc_bizrnm(){
			var url = "/CE/EPCE6645231_192.do" 
			var input = {};
			var mfc_bizrnm	= $("#MFC_BIZRNM").val(); 	 //직접회수 생산자
 			...
 			input["DRCT_RTRVL_DT"] 	= $("#DRCT_RTRVL_DT").val(); 		//직접회수일자
       	    ...
       	    ajaxPost(url, input, function(rtnData) {
   				if ("" != rtnData && null != rtnData) {   
   					 ...
   					 kora.common.setEtcCmBx2(rtnData.brch_dtssList, "","", $("#MFC_BRCH_NM"), "BRCH_ID_NO", "BRCH_NM", "N" ,'S');//직접회수대상 직매장/공장		
	   ...
	   function fn_ctnr_cd(){
	    ...
	    //행추가
	 	...
	 	insRow = function(gbn) {
			...
			if(!kora.common.fn_validDate_ck( "R", $("#DRCT_RTRVL_DT").val())){ //등록일자제한 체크
				return;
			}
    ...
    <div class="iframe_inner" >
		...
		<input type="hidden" id="rtc_dt_list" value="<c:out value='${rtc_dt_list}' />" />
			...
			<section class="secwrap" id="divInput1">
				...
				<div class="row">
						<div class="col">
							<div class="tit" id="drct_rtrvl_mfc_sel"></div>  <!--직접회수 생산자 선택 -->
							<div class="box">
