...
<!-- 페이징 사용 등록 -->
	...
	$(document).ready(function(){
		...
		if( askSeq != ''){
			...
			//페이지이동
		    kora.common.goPageD(sAction, {"ASK_SEQ": askSeq, "CNTN_SE": "A"}, {});
		}
		/*상세조회 바로이동*/
		
		
		var total = "<c:out value='${totalCnt}' />";
	...
	function fn_reg(){ //이전 linkaakm
		...
		var jParams = {"CNTN_SE" : "Q", "FN_CALLBACK" : "search"};
	...
	function fn_dtl_sel_lk(ask_seq, cntnSe){ //link
		...
		//페이지이동
	    kora.common.goPageD(sAction, {"ASK_SEQ": ask_seq, "CNTN_SE": cntnSe}, {"cate":$("#cate").val(), "word":$("#word").val(), "NOW_PAGE" : gridCurrentPage, "FN_CALLBACK" : "search"});
	...
	function gridSet(){
		...
		layoutStr.push('			<DataGridColumn dataField="ASK_SEQ" headerText="'+parent.fn_text('no')+'" textAlign="center" width="4%" />');
		layoutStr.push('			<DataGridColumn dataField="SBJ" headerText="'+parent.fn_text('sbj')+'" width="50%" itemRenderer="HtmlItem" />');
		layoutStr.push('			<DataGridColumn dataField="USER_NM" headerText="'+parent.fn_text('reg_prsn')+'" textAlign="center" width="14%" />');
		layoutStr.push('			<DataGridColumn dataField="BIZRNM" headerText="'+parent.fn_text('bizr_nm')+'" textAlign="center" width="20%" />');
		layoutStr.push('			<DataGridColumn dataField="REG_DTTM" headerText="'+parent.fn_text('reg_dt')+'" width="12%" formatter="{datefmt2}" textAlign="center" />');
		layoutStr.push('		</columns>');
					...
					<div class="box">						
						<div class="box">
							<select id="cate" style="width: 129px; float: right;">
								...
								<option value="prsn" id="reg_prsn"></option>
							</select>
						</div>
					</div>
				</div>
