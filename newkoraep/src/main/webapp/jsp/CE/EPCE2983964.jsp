  ...
  /*   //조정확인 후 상세조회 다시 조회
    ...
    //조정확인
     function fn_reg_chk(){
    	 ...
    	 if(rtn_stat_cd !="WJ" ){
	    	alertMsg("입고조정 상태의 자료만 조정 확인 처리 가능합니다.\n 다시 한 번 확인하시기 바랍니다");
			return    		 
    	 }
    	 confirm("조정 내역에 대한 확인 처리를 하시겠습니까?","fn_reg");
    }

    function fn_reg(){
    	var url = "/CE/EPCE2983964_21.do"
    	var input ={};
 	 ...
 	 //확인취소요청 상태변경 체크
	function fn_upd3_check(){
	    var rtn_stat_cd =iniList[0].RTN_STAT_CD;
    	if(rtn_stat_cd !="WC"){
    		alertMsg("입고확인 상태의 정보만 취소 요청 가능합니다. 다시 한 번 확인하시기 바랍니다.");
			return    		 
    	}
		confirm("선택된 입고확인 내역에 대해 취소요청을 진행합니다. \n\n 취소 처리된 입고확인 내역은 반환등록 상태로 변환되며 복원되지 않습니다. 계속 진행하시겠습니까?","fn_upd3_pop");
	}
 	 
	function fn_upd3_pop(){
	    var pagedata = window.frameElement.name;
	    var url = "/CE/EPCE29839884.do";
	    window.parent.NrvPub.AjaxPopup(url, pagedata);
	}
 	 
	//입고취소요청내역
	function fn_upd4_check(){
     	if( !iniList[0].CNL_REQ_SEQ ){ alertMsg('취소요청 내역에 대해서만 조회 가능 합니다.'); return; }
     	
     	var pagedata = window.frameElement.name;
		parent_item.CNL_REQ_SEQ = iniList[0].CNL_REQ_SEQ;
		window.parent.NrvPub.AjaxPopup('/CE/EPCE6622288.do', pagedata);
	}
   
   //확인취소요청 상태변경
	function fn_upd3(text){
		var url 	= "/CE/EPCE2983901_21.do"
		var input 	= {"list": ""};
		var item	= {};
		var row 	=  new Array();
		
		item["WRHS_DOC_NO"] 		= iniList[0].WRHS_DOC_NO;	//입고문서
		item["RTN_DOC_NO"] 			= iniList[0].RTN_DOC_NO;		//반환문서
    	if(iniList[0].RSRC_DOC_NO !=null){
    		item["RSRC_DOC_NO"] 		= iniList[0].RSRC_DOC_NO	//실태조사문서
    	}
    	item["CNL_REQ_YN"]           = "Y"; //취소요청 구분
    	item["REQ_STAT_CD"]             = "R"; //입고확인취소요청
    	
    	item["CFM_CHECK"] 				= "T"; //취소요청 상태체크
    	item["STAT_CHK"]				= "T"; //상태 체크 <if>
    	item["RTN_STAT_CD"] 			= "FC";//변경할 상태
    	item["RTN_STAT_CD_CHK"] 	= "WC";//where절 상태체크
    	row.push(item);
	...
	function link(){
		...
		var pagedata = window.frameElement.name;
		window.parent.NrvPub.AjaxPopup('/CE/EPCE29839883.do', pagedata);
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('            <DataGridColumn dataField="STANDARD_NM"     headerText="용기구분"        width="6%" textAlign="center"  />');
			...
			layoutStr.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{num5}" formatter="{numfmt}" textAlign="right"/>');	//도매부가세
			...
			layoutStr2.push('            <DataGridColumn dataField="STANDARD_NM"     headerText="용기구분"        width="80" textAlign="center"  />');
			...
			layoutStr2.push('			<DataGridColumn dataField="RTN_QTY"  	 headerText="'+ parent.fn_text('rtn_qty')+'"				width="80" 	 textAlign="right" formatter="{numfmt}" id="num1"/>');								//반환량
			...
			layoutStr2.push('				<DataGridColumn dataField="CFM_WHSL_FEE" 			headerText="'+ parent.fn_text('whsl_fee2')+ '" 	width="90" 	formatter="{numfmt1}" textAlign="right"  id="num6"/>'); 	//도매수수료
			layoutStr2.push('				<DataGridColumn dataField="CFM_RTL_FEE" 				headerText="'+ parent.fn_text('rtl_fee2')+ '" 		width="90" 	formatter="{numfmt1}" textAlign="right"  id="num8"/>');	//소매수수료
			layoutStr2.push('				<DataGridColumn dataField="CFM_WHSL_FEE_STAX"	headerText="'+ parent.fn_text('stax')+ '"	width="90" 	formatter="{numfmt}" textAlign="right"  id="num7"/>');	//도매부가세
			layoutStr2.push('			</DataGridColumnGroup>');
			...
			layoutStr2.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{num6}" formatter="{numfmt1}" textAlign="right"/>');	//도매수수료
			layoutStr2.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{num8}" formatter="{numfmt1}" textAlign="right"/>');	//소매수수료
...
<body>
    ...
    <div class="iframe_inner" id="testee" >
		 ...
		 <!-- 	<a href="#self" class="btn36 c1" style="width: 120px; " onclick="kora.common.saveLayout()">saveLayout</a>  -->
			...
			<div class="boxarea mt10">
				...
				<!-- <a href="#self" class="btn36 c1" style="width: 120px; " onclick="kora.common.saveLayout(2)">saveLayout2</a>  -->
