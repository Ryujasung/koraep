...
//         fnSetGrid2();
        ...
        ajaxPost(url, input, function(rtnData) {
            if ("" != rtnData && null != rtnData) {
                ...
                $.each(rtnData.selList, function(i, v){
                    var pieDataLine = {};
                    pieDataLine["TITLE_IN"]  = v.AREA_NM + "(" + "보관량" + ")";
                    pieDataLine["TITLE_OUT"] = v.AREA_NM + "(" + "사용량(출고)" + ")";
                    pieDataLine["VAL_IN"]    = v.KEEP_QTY;
   ...
   // }
    ...
    function divDataTipFunction(seriesId, code, label, data) {
        if(INQ_PARAMS.SEL_PARAM ==null) return; 
        var KEEP_QTY=0;					// 총보관량
        var KEEP_QTY_RT=0;			// 보관량 비율
        var DLIVY_QTY=0;				// 출고량
        var DLIVY_QTY_RT=0;			// 출고비율
        var RTN_QTY=0;					// 회수량
        var RTN_QTY_RT=0;				// 회수비율
        //2664-1134
        //console.log(JSON.stringify(selList))
        for(var i=0; i<selList.length; i++){
            if(selList[i].AREA_CD == code){
                KEEP_QTY = kora.common.gfn_setComma(selList[i].KEEP_QTY);
                KEEP_QTY_RT = kora.common.trunc(selList[i].KEEP_QTY_RT, 3);
                ...
                +'<div class="info_tbl" style="">'
                    +'<table>'
                        ...
                        +'<tbody>'
                            ...
                            +'<tr>'
                                    +'<td>보관량</td>'
                                    +'<td id="" style="text-align:right;padding-right:5px">'+KEEP_QTY+'</td>'
                                    +'<td id="" style="text-align:right;padding-right:5px">'+KEEP_QTY_RT+' %</td>'  
                            +'</tr>'
                        +'</tbody>'
                    +'</table>'
                +'</div>'
    ...
    function fnSetGrid() {
        ...
        layoutStr.push('            <DataGridColumn dataField="KEEP_QTY" headerText="총보관량" width="120" formatter="{numfmt}" textAlign="right" id="num1" />');
        layoutStr.push('            <DataGridColumn dataField="KEEP_QTY_RT" headerText="비율" id="dg1col1" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr.push('            <DataGridColumn dataField="DLIVY_QTY" headerText="출고량"  width="120" formatter="{numfmt}" textAlign="right" id="num2" />');
        layoutStr.push('            <DataGridColumn dataField="DLIVY_QTY_RT" headerText="비율" id="dg1col3" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr.push('            <DataGridColumn dataField="RTN_QTY" headerText="회수량"  width="120" formatter="{numfmt}" textAlign="right" id="num3"/>');
        layoutStr.push('            <DataGridColumn dataField="RTN_QTY_RT" headerText="비율" id="dg1col3" width="60" formatter="{numfmt1}" textAlign="right" />');
        //layoutStr.push('            <DataGridColumn dataField="EXCH_QTY" headerText="출고보증금합계"  width="120" formatter="{numfmt}" textAlign="right" id="num4" />');
       // layoutStr.push('            <DataGridColumn dataField="RTRVL_QTY" headerText="입고보증금합계"  width="120" formatter="{numfmt}" textAlign="right" id="num5" />');
