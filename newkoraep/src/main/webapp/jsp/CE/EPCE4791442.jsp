...
<%@include file="/jsp/include/common_page.jsp" %>
		...
		$(document).ready(function(){
			...
			ajaxPost(url, parent_item, function(rtnData){
				if(rtnData != null && rtnData != ""){
					...
					$('#EXCA_STD_NM').val(rtnData.searchDtl[0].EXCA_STD_NM);
					
					if(rtnData.searchDtl[0].CET_FYER_EXCA_YN == 'Y'){
						$("#CET_FYER_EXCA_YN").prop('checked', true) ;
					}
					
					var excaSeCd = rtnData.searchDtl[0].EXCA_SE_CD;
					$(":radio[name='EXCA_SE_CD'][value='"+excaSeCd+"']").prop("checked", true);
					
					if(excaSeCd == 'Q'){
						$('#QTY_YEAR').val(rtnData.searchDtl[0].EXCA_SE_YEAR);
						$('#QTY').val(rtnData.searchDtl[0].EXCA_SE_QTRMT);
					}else if(excaSeCd == 'M'){
						$('#MT_YEAR').val(rtnData.searchDtl[0].EXCA_SE_YEAR);
						$('#MT').val(rtnData.searchDtl[0].EXCA_SE_QTRMT);
					}else if(excaSeCd == 'S'){
						$('#EXCA_ST_DT').val(kora.common.formatter.datetime(rtnData.searchDtl[0].EXCA_ST_DT, "yyyy-mm-dd"));
						$('#EXCA_END_DT').val(kora.common.formatter.datetime(rtnData.searchDtl[0].EXCA_END_DT, "yyyy-mm-dd"));
					}
										
					$('#CRCT_PSBL_ST_DT').val(kora.common.formatter.datetime(rtnData.searchDtl[0].CRCT_PSBL_ST_DT, "yyyy-mm-dd"));
					$('#CRCT_PSBL_END_DT').val(kora.common.formatter.datetime(rtnData.searchDtl[0].CRCT_PSBL_END_DT, "yyyy-mm-dd"));
					$('#CRCT_CFM_END_DT').val(kora.common.formatter.datetime(rtnData.searchDtl[0].CRCT_CFM_END_DT, "yyyy-mm-dd"));
					
					$('#TRGT_YN').val(rtnData.searchDtl[0].EXCA_TRGT_SE);
	...
	function fn_del_exec(){
			...
			var input = {};
			input["EXCA_STD_CD"] = EXCA_STD_CD;
			var url = "/CE/EPCE4791442_04.do";
		...
		function fn_sel(){
			...
			var input = {'EXCA_STD_CD':EXCA_STD_CD};
		    	 ...
		    	 if($('#TRGT_YN').val() == 'W'){
		    		...
		    		var collection = gridRoot.getCollection();
		 		 	...
		 		 	for(var i=0;i<collection.getLength(); i++){
		 		 		...
		 		 		if(tmpData.TRGT_YN == "Y") arrIdx.push(i);
		 		 	}
							...
							<table>
								...
								<tr>
									...
									<td style="padding-bottom:10px">
										<div class="row2" >
											<div class="calendar">
												<input type="text" id="CRCT_PSBL_ST_DT" name="from" style="width: 130px;" class="i_notnull"><!--시작날짜  -->
											</div>
											<div class="obj">~</div>
											<div class="calendar">
												<input type="text" id="CRCT_PSBL_END_DT" name="to" style="width: 130px;" class="i_notnull"><!-- 끝날짜 -->
											</div>
										</div>
									</td>
								</tr>
