...
//    			kora.common.hideLoadingBar(dataGrid, gridRoot);
   ...
   function fn_reg(){
		...
		var url = "/CE/EPCE2983988.do";		//실태조사요청 (생산자);
		...
		if(selectorColumn.getSelectedIndices() == "") {
			...
			return;
		}else if(parent_item["RTN_STAT_CD"] !="WJ" && parent_item["RTN_STAT_CD"] !="RG" ){
			alertMsg("입고조정, 반환등록 상태인경우에만 조사확인요청이 가능합니다.");
			return;
		}
		...
		var url ='/CE/EPCE2983931_092.do'	//템프에 저장		
	 	ajaxPost(url, input, function(rtnData){
			if(rtnData.RSLT_CD != "0000"){
				...
				INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE2983901.do";
				kora.common.goPage('/CE/EPCE2983931.do', INQ_PARAMS);
			}
		},false);
	...
	function link(){
		...
		INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
		INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE2983901.do";
		kora.common.goPage('/CE/EPCE2983964.do', INQ_PARAMS);
	}
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('            <DataGridColumn dataField="EXCA_APLY_YN" headerText="'+parent.fn_text('exca_aply_yn')+'" width="70" textAlign="center" id="tmp7"/>');
			layoutStr.push('		</groupedColumns>');
            ...
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum12" formatter="{numfmt}" dataColumn="{num12}" textAlign="right"/>');
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum13" formatter="{numfmt}" dataColumn="{num13}" textAlign="right"/>');
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum14" formatter="{numfmt}" dataColumn="{num14}" textAlign="right"/>');
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum15" formatter="{numfmt}" dataColumn="{num15}" textAlign="right"/>');
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum16" formatter="{numfmt}" dataColumn="{num16}" textAlign="right"/>');
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum17" formatter="{numfmt}" dataColumn="{num17}" textAlign="right"/>');
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum21" formatter="{numfmt}" dataColumn="{num21}" textAlign="right"/>');
            ...
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum18" formatter="{numfmt}" dataColumn="{num18}" textAlign="right"/>');
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum19" formatter="{numfmt}" dataColumn="{num19}" textAlign="right"/>');
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum20" formatter="{numfmt}" dataColumn="{num20}" textAlign="right"/>');
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum22" formatter="{numfmt}" dataColumn="{num22}" textAlign="right"/>');
    		layoutStr.push('				<DataGridFooterColumn dataColumn="{tmp1}"/>');
    		layoutStr.push('				<DataGridFooterColumn dataColumn="{tmp2}"/>');
    		layoutStr.push('				<DataGridFooterColumn dataColumn="{tmp3}"/>');
    		layoutStr.push('				<DataGridFooterColumn dataColumn="{tmp4}"/>');
	...
	function fn_upd4_check(){
			...
			if(selectorColumn.getSelectedIndices() == "") {
			 ...
			 for(var i=0; i<selectorColumn.getSelectedIndices().length; i++) {
					...
					//취소요청 상태만 가능
					if(item["RTN_STAT_CD"] !="FC"){     
						alertMsg("취소요청 상태의 정보만 취소 가능합니다. 다시 한 번 확인하시기 바랍니다.");
						return;
					}
			 }
				confirm("취소요청 취소 처리 하시겠습니까?","fn_upd4");
	}
	...
	function fn_upd4(){
			...
			var url ="/CE/EPCE2983901_21.do";
			 ...
			 for(var i=0; i<selectorColumn.getSelectedIndices().length; i++) {
					...
					item["REQ_STAT_CD"] 			= "L"; //요청취소
					
					item["CFM_CHECK"] 			= "T"; //취소요청 상태체크
					item["STAT_CHK"]				= "T"; //상태 체크 <if>
					item["RTN_STAT_CD"] 			= "WC";//변경할 상태
					item["RTN_STAT_CD_CHK"] 	= "FC";//where절 상태체크
					row.push(item);
			 }
		     input["list"] = JSON.stringify(row);
