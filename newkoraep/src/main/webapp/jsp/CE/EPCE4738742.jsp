...
<script type="text/javaScript" language="javascript" defer="defer">
     ...
     $(function() {
   		...
   		crct_gridList2 	=  jsonObject($("#crct_gridList2").val());      
		...
		//날짜 셋팅
		 $('#CRCT_RTN_DT').YJcalendar({  
				triggerBtn : true,
				dateSetting : kora.common.getDate("yyyy-mm-dd", "D", 0, crct_gridList2[0].CRCT_RTN_DT).replaceAll('-','')
				
		 });
		$('#CRCT_WRHS_DT').YJcalendar({
				triggerBtn : true,
				dateSetting : kora.common.getDate("yyyy-mm-dd", "D", 0, crct_gridList2[0].CRCT_WRHS_CFM_DT).replaceAll('-','')
		 });  	
		...
		$("#CRCT_WRHS_DT").change(function(){
            var dt = $("#CRCT_WRHS_DT").val();
            dt = dt.replace(/-/gi, "");
            
            var today = kora.common.gfn_toDay();
            
            if(dt > today){
        	   $("#CRCT_WRHS_DT").val(prev_val);
    ...
    //빈용기 구분 선택시
    function fn_prps_cd(){
	    	var url = "/CE/EPCE4738731_19.do" 
			var input ={};
			if( $("#PRPS_CD").val() !=""){
				input["CUST_BIZRID"] 		= 	crct_gridList2[0].WHSDL_BIZRID		//도매업자아이디                     
				input["CUST_BIZRNO"] 		= 	crct_gridList2[0].WHSDL_BIZRNO		//도매업자사업자번호                   
				input["CUST_BRCH_ID"] 	= 	crct_gridList2[0].WHSDL_BRCH_ID		//도매업자 지점 아이디             
				input["CUST_BRCH_NO"] 	= 	crct_gridList2[0].WHSDL_BRCH_NO	//도매업자 지점 번호                      
				input["MFC_BIZRID"] 		= 	crct_gridList2[0].MFC_BIZRID			//생산자 아이디             
				input["MFC_BIZRNO"] 		= 	crct_gridList2[0].MFC_BIZRNO			//생산자 사업자번호           
				input["MFC_BRCH_ID"] 		= 	crct_gridList2[0].MFC_BRCH_ID			//생산자 직매장/공장 아이디      
				input["MFC_BRCH_NO"] 	= 	crct_gridList2[0].MFC_BRCH_NO		//생산자 직매장/공장 번호               
				input["WRHS_CFM_DT"] 	=	crct_gridList2[0].WRHS_CFM_DT		//반화일자
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	insRow = function(gbn) {
		...
		for(var i=0; i<ctnr_nm.length; i++){
			if(ctnr_nm[i].CTNR_CD == ctnrCd) {
				for(var k=0;k<crct_gridList.length ;k++){
					if(crct_gridList[k].CTNR_CD ==ctnrCd){
						 input["RTN_QTY"] =crct_gridList[k].RTN_QTY;	//반환량
						 break;
					}
				}
			    input["CTNR_SE"] 					= $("#CTNR_SE").val();  	 								//빈용기명 구분 구/신
			    ...
			    input["CRCT_AMT"] 					=	Number(input["CRCT_GTN"]) + Number(input["CRCT_WHSL_FEE"]) + Number(input["CRCT_WHSL_FEE_STAX"])+ Number(input["CRCT_RTL_FEE"]); //총합계
			    input["WHSDL_BIZRID"] 			= 	crct_gridList[0].WHSDL_BIZRID			//도매업자아이디
			    input["WHSDL_BIZRNO"] 			= 	crct_gridList[0].WHSDL_BIZRNO		//도매업자사업자번호
			    input["WHSDL_BRCH_ID"] 		= 	crct_gridList[0].WHSDL_BRCH_ID		//도매업자 지점 아이디
			    input["WHSDL_BRCH_NO"] 		= 	crct_gridList[0].WHSDL_BRCH_NO		//도매업자 지점 번호
			    input["MFC_BIZRID"] 				= 	crct_gridList[0].MFC_BIZRID				//생산자 아이디
			    input["MFC_BIZRNO"] 				= 	crct_gridList[0].MFC_BIZRNO			//생산자 사업자번호
			    input["MFC_BRCH_ID"] 			= 	crct_gridList[0].MFC_BRCH_ID			//생산자 직매장/공장 아이디
			    input["MFC_BRCH_NO"] 			= 	crct_gridList[0].MFC_BRCH_NO			//생산자 직매장/공장 번호
			    input["WRHS_DOC_NO"]			= 	crct_gridList[0].WRHS_DOC_NO	   	// 입고문서번호
			    break;
			}
		}
		return input;
		...
		for(var i=0;i<collection.getLength(); i++){
			    ...
			    for(var k=0; k<ctnr_nm_all.length; k++){
			    	if(ctnr_nm_all[k].CTNR_CD == tmpData["CTNR_CD"]) {
					    ...
					    input["CRCT_AMT"] 					=Number(tmpData["CRCT_QTY"])+ Number(tmpData["CRCT_WHSL_FEE"] )+ Number(tmpData["CRCT_WHSL_FEE_STAX"]); //총합계
					    break; 
			    	}
			    }
				// 해당 데이터의 이전 수정내역을 삭제
	...
	function fn_reg(){
		...
		var crct_tot=0;
		var collection = gridRoot2.getCollection();
		var url = "/CE/EPCE4738742_21.do"
		
		for(var i=0;i<collection.getLength(); i++){
	 		var tmpData 	= gridRoot2.getItemAt(i);
			tmpData["WRHS_CRCT_STAT_CD"]		= "R";
			tmpData["EXCA_STD_CD"]				= INQ_PARAMS.SEL_PARAMS.EXCA_STD_CD	//정산기준코드
			tmpData["REG_PRSN_ID"]					= crct_gridList2[0].REG_PRSN_ID		//최초등록자  
	 		tmpData["REG_DTTM"]						= crct_gridList2[0].REG_DTTM		//최초등록시간
			crct_tot += Number(tmpData["CRCT_QTY"]) +Number(tmpData["CRCT_DMGB_QTY"])+Number(tmpData["CRCT_VRSB_QTY"]);
 			row.push(tmpData);//행 데이터 넣기
	 	}
		
		for(var i=0;i<row.length ;i++){
		 		row[i].CRCT_TOT =crct_tot;	//모든 총값
		 		row[i].WRHS_CRCT_DOC_NO =INQ_PARAMS.PARAMS.WRHS_CRCT_DOC_NO;
		 		if(crct_rtn_dt_chk !="" ){	//정정반환일자 적용 클릭시
		 			row[i].CRCT_RTN_DT = crct_rtn_dt_chk;
		 		}else{
		 			row[i].CRCT_RTN_DT = iniList[0].RTN_DT;
		 		}
        ...
        */
	 	...
	 	data["list_ori"] = JSON.stringify(crct_gridList2);
  ...
  //선택한 행 입력창에 값 넣기
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="RTN_QTY"  	 headerText="'+ parent.fn_text('rtn_qty')+'" width="80" formatter="{numfmt}"  textAlign="center" id="num1"/>');															//반환량
			...
			layoutStr.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{num8}" formatter="{numfmt}" textAlign="right"/>');	//도매부가세
			...
			layoutStr2.push('			<DataGridColumn dataField="RTN_QTY"  	 headerText="'+ parent.fn_text('rtn_qty')+'"				width="80"		formatter="{numfmt}"  textAlign="center" id="num1"/>');											//반환량
			...
			layoutStr2.push('			</DataGridColumnGroup>');
			layoutStr2.push('			<DataGridColumn dataField="CRCT_AMT" 	headerText="'+ parent.fn_text('amt_tot')+ '" 		width="70"  	formatter="{numfmt}" 	textAlign="right"  id="num10"  />');//금액합계(원)
...
<body>
    ...
    <div class="iframe_inner" id="testee" >
    		...
    		<input type="hidden" id="crct_gridList2" value="<c:out value='${crct_gridList}' />" />
								...
								<table>
									...
									<tbody>
										...
										<tr>
											...
											<th class="bd_l"  id="rtrvl_dt"></th>	<!-- 반환등록일자 -->
											<td>
												<div class="row">
				...
				<div class="srcharea" style="" > 
					<div class="row" >
						...
						<div class="col" style="width: 50%;">
							<div class="tit" id="crct_wrhs_dt" style="width: 120px" ></div>	<!-- 정정입고확인일자 -->
							<div class="box">
								<div class="calendar">
