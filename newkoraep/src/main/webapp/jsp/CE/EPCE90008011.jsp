...
<script type="text/javaScript" language="javascript" defer="defer">
	...
	function fn_del_chk(){
		...
		confirm("공급단가 " +  parent.fn_text('info_del'),"fn_del");
		return;
	}
	...
	function fn_del() {//선택 건이 없을 경우 
		...
		var item = gridRoot.getItemAt(rowIndexValue);
		var url ="/CE/EPCE90008011_04.do";
		var input = {};
		
		input["URM_FIX_CD"]					= item["URM_FIX_CD"];           			//용기코드 
	...
	function fn_reg() {
		...
		INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE90008011.do";
		kora.common.goPage('/CE/EPCE900080131.do', INQ_PARAMS);
	...
	function fn_upd() {
		...
		input["URM_FIX_CD"]					= item["URM_FIX_CD"];           			//소모품코드 
		input["URM_EXP_NM"]				= item["URM_EXP_NM"];              		//소모품명     
		input["REG_SN"]					= item["REG_SN"];						//등록순번
		input["SUP_FEE"]					= item["SUP_FEE"];            			//공급단가(원)    
		...
		kora.common.goPage('/CE/EPCE900080142.do', INQ_PARAMS);

	}
	//목록
	function fn_lst() {
		kora.common.goPage('/CE/EPCE9000801.do', INQ_PARAMS);
	}
	...
	function fnSetGrid1(reDrawYn) {
		...
		layoutStr.push('			<DataGridColumn dataField="URM_FIX_CD"  headerText="'+ parent.fn_text('ctnr_cd')	+ '"	width="70"	textAlign="center"/>');
		layoutStr.push('			<DataGridColumn dataField="URM_EXP_NM" headerText="'+ parent.fn_text('ctnr_nm')	+ '"	width="350"	textAlign="center"/>');
		layoutStr	.push('			<DataGridColumn dataField="SUP_FEE"	 headerText="공급가액"   width="80"	itemRenderer="HtmlItem" formatter="{numfmt}"   textAlign="center" />');//보증금원
