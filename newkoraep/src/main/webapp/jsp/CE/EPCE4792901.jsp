...
<%@include file="/jsp/include/common_page.jsp" %>
	...
	<script type="text/javaScript" language="javascript" defer="defer">
		...
		$(document).ready(function(){
			...
			kora.common.setEtcCmBx2(mfcBizrList, "","", $("#MFC_BIZR_SEL"), "BIZRID_NO", "BIZRNM", "N" ,'T');	
		...
		function fn_page(){
			...
			INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE4792901.do";
			kora.common.goPage('/CE/EPCE4792964.do', INQ_PARAMS);
		}
		...
		function fn_page3(){
			...
			var url = '/CE/EPCE47929643.do';
			
			if(input.STAC_DOC_NO.indexOf('BL') > -1){
				url = '/CE/EPCE4707264.do'; //과거 데이터 정산서 조회
			}
		...
		function fn_page2(){
			...
			for(var i=0; i<selectorColumn.getSelectedIndices().length; i++) {
				...
				if(checkBizrId == ''){
					checkBizrId = item.EXCH_EXCA_REG_BIZRID;
				}else if(checkBizrId != item.EXCH_EXCA_REG_BIZRID){
					alertMsg("동일한 생산자만 선택 가능합니다.");
					return;
				}
				
				if(item.EXCH_EXCA_STAT_CD != 'C' ){
					alertMsg("정산확인 건만 처리 가능합니다.");
					return;
				}
				
				if(item.EXCA_ISSU_PSBL_YN != "Y") {
				    alertMsg("생산자 상호간 정산요청 완료시 가능합니다.");
				    return;
				}
			}
			...
			INQ_PARAMS["FN_CALLBACK" ] = "fn_sel";
			INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE4792901.do";
			kora.common.goPage('/CE/EPCE47929312.do', INQ_PARAMS);
		}
		...
		function fn_upd_exec(){
			...
			data["EXCH_EXCA_STAT_CD"] = exchExcaStatCd;

			var url = "/CE/EPCE4792901_21.do";
		...
		function fn_reg(){
			...
			INQ_PARAMS["FN_CALLBACK" ] = "fn_sel";
			INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE4792901.do";
			kora.common.goPage('/CE/EPCE4792931.do', INQ_PARAMS);
		}
		 ...
		 */
		 function fn_set_grid() {
			 ...
			 layoutStr.push('	<DataGridColumn dataField="EXCA_PROC_STAT_NM"  headerText="'+parent.fn_text('exca')+parent.fn_text('stat')+'" width="100"  />');
			 layoutStr.push('	</groupedColumns>');
...
<body>
	<div class="iframe_inner">
        ...
        <input type="hidden" id="regPsblYn" value="<c:out value='${regPsblYn}' />" />
