...
<%@include file="/jsp/include/common_page.jsp" %>
	...
	<script type="text/javaScript" language="javascript" defer="defer">
		...
		$(document).ready(function(){
			...
			//가산금구분 옵션셋팅
			if(kora.common.null2void(parent_item.EXCA_ISSU_SE_CD) == 'G'){ //보증금
				$('#ADD_AMT_SE').append('<option value="G" selected>보증금</option>');
				$('#ADD_AMT_SE').append('<option value="E">기타</option>');
			}else if(kora.common.null2void(parent_item.EXCA_ISSU_SE_CD) == 'F'){ //취급수수료
			...
			if(kora.common.null2void(parent_item.ADD_AMT_SE) == ''){
				//$('#ADD_AMT_SE').val(parent_item.EXCA_ISSU_SE_CD);
			}else{
