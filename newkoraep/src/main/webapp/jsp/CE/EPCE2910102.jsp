...
<script type="text/javaScript" language="javascript" defer="defer">
	...
	gridTotalRowCount = 0;//전체 행 수
     ...
     $(function() {
  	    ...
  	    ctnrNmList_C 		= jsonObject($("#ctnrNmList").val());	
		...
		$('#ctnr_nm_txt').text(parent.fn_text('ctnr_nm_txt'));//빈용기명
		$('#whsdl_bizrno_txt').text(parent.fn_text('whsdl')+parent.fn_text('bizrno'));//도매업자 사업자번호
   ...
   //변경 페이지 이동 
  	function fn_upd() {
 		...
 		INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE2910102.do";

 		kora.common.goPage('/CE/EPCE2910142.do', INQ_PARAMS);
  	}
    ...
    function fn_init(){
		...
		//일자 option 순서값으로 선택(20200403추가)
		//$("#SEARCH_GBN option:eq(1)").prop("selected", true);
   ...
   //반환내역서 조회
    function fn_sel(){
		...
		//직접입력 사업자번호
// 		input["WHSDL_BIZRNO"]	= $("#WHSDL_BIZRNO_DE").val();
...
// 			hideLoadingBar();// 그리드 loading bar on
   ...
   function fn_del(){
	...
	function fn_page() {
		INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
		INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE2910102.do";
		kora.common.goPage('/CE/EPCE2910131.do', INQ_PARAMS);
	}
	//반환관리 상세
	function link(){
		...
		INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
		INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE2910102.do";
		kora.common.goPage('/CE/EPCE2910164.do', INQ_PARAMS);
	}
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
		...
		layoutStr.push('			<DataGridColumn dataField="PRPS_CD" headerText="'+ parent.fn_text('prps_cd')+ '"  textAlign="center" width="120"  />');//용도
		layoutStr.push('			<DataGridColumn dataField="CTNR_NM" headerText="'+ parent.fn_text('ctnr_nm')+ '"  textAlign="center" width="280"  />');//빈용기명
		layoutStr.push('			<DataGridColumn dataField="CTNR_CD" headerText="'+ parent.fn_text('cd')+ '" width="100" textAlign="center" />');//코드
		...
		layoutStr.push('				<DataGridColumn dataField="BOX_QTY" headerText="'+ parent.fn_text('box_qty')+ '" width="90" formatter="{numfmt}" textAlign="right" id="num1" />');//상자
		layoutStr.push('				<DataGridColumn dataField="RTN_QTY"	headerText="'+ parent.fn_text('btl')+ '" width="90" formatter="{numfmt}" textAlign="right" id="num2" />');//병
		layoutStr.push('			</DataGridColumnGroup>');
		layoutStr.push('			<DataGridColumn dataField="RTN_GTN_UTPC" headerText="'+ parent.fn_text('utpc')+ '"	width="90" formatter="{numfmt}" textAlign="right" id="num3"  />');//단가
		layoutStr.push('			<DataGridColumn dataField="RTN_GTN" headerText="'+ parent.fn_text('amt')+ '"	width="90" formatter="{numfmt}" textAlign="right"   id="num4"  />');//금액
		layoutStr.push('			<DataGridColumnGroup  headerText="'+ parent.fn_text('cntr')+' '+ parent.fn_text('std_fee')+'">');//빈용기 취급수수료(원)
		layoutStr.push('				<DataGridColumn dataField="RTN_WHSL_FEE_UTPC" headerText="'+ parent.fn_text('utpc')+ '" width="90" formatter="{numfmt}" textAlign="right"  id="num5"/>');//단가
		layoutStr.push('				<DataGridColumn dataField="RTN_WHSL_FEE" headerText="'+ parent.fn_text('whsl_fee2')+ '" width="90" formatter="{numfmt1}" textAlign="right"   id="num6"  />'); //도매수수료
		layoutStr.push('				<DataGridColumn dataField="RTN_RTL_FEE_UTPC" headerText="'+ parent.fn_text('utpc')+ '" width="90" 	formatter="{numfmt}" textAlign="right"  id="num7" />');//단가
		layoutStr.push('				<DataGridColumn dataField="RTN_RTL_FEE" headerText="'+ parent.fn_text('rtl_fee2')+ '" width="90" 	formatter="{numfmt1}" textAlign="right"  id="num8"  />');//소매수수료
		layoutStr.push('				<DataGridColumn dataField="RTN_WHSL_FEE_STAX" headerText="'+ parent.fn_text('stax')+ '" width="90" formatter="{numfmt}" 	textAlign="right"   id="num9"  />');//부가세
		layoutStr.push('			</DataGridColumnGroup>');
		layoutStr.push('			<DataGridColumn dataField="AMT_TOT"	headerText="'+ parent.fn_text('amt_tot')+ '" width="100" textAlign="right" formatter="{numfmt}" id="num10" />');//금액합계(원)
		layoutStr.push('			<DataGridColumn dataField="MFC_BIZRNM" headerText="'+ parent.fn_text('mfc_bizrnm')+ '"  	width="150" id="tmp1" textAlign="center" />');//생산자
		layoutStr.push('			<DataGridColumn dataField="MFC_BRCH_NM" headerText="'+ parent.fn_text('mfc_brch_nm')+ '" width="150" id="tmp2" textAlign="center" />');//직매장/공장
		layoutStr.push('			<DataGridColumn dataField="WRHS_CFM_DT" headerText="'+ parent.fn_text('wrhs_cfm_dt')+ '" width="100" id="tmp3" formatter="{datefmt2}" textAlign="center"/>');//입고확인일자
		layoutStr.push('			<DataGridColumn dataField="REAL_PAY_DT" headerText="'+parent.fn_text('real_pay_dt')+'" width="140" id="tmp4" textAlign="center"/>');//지급일시
		layoutStr.push('			<DataGridColumn dataField="CFM_QTY" headerText="'+ parent.fn_text('wrhs_qty')+ '" width="100" formatter="{numfmt}" id="num11" textAlign="right" />');//입고량
		layoutStr.push('			<DataGridColumn dataField="CAR_NO" headerText="'+ parent.fn_text('car_no')+ '" width="80" id="tmp5" textAlign="right" />');//차량번호
		layoutStr.push('			<DataGridColumn dataField="SYS_SE" headerText="'+ parent.fn_text('reg_se')+ '" width="100" id="tmp6" textAlign="center" />');//등록구분
		layoutStr.push('			<DataGridColumn dataField="WHSDL_BIZRNO" headerText="'+parent.fn_text('whsdl')+parent.fn_text('bizrno')+ '" formatter="{maskfmt1}" width="70" id="tmp7" textAlign="center" />');//도매업자 사업자 번호
		layoutStr.push('		</groupedColumns>');
		...
		layoutStr.push('				<DataGridFooterColumn formatter="{numfmt}" textAlign="right"/>');	
	...
	function totalsum3(column, data) {
		if(sumData) 
			return sumData.RTN_GTN_UTPC_TOT; 
		else 
			return 0;
	}
	...
	function totalsum5(column, data) {
		if(sumData) 
			return sumData.RTN_WHSL_FEE_UTPC_TOT; 
		else 
			return 0;
	}
	...
	function totalsum7(column, data) {
		if(sumData) 
			return sumData.RTN_RTL_FEE_UTPC_TOT; 
		else 
			return 0;
	}
			...
			<section class="secwrap"  id="params">
				...
				<div class="row" >
					...
					<div class="col" style="margin-left: 21px;">
						<div class="tit" id="ctnr_nm_txt" >빈용기명</div>  <!-- 빈용기명 -->
						<div class="box"  >
							<!-- <select id="CTNR_CD" style="width: 200px" class="i_notnull" ></select> -->
							<div data-ax5autocomplete="autocomplete" data-ax5autocomplete-config="{multiple: true}" style="max-width :600px; z-index: 0;"  id="data-ax5autocomplete_C" ></div>
					    ...
					    <div class="col">
					     ...
					     <div class="row" >
                                <div class="col"  style="width:300px;">
                                    <div class="tit" id="reg_se"  style="width:80px"></div> <!-- 등록구분-->
                                    
                                        <select id="SYS_SE" style="width: 119px"></select> 
 	
                                </div>
                                <div class="col"  style="width:300px">
                                    <div class="tit" id="whsl_se_cd" style="right:50px"></div> <!-- 도매업자구분 -->
                                        <select id="WHSL_SE_CD" style="width: 119px"></select>   
                                </div>
							<div class="col"  style="width:400px;">
									<div class="tit" id="enp_nm" style="width:119px; right:50px"></div><!--도매업자업체명-->
								<div class="box">
									<select id="WHSDL_BIZRNM" name="WHSDL_BIZRNM" style="x; right:100px"></select>
								</div>
							</div>
							<div class="col" style="position:absolute; right:100px">
								<div class="tit" id="whsdl_bizrno_txt" style="position:relative; width:135px; right:105px"></div><!--도매업자 사업자번호-->
								<div class="box" style="position:relative; right:110px">
									<input type="text" id=WHSDL_BIZRNO_DE style="width:250px" format="number"></select>
								</div>
							</div>	                                
