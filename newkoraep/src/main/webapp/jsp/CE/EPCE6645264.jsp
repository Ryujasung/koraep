   ...
   //엑셀저장
    ...
    function  fn_init(){
     	...
     	if(iniList != null && iniList != ""){
     		...
     		$("#DRCT_RTRVL_DT").text(kora.common.setDelim(iniList[0].DRCT_RTRVL_DT, "9999-99-99"));   //직접회수일자
     ...
     function fn_page(){
    	...
    	if(iniList[0].DRCT_RTRVL_STAT_CD != 'RG'){
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="CUST_NM"  		headerText="'+ parent.fn_text('cust2')+ '" textAlign="center" width="150" />');									//거래처
			layoutStr.push('			<DataGridColumn dataField="PRPS_NM"  		headerText="'+ parent.fn_text('prps_cd')+ '" textAlign="center" width="110" />');								//용도(유흥용/가정용)
			layoutStr.push('			<DataGridColumn dataField="CTNR_NM" 		headerText="'+ parent.fn_text('ctnr_nm')+ '" textAlign="center" width="210" />');								//빈용기명
			layoutStr.push('			<DataGridColumn dataField="CTNR_CD" 		headerText="'+ parent.fn_text('cd')+ '"  textAlign="center" width="90" />');									//코드
			layoutStr.push('			<DataGridColumn dataField="CPCT_NM"  		headerText="'+ parent.fn_text('cpct_cd')+'(ml)'+ '" textAlign="center" width="110" />');						//용량(ml)
			layoutStr.push('			<DataGridColumn dataField="DRCT_RTRVL_QTY" id="num1"  headerText="'+ parent.fn_text('drct_rtrvl_qty2')+ '" width="130" formatter="{numfmt}" textAlign="right" />');		//직접회수량(개)
			layoutStr.push('			<DataGridColumn dataField="DRCT_PAY_GTN" id="num2"  headerText="'+ parent.fn_text('drct_pay_dps2')+ '" width="150" formatter="{numfmt}" textAlign="right" />');//직접지급보증금(원)
			layoutStr.push('			<DataGridColumn dataField="DRCT_PAY_FEE"  id="num3"  headerText="'+ parent.fn_text('drct_pay_fee')+ '" width="150" formatter="{numfmt}" textAlign="right" />');	//직접지급수수료(원)
			layoutStr.push('		</groupedColumns>');
