...
<script type="text/javaScript" language="javascript" defer="defer">
	...
	var mfc_bizrnmList;		//생산자원본
	var ctnrNmList_C ;	//용기코드원본
	var ctnrNmList ;
    ...
    $(function() {
		...
		var statCdList 		= jsonObject($("#statCdList").val());
		...
		kora.common.setEtcCmBx2(statCdList, "","", $("#DRCT_RTRVL_STAT_CD"), "ETC_CD", "ETC_CD_NM", "N" ,'T');
  ...
  //엑셀저장
    ...
    function fn_mfc_brch(){
    	if(acSelected_M.length ==1){
    			var url 		= "/CE/EPCE6150601_192.do" 
	    		var input 	= {};
	    		input["BIZRID"]	=	acSelected_M[0].MFC_BIZRID;
	    		input["BIZRNO"]	=	acSelected_M[0].MFC_BIZRNO;
	    		ajaxPost(url, input, function(rtnData) {
	 				if ("" != rtnData && null != rtnData) {   
	 					kora.common.setEtcCmBx2(rtnData.mfcBrchList, "","", $("#MFC_BRCH_NM"), "BRCH_ID_NO", "BRCH_NM", "N" ,'T'); //직매장
	 				}else{
	 					alertMsg("error");
	 				}
	 			},false);
	...
	function fn_sel(chartYn,page){
		...
		var url = "/CE/EPCE6150601_19.do";
		var input = {};
		if(page !="P"){
			input['START_DT'] = $("#START_DT").val();
			input['END_DT'] = $("#END_DT").val();
			input['MFC_BRCH_NM'] = $("#MFC_BRCH_NM option:selected").val();
			input['DRCT_RTRVL_STAT_CD'] = $("#DRCT_RTRVL_STAT_CD option:selected").val();
			...
			if(rtnData != null && rtnData != ""){
				...
				if(chartYn == "Y"){ //조회버튼 클릭 시 챠트조회
					...
					for(var i in rtnData.searchList2){
						drctPayGtn += Number(rtnData.searchList2[i].DRCT_PAY_GTN);
						drctPayFee += Number(rtnData.searchList2[i].DRCT_PAY_FEE);
					}
					
					$('#DRCT_PAY_GTN').text(kora.common.format_comma(drctPayGtn));
					$('#DRCT_PAY_FEE').text(kora.common.format_comma(drctPayFee));
					$('#TOT').text(kora.common.format_comma(drctPayGtn + drctPayFee));
...
// 			kora.common.hideLoadingBar(dataGrid, gridRoot);// 그리드 loading bar off
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="CUST_BIZRNM"	headerText="'+ parent.fn_text('cust2')+ '" width="140" />');
			layoutStr.push('			<DataGridColumn dataField="CUST_BIZRNO" headerText="'+ parent.fn_text('cust_bizrno')+ '" width="130" formatter="{maskfmt}" />');
			layoutStr.push('			<DataGridColumn dataField="CTNR_NM" headerText="'+ parent.fn_text('ctnr_nm')+ '" width="200" />');
			layoutStr.push('			<DataGridColumn dataField="PRPS_NM" headerText="'+ parent.fn_text('prps_cd')+ '" width="130" />');
			...
			layoutStr.push('			<DataGridColumn dataField="CPCT_NM" headerText="'+ parent.fn_text('cpct')+'"	width="130" />');
			layoutStr.push('			<DataGridColumn dataField="DRCT_RTRVL_QTY" headerText="'+ parent.fn_text('drct_rtrvl_qty2')+ '" width="130" textAlign="right" formatter="{numfmt}" id="sum1" />');
			layoutStr.push('			<DataGridColumn dataField="DRCT_PAY_GTN" headerText="'+ parent.fn_text('drct_pay_dps')+ '" width="130" textAlign="right" formatter="{numfmt}" id="sum2" />');
			layoutStr.push('			<DataGridColumn dataField="DRCT_PAY_FEE" headerText="'+ parent.fn_text('drct_pay_fee')+ '" width="130" textAlign="right" formatter="{numfmt}" id="sum3" />');
			layoutStr.push('			<DataGridColumn dataField="CRCT_DRCT_RTRVL_QTY" headerText="'+ parent.fn_text('crct_drct_rtrvl_qty2')+ '" width="130" textAlign="right" formatter="{numfmt}" id="sum4" />');
			layoutStr.push('			<DataGridColumn dataField="EXCA_DRCT_RTRVL_QTY" headerText="'+ parent.fn_text('exca_drct_rtrvl_qty2')+ '" width="130" textAlign="right" formatter="{numfmt}" id="sum5" />');
			layoutStr.push('			<DataGridColumn dataField="AREA_NM" headerText="'+ parent.fn_text('area_nm')+ '" width="130" textAlign="center"/>');
			layoutStr.push('			<DataGridColumn dataField="BIZR_TP_NM" headerText="'+ parent.fn_text('bizr_tp')+ '" width="130" textAlign="center" />');			
			layoutStr.push('		</groupedColumns>');
    ...
    function totalsum4(column, data) {
        if(sumData) 
            return sumData.CRCT_DRCT_RTRVL_QTY; 
        else 
            return 0;
    }
    ...
    function totalsum5(column, data) {
        if(sumData) 
            return sumData.EXCA_DRCT_RTRVL_QTY; 
        else 
            return 0;
    }
	...
	layoutStrChart.push('             <Column3DSeries labelPosition="outside" yField="DRCT_PAY_GTN" displayName="직접지급보증금" showValueLabels="[]"  outsideLabelYOffset="-10" showTotalLabel="true" halfWidthOffset="0" itemRenderer="CylinderItemRenderer">');
	layoutStrChart.push('                  <showDataEffect>');
	layoutStrChart.push('                     <SeriesInterpolate/>');
	layoutStrChart.push('                </showDataEffect>');
	layoutStrChart.push('            </Column3DSeries>');
	layoutStrChart.push('            <Column3DSeries labelPosition="outside" yField="DRCT_PAY_FEE" displayName="직접지급수수료" showValueLabels="[]"  outsideLabelYOffset="-10" showTotalLabel="true" halfWidthOffset="0" itemRenderer="CylinderItemRenderer">');
	layoutStrChart.push('                <showDataEffect>');
	layoutStrChart.push('                     <SeriesInterpolate/>');
	layoutStrChart.push('                </showDataEffect>');
	layoutStrChart.push('            </Column3DSeries>');
	...
	// 차트 데이터
	var chartData = [{BIZRNM:"", DRCT_PAY_GTN:0, DRCT_PAY_FEE:0}];
...
<body>
    <div class="iframe_inner" >
    		<input type="hidden" id="statCdList" value="<c:out value='${statCdList}' />" />
		...
		<section class="secwrap"  id="params">
				<div class="srcharea" > 
					<div class="row" >
						...
						<div class="col">
							<div class="tit" id="stat_txt" style="width: 84px"></div>
							<div class="box">
								<select id="DRCT_RTRVL_STAT_CD" style="width: 179px" class="i_notnull" ></select>
							</div>
						</div>
					</div> <!-- end of row -->
                    <div class="row">
					...
					<div class="row">
						...
						<div class="col">
							...
							<div class="box">
								<select id="MFC_BRCH_NM" name="MFC_BRCH_NM" style="width: 179px"></select>
							</div>
						</div>
					</div> <!-- end of row -->
					...
					<div class="row">
						<div class="col"  style="width: 50%">
							<div class="tit" id="cust2_txt""></div>
							<div class="box">
								<input type="text" id="CUST_BIZRNM" name="CUST_BIZRNM" style="width: 200px" />
							</div>
						</div>
					...
					<div class="info_tbl" style="">
						<table>
							...
							<tbody>
								...
								<tr>
									<td id="DRCT_PAY_GTN"></td>
									<td id="DRCT_PAY_FEE"></td>
									<td class="bold c_01" id="TOT"></td>
								</tr>
							</tbody>
						</table>
					</div>
