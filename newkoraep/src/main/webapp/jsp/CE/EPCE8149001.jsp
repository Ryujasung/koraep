...
<script src="/js/kora/paging_common.js"></script>
	...
	$(document).ready(function(){
		...
		$('#sel_cnt').text(parent.fn_text('sel_cnt'));
				
		
		/*상세조회 바로이동*/
		var notiSeq = "<c:out value='${NOTI_SEQ}' />";
		if( notiSeq != ''){
			var sAction="/CE/EPCE8149093.do";

			//페이지이동
		    kora.common.goPageD(sAction, {"NOTI_SEQ": notiSeq}, {});
		}
		/*상세조회 바로이동*/
		
		
		/* 페이징 사용 등록 */
		gridTotalRowCount = "<c:out value='${totalCnt}[0].CNT' />";
	...
	function fn_dtl_sel_lk(noti_seq){
		...
		//페이지이동
	    kora.common.goPageD(sAction, {"NOTI_SEQ": noti_seq}, {"cate":$("#cate").val(), "word":$("#word").val(), "NOW_PAGE" : gridCurrentPage, "FN_CALLBACK" : "search"});
...
// 			layoutStr.push('			<DataGridColumn dataField="NOTI_SEQ" headerText="'+111+'" textAlign="center"   width="4%" />');
// 		}else{
// 			layoutStr.push('			<DataGridColumn dataField="NOTI_SEQ" headerText="'+parent.fn_text('no')+'" textAlign="center"   width="4%" />');
// 		}
		layoutStr.push('			<DataGridColumn dataField="NOTI_NO" headerText="'+parent.fn_text('no')+'" textAlign="center"   width="4%" />');
		layoutStr.push('			<DataGridColumn dataField="SBJ" headerText="'+parent.fn_text('sbj')+'" itemRenderer="HtmlItem"  width="76%" />');
		layoutStr.push('			<DataGridColumn dataField="REG_DTTM" headerText="'+parent.fn_text('reg_dt')+'" formatter="{datefmt2}" textAlign="center"  width="14%"/>');		
		layoutStr.push('			<DataGridColumn dataField="SEL_CNT" headerText="'+parent.fn_text('sel_cnt')+'" formatter="{numfmt}" textAlign="center"  width="6%" />');
		layoutStr.push('		</columns>');
