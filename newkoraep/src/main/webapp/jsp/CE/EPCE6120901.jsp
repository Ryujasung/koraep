		...
		ajaxPost(url, input, function(rtnData){
			...
			/* 	if(chartYn == "Y"){ //조회버튼 클릭 시 챠트조회
					...
					for(var i in rtnData.searchList){
						regCnt += Number(rtnData.searchList[i].REG_CNT);
					}
					
					$('#REG_CNT').text(kora.common.format_comma(regCnt));
...
// 		kora.common.hideLoadingBar(dataGrid, gridRoot);// 그리드 loading bar off
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('	<DataGrid headerColors="[#EFF6FC,#EFF6FC]" verticalAlign="middle" id="dg1" headerHeight="35" draggableColumns="true" sortableColumns="true" textAlign="center" >');
			layoutStr.push('		<groupedColumns>');   	
			layoutStr.push('			<DataGridColumn dataField="index" headerText="'+parent.fn_text('sn')+'" itemRenderer="IndexNoItem" width="50"/>');
			layoutStr.push('			<DataGridColumn dataField="BIZRNM" headerText="'+ parent.fn_text('mfc_bizrnm')+ '" />');
			layoutStr.push('			<DataGridColumn dataField="REG_CNT" headerText="'+ parent.fn_text('reg_cnt')+ '" width="150" textAlign="right" formatter="{numfmt}" id="sum" />');
			layoutStr.push('			<DataGridColumn dataField="LST_REG_HR" headerText="'+ parent.fn_text('lst_reg_hr')+'" width="150" />');
			layoutStr.push('		</groupedColumns>');
	...
	layoutStrChart.push('             <Column3DSeries labelPosition="outside" yField="REG_CNT" displayName="등록건수" outsideLabelYOffset="-10" showTotalLabel="true" halfWidthOffset="0" itemRenderer="CylinderItemRenderer">');
	...
	// 차트 데이터
	var chartData = [{BIZRNM:"", REG_CNT:0}];
	  */
	/**
    ...
    <div class="iframe_inner" >
		...
		<section class="secwrap"  id="params">
				<div class="srcharea" > 
					<div class="row">
						<div class="col">
							<div class="tit" id="sel_dt_txt""></div>
							<div class="box">
								<div class="calendar">
									<input type="text" id="START_DT" name="from" style="width: 179px;" class="i_notnull">
								</div>
							</div>
						</div>
						<div class="btn" id="CR">
		...
		<!-- 	<section class="secwrap2 mt15">
				<div class="boxarea" style="width:10%;margin:0px;">
				...
				<div style="width:90%;height:300px;float:right;padding:0 20px 0 15px;margin:0px">
					차트가 삽입될 DIV
			...
			<div class="boxarea mt15">
				<div id="gridHolder" style="height: 510px; background: #FFF;"></div>
			</div>
