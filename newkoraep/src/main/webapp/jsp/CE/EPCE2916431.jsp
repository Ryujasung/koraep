...
//소매수수료 적용여부 ㅇㄷ----------------------------------------------------------------------------------------------------
	...
	insRow = function(gbn) {
		...
		for(var i=0; i<ctnr_nm.length; i++){
			if(ctnr_nm[i].CTNR_CD == ctnrCd) {
			    ...
			    input["RSRC_DOC_NO"]			= 	gridList[0].RSRC_DOC_NO	   	 // 실태조사문서번호
			   	input["RTN_DT"]						=	gridList[0].RTN_DT					//반화일자
	...
	function fn_reg(){
		...
		if(INQ_PARAMS.PARAMS.WRHS_DOC_NO !=null){
			url = "/CE/EPCE2916431_21.do"; 
		//입고문서 없을경우
		}else if(INQ_PARAMS.PARAMS.WRHS_DOC_NO ==null){
			url = "/CE/EPCE2916431_09.do"; 
		}
		...
		for(var i=0;i<collection.getLength(); i++){
				...
				if(INQ_PARAMS.PARAMS.WRHS_DOC_NO !=null){	//입고문서있을경우 최초등록자 아이디
					tmpData["REG_PRSN_ID"]		= cfm_gridList[0].REG_PRSN_ID
					tmpData["REG_DTTM"]			= cfm_gridList[0].REG_DTTM
				}
     ...
     //취소
  	function fn_cnl(){
		...
		var	input	={};
		input["RTN_DOC_NO"]	= 	gridList[0].RTN_DOC_NO	   		 // 반환문서번호 템프증빙사진 삭제
...
/* 
		var url ='/CE/EPCE2916431_092.do'	//템프에 저장		
		showLoadingBar()
	 	ajaxPost(url, input, function(rtnData){
			if(rtnData.RSLT_CD != "0000"){
				alertMsg(rtnData.RSLT_MSG);
  ...
  //선택한 행 입력창에 값 넣기
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('	<DataGrid headerColors="[#EFF6FC,#EFF6FC]" verticalAlign="middle" id="dg2" headerWordWrap="true"  draggableColumns="true" sortableColumns="true"  headerHeight="35">');
