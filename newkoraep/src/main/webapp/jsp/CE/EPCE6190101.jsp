        ...
        ajaxPost(url, input, function(rtnData) {
            if ("" != rtnData && null != rtnData) {
                ...
                $.each(rtnData.selList, function(i, v){
                    var pieDataLine = {};
                    pieDataLine["TITLE_IN"]  = v.AREA_NM + "(" + "회수" + ")";
                    pieDataLine["TITLE_OUT"] = v.AREA_NM + "(" + "출고" + ")";
...
//             document.body.style.cursor = "default";
    ...
    function clickFunction(code, label, data) {
        ...
        ajaxPost(url, input, function(rtnData) {
            if ("" != rtnData && null != rtnData) {
                ...
                $.each(rtnData.selList, function(i, v){
                    var pieDataLine = {};
                    pieDataLine["TITLE_IN"]  = v.BIZRNM + "(" + "회수" + ")";
    ...
    function divDataTipFunction(seriesId, code, label, data) {
        ...
        if(INQ_PARAMS.SEL_PARAM ==null) return; 
        var DLIVY_QTY =0;               //출고량
        var DLIVY_QTY_RT =0;            //출고비율
        var CFM_QTY=0;                  //입고량
        var CFM_QTY_RT=0;           //입고비율  
        var CFM_GTN=0;                  //입고보증금
        var CFM_GTN_TOT_RT=0;       //입고보증금비율
        //2664-1134
        //console.log(JSON.stringify(selList))
        for(var i=0; i<selList.length;i++){
            if(selList[i].AREA_CD == code){
                DLIVY_QTY      = kora.common.gfn_setComma(selList[i].DLIVY_QTY);
                DLIVY_QTY_RT   = kora.common.trunc(selList[i].DLIVY_QTY_RT,3);
                CFM_QTY        = kora.common.gfn_setComma(selList[i].CFM_QTY);
                CFM_QTY_RT     = kora.common.trunc(selList[i].CFM_QTY_RT,3); 
                CFM_GTN        = kora.common.gfn_setComma(selList[i].CFM_GTN);
                CFM_GTN_TOT_RT = kora.common.trunc(selList[i].CFM_GTN_RT,3); 
                break;
            } 
        }
                ...
                +'<div class="info_tbl" style="">'
                    +'<table>'
                        ...
                        +'<tbody>'
                            ...
                            +'<tr>'
                                    +'<td>출고량</td>'
                                    +'<td id="" style="text-align:right;padding-right:5px">'+DLIVY_QTY+'</td>'
                                    +'<td id="" style="text-align:right;padding-right:5px">'+DLIVY_QTY_RT+' %</td>'
                            +'</tr>'
                            +'<tr>'
                                    +'<td>회수량</td>'
                                    +'<td id="" style="text-align:right;padding-right:5px">'+CFM_QTY+'</td>'
                                    +'<td id="" style="text-align:right;padding-right:5px">'+CFM_QTY_RT+' %</td>'
                            +'</tr>'
                            +'<tr>'
                                    +'<td>지급보증금</td>'
                                    +'<td id="" style="text-align:right;padding-right:5px">'+CFM_GTN+'</td>'
                                    +'<td id="" style="text-align:right;padding-right:5px">'+CFM_GTN_TOT_RT+' %</td>'  
                            +'</tr>'
                        +'</tbody>'
                    +'</table>'
                +'</div>'
    ...
    function fnSetGrid() {
        ...
        layoutStr.push('            <DataGridColumn dataField="index" headerText="'+parent.fn_text('rank')+'" itemRenderer="IndexNoItem" textAlign="center" width="50" />');
        layoutStr.push('            <DataGridColumn dataField="AREA_NM" headerText="'+parent.fn_text('area')+'" textAlign="center" width="150" />');
        layoutStr.push('            <DataGridColumn dataField="DLIVY_QTY" headerText="'+parent.fn_text('dlivy')+'" width="120" formatter="{numfmt}" textAlign="right" id="num1" />');
        layoutStr.push('            <DataGridColumn dataField="DLIVY_QTY_RT" headerText="비율" id="dg1col1" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr.push('            <DataGridColumn dataField="CFM_QTY" headerText="'+parent.fn_text('wrhs')+'"  width="120" formatter="{numfmt}" textAlign="right" id="num2" />');
        layoutStr.push('            <DataGridColumn dataField="CFM_QTY_RT" headerText="비율" id="dg1col3" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr.push('            <DataGridColumn dataField="DRCT_RTRVL_QTY" headerText="'+parent.fn_text('drct_rtrvl')+'"  width="120" formatter="{numfmt}" textAlign="right" id="num3"/>');
        layoutStr.push('            <DataGridColumn dataField="DRCT_RTRVL_QTY_RT" headerText="비율" id="dg1col3" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr.push('            <DataGridColumn dataField="EXCH_QTY" headerText="'+parent.fn_text('exch')+'"  width="120" formatter="{numfmt}" textAlign="right" id="num4" />');
        layoutStr.push('            <DataGridColumn dataField="RTRVL_QTY" headerText="'+parent.fn_text('rtrvl_sum')+'"  width="120" formatter="{numfmt}" textAlign="right" id="num5" />');
        layoutStr.push('            <DataGridColumn dataField="RTRVL_QTY_RT" headerText="비율" id="dg1col3" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr.push('            <DataGridColumn dataField="CFM_GTN" headerText="'+parent.fn_text('drct_pay_dps')+'"  width="120" formatter="{numfmt}" textAlign="right" id="num6"/>');
        layoutStr.push('            <DataGridColumn dataField="CFM_GTN_RT" headerText="비율" id="dg1col3" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr.push('            <DataGridColumn dataField="QTY_RT" headerText="'+parent.fn_text('dlivy_cpr_rtrvl_rt')+'" id="dg1col5" width="100" formatter="{numfmt1}" textAlign="right" />');
        layoutStr.push('        </groupedColumns>');
    ...
    function fnSetGrid2() {
        ...
        layoutStr2.push('            <DataGridColumn dataField="BIZRNM" headerText="'+parent.fn_text('area')+'" textAlign="center" width="150" />');
        layoutStr2.push('            <DataGridColumn dataField="DLIVY_QTY" headerText="'+parent.fn_text('dlivy')+'" width="120" formatter="{numfmt}" textAlign="right"  id="num11" />');
        layoutStr2.push('            <DataGridColumn dataField="DLIVY_QTY_RT" headerText="비율" id="dg1col1" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr2.push('            <DataGridColumn dataField="CFM_QTY" headerText="'+parent.fn_text('wrhs')+'" width="120" formatter="{numfmt}" textAlign="right" id="num22"  />');
        layoutStr2.push('            <DataGridColumn dataField="CFM_QTY_RT" headerText="비율" id="dg1col3" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr2.push('            <DataGridColumn dataField="DRCT_RTRVL_QTY" headerText="'+parent.fn_text('drct_rtrvl')+'" width="120" formatter="{numfmt}" textAlign="right" id="num33"  />');
        layoutStr2.push('            <DataGridColumn dataField="DRCT_RTRVL_QTY_RT" headerText="비율" id="dg1col3" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr2.push('            <DataGridColumn dataField="EXCH_QTY" headerText="'+parent.fn_text('exch')+'"  width="120" formatter="{numfmt}" textAlign="right" id="num44"  />');
        layoutStr2.push('            <DataGridColumn dataField="RTRVL_QTY" headerText="'+parent.fn_text('rtrvl_sum')+'"  width="120" formatter="{numfmt}" textAlign="right" id="num55"  />');
        layoutStr2.push('            <DataGridColumn dataField="RTRVL_QTY_RT" headerText="비율" id="dg1col3" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr2.push('            <DataGridColumn dataField="CFM_GTN" headerText="'+parent.fn_text('drct_pay_dps')+'"  width="120" formatter="{numfmt}" textAlign="right" id="num66"  />');
        layoutStr2.push('            <DataGridColumn dataField="CFM_GTN_RT" headerText="비율" id="dg1col3" width="60" formatter="{numfmt1}" textAlign="right" />');
        layoutStr2.push('            <DataGridColumn dataField="QTY_RT" headerText="'+parent.fn_text('dlivy_cpr_rtrvl_rt')+'" id="dg1col5" width="100" formatter="{numfmt1}" textAlign="right" />');
        layoutStr2.push('        </groupedColumns>');
		...
		layoutStr2.push('				<DataGridFooterColumn/>');
		layoutStr2.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{num33}" formatter="{numfmt}" textAlign="right"/>');		
		layoutStr2.push('				<DataGridFooterColumn/>');
		layoutStr2.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{num44}" formatter="{numfmt}" textAlign="right"/>');
		layoutStr2.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{num55}" formatter="{numfmt}" textAlign="right"/>');		
        layoutStr2.push('                <DataGridFooterColumn/>');
		layoutStr2.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{num66}" formatter="{numfmt}" textAlign="right"/>');        
        layoutStr2.push('                <DataGridFooterColumn/>');
		layoutStr2.push('				<DataGridFooterColumn/>');	
