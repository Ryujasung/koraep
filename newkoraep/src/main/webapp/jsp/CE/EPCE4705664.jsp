...
<script type="text/javaScript" language="javascript" defer="defer">
	 ...
	 var cfm_gridList;	//입고정정 그리드 데이터
	 var crct_gridList;	//입고정정 재등록 그리드 데이터
	 var mnulYn="N";
     ...
     $(function() {
		...
		$('#wrhs_crct_re_data').text('('+ parent.fn_text('wrhs_crct_re_data') +')'); 	//입고정정재등록내역
    ...
    function  fn_init(){
        ...
        $("#prtForm").find("#WRHS_CRCT_DOC_NO_RE").val(INQ_PARAMS.PARAMS.WRHS_CRCT_DOC_NO_RE);
        $("#prtForm").find("#WRHS_DOC_NO").val(INQ_PARAMS.PARAMS.WRHS_DOC_NO);
        $("#prtForm").find("#MNUL_YN").val(mnulYn);
    }
    ...
    //삭제체크
    function fn_del_chk(){
	   	 var wrhs_crct_stat_cd =iniList[0].WRHS_CRCT_STAT_CD;
		 
		 if(wrhs_crct_stat_cd !="R" && wrhs_crct_stat_cd !="T" ){
	    	alertMsg("정정등록, 정정반려 상태인 건에 대해서만 처리 가능합니다.");
	    	return    		 
		 }
		 confirm("표시된 정정 등록 내역이 모두 삭제됩니다.\n\n계속 진행하시겠습니까?","fn_del");
    }
     
    //삭제
    function fn_del(){
    	var url = "/CE/EPCE4705664_04.do"
    	var input ={};
    	input["WRHS_CRCT_DOC_NO"] 	= iniList[0].WRHS_CRCT_DOC_NO;	//입고정정문서번호
    	input["WRHS_DOC_NO"] 			= iniList[0].WRHS_DOC_NO;			//입고문서번호
    	input["STAT_CD_CHK"] 			= "2";									//상태체크
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			</DataGridColumnGroup>');
			layoutStr.push('			<DataGridColumn dataField="CFM_AMT" 						headerText="'+ parent.fn_text('amt_tot')+ '" 		width="70"  	formatter="{numfmt}" 	textAlign="right"  id="num9"  />');//금액합계(원)
			...
			layoutStr2.push('			<DataGridColumn dataField="RTN_QTY"  	 headerText="'+ parent.fn_text('rtn_qty')+'"				width="80"	 formatter="{numfmt}" textAlign="right" id="num0" />');															//반환량
			...
			layoutStr2.push('			<DataGridColumn dataField="CRCT_WRHS_CFM_DT" headerText="'+ parent.fn_text('crct_wrhs_dt')+ '" 		width="120"  	formatter="{datefmt2}" 	textAlign="center"    />');//입고정정확인일자
...
<body>
    <div class="iframe_inner" id="testee" >
			...
			<div class="boxarea mt10" id="crctDiv">
				<div class="h4group" >
			...
			<div class="boxarea mt10">
				<div class="h4group" >
					<h5 class="tit"  style="font-size: 16px;" id="wrhs_crct_re_data"><h5>
				</div>
    ...
    <form name="prtForm" id="prtForm">
        <input type="hidden" name="CRF_NAME" value="EPCMOROAt_RE.crf" /> <!-- 필수 -->
        ...
        <input type="hidden" name="WRHS_CRCT_DOC_NO_RE" id="WRHS_CRCT_DOC_NO_RE"/>
        <input type="hidden" name="WRHS_DOC_NO" id="WRHS_DOC_NO"/>
        <input type="hidden" name="MNUL_YN" id="MNUL_YN"/>
    </form>
</body>
</html>