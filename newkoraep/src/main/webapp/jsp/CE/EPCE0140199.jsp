...
<script src="/js/kora/paging_common.js"></script>
	...
	<script type="text/javaScript" language="javascript" defer="defer">
		...
		$(document).ready(function(){
			...
			fn_btnSetting();
			$('#title_sub').text('<c:out value="휴면계정메일발송" />');
			
			$('#last_login_dt').text(parent.fn_text('last_login_dt'));
			
			$('#user_nm_sel').text(parent.fn_text('user_nm'));
			$('#user_id_sel').text(parent.fn_text('id'));
			$('#user_stat_sel').text(parent.fn_text('user_stat'));
			$('#mail_stat_sel').text(parent.fn_text('mail_stat'));
			...
			$('#ath_grp_nm_sel').text(parent.fn_text('ath_grp'));
			...
			var mailStatList = jsonObject($('#mailStatList').val());
			...
			kora.common.setEtcCmBx2(mailStatList, "", "N", $("#MAIL_STAT_CD_SEL"), "ETC_CD", "ETC_CD_NM", "N", "T");
		...
		function fn_reg(){
			...
			INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE0140199.do";
		...
		function fn_sel(){
			var input	={};
			var url = "/CE/EPCE0140199_19.do";
			var start_dt = $("#START_DT").val();
			//start_dt = start_dt.replace(/-/gi, "");
			if(start_dt == "" ||start_dt == null ){
				alertMsg("최근 로그인 기준일수를 입력해주시기바랍니다.");
				return;
			}
			
			/* if(!kora.common.fn_validDate($("#START_DT").val())){ 
				alertMsg("날짜를 확인해주세요.\n정상적인 날짜가 아닙니다."); 
				return; 
			} */
			input["BIZR_TP_CD_SEL"] = $("#BIZR_TP_CD_SEL option:selected").val();
			input["START_DT"] = $("#START_DT").val();
			input["MAIL_STAT_CD_SEL"] = $("#MAIL_STAT_CD_SEL").val();
			input["USER_SE_CD_SEL"] = $("#USER_SE_CD_SEL").val();
		 ...
		 */
		 function fn_set_grid() {
			 ...
			 layoutStr.push('	<DataGridColumn dataField="USER_ID"  headerText="'+parent.fn_text('id')+'" width="100" itemRenderer="HtmlItem"/>');
			 ...
			 layoutStr.push('	<DataGridColumn dataField="LST_LGN_DTTM"  headerText="'+parent.fn_text('lst_lgn_dttm')+'" width="100"/>');
			 
			 layoutStr.push('	<DataGridColumn dataField="SEND_MAIL_YN"  headerText="'+parent.fn_text('send_mail_yn')+'" width="100"/>');
			 layoutStr.push('	<DataGridColumn dataField="SEND_MAIL_DT"  headerText="'+parent.fn_text('send_mail_dt')+'" width="150"/>');
			 layoutStr.push('	<DataGridColumn dataField="SEND_MAIL_TITLE"  headerText="'+parent.fn_text('send_mail_title')+'" width="250"/>');
			 layoutStr.push('	<DataGridColumn dataField="SEND_MAIL_CONTENTS"  headerText="'+parent.fn_text('send_mail_contents')+'" width="350"/>');
		...
		function fn_upd(){
			...
			for(var i=0; i<chkLst.length; i++){
				...
				//이미 메일 발송한 경우
				if(chkLst[i].SEND_MAIL_YN == "발송"){
					alertMsg("이미 발송 상태인 회원이 선택되었습니다.\n다시 한 번 확인하시기 바랍니다.");
					return;
				}
				//이메일정보가 없는 탈퇴회원
				if (chkLst[i].EMAIL ==null || chkLst[i].EMAIL ==" "){
...
<input type="hidden" id="mailStatList" value="<c:out value='${mailStatList}' />"/>
	...
	<div class="iframe_inner">
		...
		<section class="secwrap">
			<div class="srcharea" id="sel_params">
				<div class="row">
					<div class="col">
						<div class="tit" id="last_login_dt" style="width: 150px"></div>
						<div class="box">						
							<input type="number" id="START_DT" name="START_DT" style="width: 120px;"><!--시작날짜  -->
						</div>
					</div>
					<div class="col">
					...
					<div class="col">
						<div class="tit" id="mail_stat_sel"></div>
						<div class="box">
							<select id="MAIL_STAT_CD_SEL" name="MAIL_STAT_CD_SEL" style="width: 159px;">
							</select>
						</div>
					</div>
					<div class="col">
					...
					<div class="col">
						<div class="tit" id="ath_grp_nm_sel"></div>
						<div class="box">						
							<input id="ATH_GRP_NM_SEL" name="ATH_GRP_NM_SEL" style="width: 159px;" type="text"/>
						</div>
					</div>
					<div class="btn" id="CR">
					</div>
		...
		<div class="boxarea mt10">
			...
			<div class="gridPaging" id="gridPageNavigationDiv" style="display: none;"></div><!-- 페이징 사용 등록 -->
		</div>
