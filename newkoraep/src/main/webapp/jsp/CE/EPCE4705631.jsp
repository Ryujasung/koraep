...
<script type="text/javaScript" language="javascript" defer="defer">
     ...
     $(function() {
    	...
    	INQ_PARAMS 	=  jsonObject($("#INQ_PARAMS").val());	
   		...
   		$.each(cfm_gridList, function(i, v){
   			if(v.WRHS_CRCT_DOC_NO != null && v.WRHS_CRCT_DOC_NO != ''){
   				cfm_gridList2.push(v);
   				wrhsCrctDocNo = v.WRHS_CRCT_DOC_NO;
   			}
		});
		...
		//날짜 셋팅
		 $('#CRCT_RTN_DT').YJcalendar({  
				triggerBtn : true,
				dateSetting : kora.common.getDate("yyyy-mm-dd", "D", 0, iniList[0].RTN_DT).replaceAll('-','')
    ...
    function  fn_init(){
		...
		$("#MFC_BIZRNO")    			.text(kora.common.setDelim(iniList[0].MFC_BIZRNO_DE, "999-99-99999"));				//생산자 사업자번호
		...
		$("#WHSDL_BIZRNO")      	.text(kora.common.setDelim(iniList[0].WHSDL_BIZRNO_DE, "999-99-99999"));        //도매업자 사업자번호                   
    ...
    function fn_prps_cd(){
			...
			var input ={};
			if( $("#PRPS_CD").val() !=""){
				input["CUST_BIZRID"] 		= 	iniList[0].WHSDL_BIZRID		//도매업자아이디                     
				input["CUST_BIZRNO"] 	= 	iniList[0].WHSDL_BIZRNO	//도매업자사업자번호                   
				input["CUST_BRCH_ID"] 	= 	iniList[0].WHSDL_BRCH_ID	//도매업자 지점 아이디             
				input["CUST_BRCH_NO"] 	= 	iniList[0].WHSDL_BRCH_NO	//도매업자 지점 번호                      
				input["MFC_BIZRID"] 		= 	iniList[0].MFC_BIZRID			//생산자 아이디             
				input["MFC_BIZRNO"] 		= 	iniList[0].MFC_BIZRNO		//생산자 사업자번호           
				input["MFC_BRCH_ID"] 	= 	iniList[0].MFC_BRCH_ID		//생산자 직매장/공장 아이디      
				input["MFC_BRCH_NO"] 	= 	iniList[0].MFC_BRCH_NO		//생산자 직매장/공장 번호               
				input["WRHS_CFM_DT"] 	=	iniList[0].WRHS_CFM_DT		//반환일자
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	insRow = function(gbn) {
		...
		for(var i=0; i<ctnr_nm.length; i++){
			...
			if(ctnr_nm[i].CTNR_CD == ctnrCd) { //같은 용기코드가 있을경우
			    ...
			    input["WHSDL_BIZRID"] 			= 	iniList[0].WHSDL_BIZRID;		//도매업자아이디
			    input["WHSDL_BIZRNO"] 			= 	iniList[0].WHSDL_BIZRNO;		//도매업자사업자번호
			    input["WHSDL_BRCH_ID"] 		= 	iniList[0].WHSDL_BRCH_ID;		//도매업자 지점 아이디
			    input["WHSDL_BRCH_NO"] 		= 	iniList[0].WHSDL_BRCH_NO;		//도매업자 지점 번호
			    input["MFC_BIZRID"] 				= 	iniList[0].MFC_BIZRID;				//생산자 아이디
			    input["MFC_BIZRNO"] 				= 	iniList[0].MFC_BIZRNO;			//생산자 사업자번호
			    input["MFC_BRCH_ID"] 			= 	iniList[0].MFC_BRCH_ID;			//생산자 직매장/공장 아이디
			    input["MFC_BRCH_NO"] 			= 	iniList[0].MFC_BRCH_NO;			//생산자 직매장/공장 번호
			    input["WRHS_DOC_NO"]			= 	iniList[0].WRHS_DOC_NO;	   	// 입고문서번호
	...
	function fn_reg(){
		...
		var crct_tot=0;
		var collection = gridRoot2.getCollection();
		var url = "/CE/EPCE4705631_09.do"
		
		for(var i=0;i<collection.getLength(); i++){
	 		var tmpData = gridRoot2.getItemAt(i);
			tmpData["WRHS_CRCT_STAT_CD"]	 = "R";
			//tmpData["EXCA_STD_CD"]			 = INQ_PARAMS.EXCA_PARAM.EXCA_STD_CD	//정산기준코드
			
			//사업자정보 넣어줌..
			tmpData["WHSDL_BIZRID"] 			= 	iniList[0].WHSDL_BIZRID;		//도매업자아이디
			tmpData["WHSDL_BIZRNO"] 		= 	iniList[0].WHSDL_BIZRNO;		//도매업자사업자번호
			tmpData["WHSDL_BRCH_ID"] 		= 	iniList[0].WHSDL_BRCH_ID;		//도매업자 지점 아이디
			tmpData["WHSDL_BRCH_NO"] 		= 	iniList[0].WHSDL_BRCH_NO;		//도매업자 지점 번호
		    tmpData["MFC_BIZRID"] 				= 	iniList[0].MFC_BIZRID;				//생산자 아이디
		    tmpData["MFC_BIZRNO"] 				= 	iniList[0].MFC_BIZRNO;			//생산자 사업자번호
		    tmpData["MFC_BRCH_ID"] 			= 	iniList[0].MFC_BRCH_ID;			//생산자 직매장/공장 아이디
		    tmpData["MFC_BRCH_NO"] 			= 	iniList[0].MFC_BRCH_NO;			//생산자 직매장/공장 번호
        ...
        */
	 	...
	 	data["list_ori"] = JSON.stringify(jsonObject($("#cfm_gridList").val()));
