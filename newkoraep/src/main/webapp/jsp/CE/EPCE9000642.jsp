...
<script type="text/javaScript" language="javascript" defer="defer">
     ...
     $(function() {
    	...
    	serialList 		= jsonObject($("#serialList").val());		
...
//     		kora.common.setEtcCmBx2(brch_nmList, "","", $("#WHSDL_BRCH_NM"), "BRCH_ID_NO", "BRCH_NM", "N" ,'S');		//지점
    		kora.common.setEtcCmBx2(serialList, "","", $("#SERIAL_NO"), "SERIAL_NO", "SERIAL_NO", "N" ,'S');		//지점
...
// 					    input["RTRVL_RTL_FEE"]    		=	input["RTRVL_QTY"] *dps_fee_list[i].RTRVL_FEE;		//회수소매수수료
			...
			input["URM_NM"] = initList[0].URM_NM;
			input["SERIAL_NO"] = initList[0].SERIAL_NO;
			input["URM_RECEIPT_NO"] = initList[0].URM_RECEIPT_NO;
		    input["URM_DOC_NO"] 			= initList[0].URM_DOC_NO;						//회수문서번호
			input["SYS_SE"]						= 'W';	//시스템구분	
			console.log("11"+input + "11")
			return input;   
	};	
	...
	function fn_reg(){
		...
		var data = {"list": ""};
		var row = new Array();
		var url = "/CE/EPCE9000642_09.do"; 
		
		var changedData = gridRoot.getChangedData();
		console.log("111"+changedData);
	...
	//삭제
	function fn_del_chk(){
		confirm("회수 내역이 모두 삭제되었습니다. 계속 진행하시겠습니까? 삭제 처리된 내역은 복원되지 않으며 재등록 하셔야 합니다.","fn_del_exec");
	}
	
	function fn_del_exec(){
		var url ="/CE/EPCE2925801_04.do"; 
		var input ={}
		input["URM_DOC_NO"] = initList[0].URM_DOC_NO;	//회수문서번호
	 	ajaxPost(url, input, function(rtnData){
	...
	function fn_rowToInput (rowIndex){
		...
		fn_dataSet(item);
		console.log(item);
		$("#SERIAL_NO").val(item["SERIAL_NO"]).prop("selected", true); 	//지점
		...
		$("#URM_QTY").val( item["URM_QTY"]);   															//회수량(개)
		$("#RTRVL_RTL_FEE").val(item["RTRVL_RTL_FEE"]);													//소매수수료
...
// 		prpsCdCheck();
	...
	function fn_dataSet(item){
		var input	={};
		var url 	 	= "/CE/EPCE9000631_19.do"; // 수수료조회
		dps_fee_list=[];
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="RECEIPT_NO"	visible="false" />');
			layoutStr.push('			<DataGridColumn dataField="RECEIPT_SN"	visible="false" />');
			layoutStr.push('			<DataGridColumn dataField="URM_DOC_NO"	visible="false" />');
			layoutStr.push('		</groupedColumns>');
...
<body>
    <div class="iframe_inner"  >
		...
		<input type="hidden" id="serialList" value="<c:out value='${serialList}' />" />
		...
		<section class="secwrap">
				 <div class="write_area">
						<div class="write_tbl">
							...
							<tbody>
								 <tr>
									<th class="bd_l" >소매점명</th> <!-- 도매업자업체명 -->		
									<td>
										<div class="row">
											<div class="txtbox" id="URM_NM"></div>
										</div>
									</td>
									<!-- <th class="bd_l">시리얼번호</th> 도매업자 사업자번호
									<td>
										<div class="row">
		...
		<section class="secwrap mt10">
				<div class="srcharea"  id="divInput" > 
						<div class="row">
<!-- 								<div class="col"> -->
<!-- 									<div class="tit" >시리얼번호</div>  지점 -->
<!-- 									<div class="box"> -->
<!-- 										<select id="SERIAL_NO" style="width: 179px"  class="i_notnull"  ></select> -->
<!-- 									</div> -->
<!-- 								</div> -->
								<div class="col">
							    ...
							    <div class="col">
									<div class="tit">용량(신/구병)</div>  <!-- 빈용기명(소매) -->
									<div class="box">
						...
						<div class="row">
								<div class="col">
									<div class="tit" >회수량</div>   <!-- 회수량 -->
									<div class="box">
										<input type="text" id="URM_QTY" style="width: 179px"  format="minus"   class="i_notnull"  maxlength="8"/>
									</div>
								</div>
								<div class="col" style="">
									<div class="tit">소매수수료</div>  <!-- 소매수수료 -->
									<div class="box" style="" >
										<input type="text" id="RTRVL_RTL_FEE" style="width: 179px" format="minus"   class="i_notnull"  />
									</div>
								</div>   
