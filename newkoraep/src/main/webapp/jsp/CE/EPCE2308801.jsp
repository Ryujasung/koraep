	...
	<script type="text/javaScript" language="javascript" defer="defer">
		...
		$(document).ready(function(){
			...
			$.each(billSeCdList, function(i){
				$('#BILL_SE_CD_DIV').append('<label class="chk"><input type="checkbox" id="BILL_SE_CD_SEL" checked name="BILL_SE_CD_SEL" value="'+billSeCdList[i].ETC_CD+'"><span>'+billSeCdList[i].ETC_CD_NM+'</span></label>');
			});
			//kora.common.setEtcCmBx2(billSeCdList, "", "", $("#BILL_SE_CD_SEL"), "ETC_CD", "ETC_CD_NM", "N", "T");
			
			var acpCfmMnulYnList = jsonObject($('#acpCfmMnulYnList').val());
			kora.common.setEtcCmBx2(acpCfmMnulYnList, "", "", $("#ACP_CFM_MNUL_YN_SEL"), "ETC_CD", "ETC_CD_NM", "N", "T");
		...
		function fn_excel(){
			...
			for(i=0; i<columns.length; i++){
				if(columns[i].getDataField() != undefined && columns[i].getDataField() != 'index'){ //순번 제외
					...
					}else if(columns[i].getDataField() == 'ACP_CFM_DT_PAGE'){// html 태크 사용중 컬럼은 대체
						item['dataField'] = 'ACP_CFM_DT';
					}else{
		...
		function fn_page(){
			...
			INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE2308801.do";
		...
		function fn_pop(){
			...
			window.parent.NrvPub.AjaxPopup('/CE/EPCE2308888.do', pagedata);
		}
		...
		function fn_sel(){
			...
			//input['BILL_SE_CD_SEL'] = $("#BILL_SE_CD_SEL option:selected").val();
			...
			console.log(billSeCdSel);
			input['BILL_SE_CD_SEL'] = JSON.stringify(billSeCdSel);
			
			input['ACP_CFM_MNUL_YN_SEL'] = $("#ACP_CFM_MNUL_YN_SEL option:selected").val();
		 ...
		 */
		 function fn_set_grid() {
			 ...
			 layoutStr.push('	<DataGridColumn dataField="VACCT_NO" headerText="'+parent.fn_text('vr_vacct_no')+'" width="150" />');
			 layoutStr.push('	<DataGridColumn dataField="NOTY_AMT" id="sum" headerText="'+parent.fn_text('acp_amt')+'" width="150" formatter="{numfmt}" textAlign="right" />');
			 layoutStr.push('	<DataGridColumn dataField="BILL_SE_NM" headerText="'+parent.fn_text('bill_issu_se')+'" width="150" />');
			 layoutStr.push('	<DataGridColumn dataField="MFC_BIZRNM" headerText="'+parent.fn_text('mfc_bizrnm')+'" width="150" />');
			 layoutStr.push('	<DataGridColumn dataField="BILL_ISSU_DT_PAGE" headerText="'+parent.fn_text('bill_issu_dt2')+'" width="110" itemRenderer="HtmlItem"/>');
			 layoutStr.push('	<DataGridColumn dataField="ACP_CFM_DT_PAGE" headerText="'+parent.fn_text('acp_cfm_dt2')+'" width="110" itemRenderer="HtmlItem"/>');
			 layoutStr.push('	<DataGridColumn dataField="CFM_SE_NM" headerText="'+parent.fn_text('cfm_se')+'" width="150"/>');
			 layoutStr.push('</groupedColumns>');
...
<input type="hidden" id="acpCfmMnulYnList" value="<c:out value='${acpCfmMnulYnList}' />"/>
	...
	<div class="iframe_inner">
		...
		<section class="secwrap">
			<div class="srcharea" id="sel_params">
				...
				<div class="row">
					<div class="col" style="width:405px">
						<div class="tit" id="bill_issu_se_txt"></div>
						<div class="box" id="BILL_SE_CD_DIV">
							<!--
							<select id="BILL_SE_CD_SEL" name="BILL_SE_CD_SEL" style="width: 179px;">
							</select>
					...
					<div class="col">
						<div class="tit" id="cfm_se_txt" style=""></div>
						<div class="box">
							<select id="ACP_CFM_MNUL_YN_SEL" name="ACP_CFM_MNUL_YN_SEL" style="width: 179px;">
							</select>
						</div>
					</div>
					<div class="btn" id="CR">
