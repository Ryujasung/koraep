...
<script type="text/javaScript" language="javascript" defer="defer">
    ...
    $(function() {
		...
		var mfcBrchList  = jsonObject($("#mfcBrchList").val());
		...
		whsdlList_C = jsonObject($("#whsdlBizrList").val());
		...
		kora.common.setEtcCmBx2(statCdList, "","", $("#DLIVY_STAT_CD"), "ETC_CD", "ETC_CD_NM", "N" ,'T');
		kora.common.setEtcCmBx2(mfcBrchList, "","", $("#MFC_BRCH_NM"), "BRCH_ID_NO", "BRCH_NM", "N" ,'T'); //직매장
	...
	function fn_excel(){
		...
		for(i=0; i<columns.length; i++){
			if(columns[i].getDataField() != undefined && columns[i].getDataField() != 'index'){ //순번 제외
				...
				if( columns[i].getDataField()== "CTNR_NM"){
					item['headerText'] = "빈용기명";
					item['dataField'] = "CTNR_NM";
// 					item['dataField'] = "CTNR_CD";
					item['textAlign'] ="";
					item['id'] ="";
				}
	...
	function fn_sel(chartYn,page){
		...
		var url = "/CE/EPCE6104901_19.do";
		var input = {};
		if(page !="P"){
				input['START_DT'] = $("#START_DT").val();
				input['END_DT'] = $("#END_DT").val();
				input['DLIVY_STAT_CD'] = $("#DLIVY_STAT_CD option:selected").val();
...
// 			kora.common.hideLoadingBar(dataGrid, gridRoot);// 그리드 loading bar off
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="DLIVY_REG_DT" headerText="'+parent.fn_text('dlivy_reg_dt')+'" width="100" formatter="{datefmt}"/>');
			...
			layoutStr.push('			<DataGridColumn dataField="BRCH_NM" headerText="'+ parent.fn_text('mfc_brch_nm')+ '" width="150" />');
			layoutStr.push('			<DataGridColumn dataField="CUST_NM"	headerText="'+ parent.fn_text('whsdl')+ '" width="150" />');
			layoutStr.push('			<DataGridColumn dataField="CUST_BIZRNO" headerText="'+ parent.fn_text('whsdl_bizrno')+ '" width="150" formatter="{maskfmt}" />');
			layoutStr.push('			<DataGridColumn dataField="CTNR_NM" headerText="'+ parent.fn_text('ctnr_nm')+ '" width="200" />');
			layoutStr.push('			<DataGridColumn dataField="PRPS_NM" headerText="'+ parent.fn_text('prps_cd')+ '" width="100" />');
			...
			layoutStr.push('			<DataGridColumn dataField="DLIVY_QTY" id="sum1" headerText="'+ parent.fn_text('dlivy_qty')+ '" width="130" textAlign="right" formatter="{numfmt}"  />');
			layoutStr.push('			<DataGridColumn dataField="DLIVY_GTN" id="sum2" headerText="'+ parent.fn_text('gtn')+ '" width="130" textAlign="right" formatter="{numfmt}" />');
			layoutStr.push('			<DataGridColumn dataField="DLIVY_STAT_NM" headerText="'+ parent.fn_text('stat')+'" width="100" id="tmp1"/>');
			layoutStr.push('		</groupedColumns>');
	...
	// 차트 데이터
	var chartData = [{BIZRNM:"", PRPS0:0, PRPS1:0}];
...
<body>
    ...
    <div class="iframe_inner" >
		<input type="hidden" id="mfcBrchList" value="<c:out value='${mfcBrchList}' />" />
		...
		<section class="secwrap"  id="params">
				<div class="srcharea" > 
					<div class="row" >
						<div class="col"  style="width:595px">
						...
						<div class="col">
							<div class="tit" id="stat_txt" style="width: 90px"></div>
							<div class="box">
								<select id="DLIVY_STAT_CD" style="width: 179px" class="i_notnull" ></select>
							</div>
						</div>
					</div> <!-- end of row -->
					...
					<div class="row">
						...
						<div class="col">
							<div class="tit" id="mfc_brch_nm_txt" style="width: 90px"></div>
							<div class="box">
			...
			<div class="boxarea mt15">
				<div id="gridHolder" style="height: 428px; background: #FFF;"></div>
