...
<%@include file="/jsp/include/common_page.jsp" %>
	...
	<script type="text/javaScript" language="javascript" defer="defer">
		...
		function fn_excel(){
			...
			for(i=0; i<columns.length; i++){
				if(columns[i].getDataField() != undefined && columns[i].getDataField() != 'index'){ //순번 제외
					...
					}else if(columns[i].getDataField() == 'EXCA_PROC_STAT_NM_PAGE'){// html 태그 사용중 컬럼은 대체
		...
		function fn_pop(){
			...
			window.parent.NrvPub.AjaxPopup('/CE/EPCE4707288.do', pagedata);
		}
		...
		function fn_upd(){
	        ...
	        if("C" == EXCA_STD_CD_SEL) {
	            alertMsg("교환정산서 발급취소는 교환정산서 상세조회 화면에서 처리 가능합니다.");
	            return false;
	        }

            if("M" == EXCA_STD_CD_SEL) {
                alertMsg("수기정산서 발급취소는 수기정산서 상세조회 화면에서 처리 가능합니다.");
                return false;
            }
			
			//confirm("해당 정산기간에 발급된 전체 정산서가 발급 취소됩니다.\n계속 진행하시겠습니까?", 'fn_upd_exec');
			
			parent_item["EXCA_STD_CD_SEL"] = $("#EXCA_STD_CD_SEL option:selected").val();
			parent_item["EXCA_STD_NM"] = $("#EXCA_STD_CD_SEL option:selected").text();
	 		var pagedata = window.frameElement.name;
	 		window.parent.NrvPub.AjaxPopup('/CE/EPCE47072883.do', pagedata);
	 		
		}
		
		function fn_upd_exec(){
			
			var url = "/CE/EPCE4707201_21.do";
			//var input = {};
		...
		function fn_page(){
			...
			INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE4707201.do"; 
		 ...
		 */
		 function fn_set_grid() {
			 ...
			 layoutStr.push('	<DataGridColumn dataField="EXCA_PROC_STAT_NM_PAGE" headerText="'+parent.fn_text('stat')+'" width="100" itemRenderer="HtmlItem" />');
             layoutStr.push('   <DataGridColumn dataField="STD_YEAR" headerText="'+parent.fn_text('std_year')+'" width="80" />');
             layoutStr.push('   <DataGridColumn dataField="YEAR_CHG_YN" headerText="'+parent.fn_text('year_chg_exca_yn')+'" width="120" />');
			 layoutStr.push('</groupedColumns>');
