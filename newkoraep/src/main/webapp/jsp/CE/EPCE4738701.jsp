...
<script type="text/javaScript" language="javascript" defer="defer">
   ...
   //입고정정  조회
    function fn_sel(){
		 ...
		 input["EXCA_TRGT_SE"] = arr4[1];
		 console.log("356")
		 console.log(input["EXCA_TRGT_SE"]);
	...
	function fn_upd_chk(data) {
	    ...
	    if(data=="1"){
	    	alertMessage = "선택하신 입고정정내역이 모두 정정확인 상태로 변경됩니다.\n\n계속 진행하시겠습니까?";
	    }else if(data=="2"){
	    	alertMessage = "선택하신 입고정정내역이 모두 정정반려 상태로 변경됩니다.\n\n계속 진행하시겠습니까?";
	    }else if(data=="3"){
	...
	function dtl_link(data){
		...
		if(data == '2'){
			url = "/CE/EPCE4738764.do";	//입고정정내역조회
		}
		//파라미터에 조회조건값 저장 
		INQ_PARAMS["PARAMS"] = {};
		INQ_PARAMS["PARAMS"] = input;
		INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
		INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE4738701.do";
	...
	function fn_page(){
			...
			INQ_PARAMS["URL_CALLBACK"] = "/CE/EPCE4738701.do";
			INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
			kora.common.goPage('/CE/EPCE4738742.do', INQ_PARAMS); 
	}
	...
	function fn_forwd_chk() {
        ...
        ajaxPost(url, input, function(rtnData){
            if(rtnData.RSLT_CD == "0000"){
        	   excaData = rtnData;
        	   confirm("선택하신 입고정정내역이 모두 ["+rtnData.EXCA_STD_NM +"] 으로 이월처리 됩니다.\n\n계속 진행하시겠습니까?" ,"fn_forwd");	
            }else{
                alertMsg(rtnData.RSLT_MSG);
            }
        });         
	...
	function fn_page2(data){
		...
		if(data =="AR"){
			url = "/CE/EPCE47387312.do"; //정정일괄등록 페이지 이동
		}else{
			url = "/CE/EPCE47387642.do"; //입고내역선택 페이지 이동
		}
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="WRHS_CFM_DT" headerText="'+ parent.fn_text('wrhs_cfm_dt')+ '"	width="120"  textAlign="center"   itemRenderer="HtmlItem" />');//입고확인일자
			layoutStr.push('			<DataGridColumn dataField="CUST_BIZRNM" headerText="'+ parent.fn_text('whsdl')+ '" width="130"  textAlign="center"  />');//도매업자명
			layoutStr.push('			<DataGridColumn dataField="WHSDL_BIZRNO" headerText="'+parent.fn_text('whsdl')+parent.fn_text('bizrno')+ '"  width="110"  formatter="{maskfmt1}" textAlign="center" />');//도매업자 사업자 번호
			layoutStr.push('			<DataGridColumn dataField="CUST_BRCH_NM" headerText="'+ parent.fn_text('whsdl_brch')+ '" width="130"  textAlign="center"  />');//도매업자지점
			...
			layoutStr.push('				<DataGridColumn dataField="CFM_WHSL_FEE_STAX_TOT" headerText="'+ parent.fn_text('stax')+ '" width="110" formatter="{numfmt}" id="num4" textAlign="right" />');//부가세
			...
			layoutStr.push('			</DataGridColumnGroup>');
			layoutStr.push('			<DataGridColumn dataField="CRCT_RTN_DT" headerText="'+ parent.fn_text('crct_rtn_dt')+ '" width="100" id="tmp1" textAlign="center"  formatter="{datefmt2}"/>');//생산자  직매장			
			layoutStr.push('			<DataGridColumn dataField="CRCT_WRHS_CFM_DT" headerText="'+ parent.fn_text('crct_wrhs_cfm_dt')+ '" width="120" id="tmp2" textAlign="center"  formatter="{datefmt2}"/>');//정정입고확인일자
            ...
            layoutStr.push('                <DataGridFooterColumn summaryOperation="SUM" dataColumn="{num12}" formatter="{numfmt1}" textAlign="right"/>');
            ...
            layoutStr.push('                <DataGridFooterColumn labelJsFunction="totalsum12" formatter="{numfmt1}" dataColumn="{num12}" textAlign="right"/>');
