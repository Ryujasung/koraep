...
<script type="text/javaScript" language="javascript" defer="defer">
	...
	$(document).ready(function(){
		...
		searchList4 = jsonObject($('#searchList4').val());
		...
		if(INQ_PARAMS.PARAMS.STD_YEAR != null || INQ_PARAMS.PARAMS.STD_YEAR != undefined) {
			$('#year').text("기준년도 : " + INQ_PARAMS.PARAMS.STD_YEAR + "년");
		}
		...
		$('#ACP_ACCT_NO').text("(" + searchDtl.ACP_BANK_NM + ") " + searchDtl.ACP_ACCT_NO);
		...
		$('#MNUL_ISSU_RSN').text(searchDtl.MNUL_ISSU_RSN);
		
		if(INQ_PARAMS.PARAMS.EXCA_ISSU_SE_CD == 'G'){
	        ...
	        $('#DRVL_BAL_MDT_AMT').text(kora.common.format_comma(Number(kora.common.null2void(searchDtl.DRVL_BAL_ADJ_AMT,"0"))*-1) + " 원");
	        $('#DRVL_BAL').text(kora.common.format_comma(searchDtl.DRVL_BAL) + " 원");
			
			$('#EXCA_ISSU_SE_CD_G').show();
		}
	        
        if( Number(kora.common.null2void(searchDtl.AGTN_BAL_PAY_AMT,"0")) != 0 || Number(kora.common.null2void(searchDtl.AGTN_INDE_AMT,"0") ) != 0  ) {
            $('#AGTN_TR').show();
        }
		
		if( Number(kora.common.null2void(searchDtl.DRVL_BAL_PAY_AMT,"0")) != 0 || Number(kora.common.null2void(searchDtl.DRVL_BAL_ADJ_AMT,"0") ) != 0  ) {
	        $('#DRCT_RTRVL_TR').show();
	    }
		
		$('#ADD_AMT_SE_NM').text(searchDtl.ADD_AMT_SE_NM);
		$('#ADD_AMT').text(kora.common.format_comma(searchDtl.ADD_AMT) + " 원");
		$('#RISU_RSN').html(kora.common.null2void(searchDtl.RISU_RSN).replaceAll('\\n', '<br>'));
		$('#RISU_DT').text(kora.common.setDelim(searchDtl.RISU_DT, '9999-99-99'));
		
		//가산금 존재시 보여줌
		if(searchDtl.ADD_AMT_SE != undefined){
			$('#risuSection').show();
		}
		
		if(INQ_PARAMS.PARAMS.EXCA_STD_CD != 'M') {
		    $('#btn_upd2').hide();
		    $('#rsnSection').hide();
		}
		...
		if(searchList4.length > 0){ fn_set_grid4(); }else{ $('#gridDiv4').hide(); }
	...
	function fn_upd(){
		...
		if(searchDtl.ADD_AMT_SE == '' || searchDtl.ADD_AMT_SE == undefined ){
     		alertMsg('재고지처리된 정산서가 아닙니다.');
     		return;
     	}
		
     	if(searchDtl.EXCA_PROC_STAT_CD != 'I' || searchDtl.EXCA_SE_CD != 'A' ){
     		alertMsg('발급상태의 납부 정산서만 재고지취소 가능합니다.');
     		return;
     	}
     	
     	confirm("재고지취소 하시겠습니까?", "fn_upd_exec");
	...
	function fn_upd_exec(){
     	...
     	var data = {};
		data['STAC_DOC_NO'] = searchDtl.STAC_DOC_NO;
		data['EXCA_ISSU_SE_CD'] = searchDtl.EXCA_ISSU_SE_CD;
		
		var url  = "/CE/EPCE4707264_21.do";
    ...
    //정산서발급 취소
    function fn_upd2(){
	    if(searchDtl.EXCA_PROC_STAT_CD != 'I'){
            alertMsg('발급상태의 정산서만 발급취소 가능합니다.');
            return;
        }
        
        confirm("정산서발급 취소 하시겠습니까?", "fn_upd_exec2");
    }
    ...
    function fn_upd_exec2(){
        ...
        var url = "/CE/EPCE4707264_19.do";
        //var url = "/CE/EPCE47929643_21.do";
        ajaxPost(url, data, function(rtnData){
            if ("" != rtnData && null != rtnData) {
                if(rtnData.RSLT_CD =="0000"){
	...
	//재고지
     ...
     function fn_pop(){
     	...
     	if(searchDtl.EXCA_PROC_STAT_CD != 'I' || searchDtl.EXCA_SE_CD != 'A' ){
     		alertMsg('발급상태의 납부 정산서만 재고지 가능합니다.');
     		return;
     	}
     	
 		parent_item = searchDtl;
 		var pagedata = window.frameElement.name;
 		window.parent.NrvPub.AjaxPopup('/CE/EPCE47072882.do', pagedata);
 	}
     
    //현재페이지 재조회
   	function fn_reload(){
   		kora.common.goPage("/CE/EPCE4707264.do", INQ_PARAMS, "R");
    }
	 ...
	 //인쇄
   	function fn_pnt(){
		...
		var number = parseInt(day);
   		...
   		$('form[name="prtForm"] input[name="EXCA_ISSU_SE_CD"]').val(INQ_PARAMS.PARAMS.EXCA_ISSU_SE_CD);
        $('form[name="prtForm"] input[name="MFC_BIZRNO"]').val(searchDtl.BIZRNO);
        
        $('form[name="prtForm3"] input[name="STAC_DOC_NO"]').val(INQ_PARAMS.PARAMS.STAC_DOC_NO);
   		$('form[name="prtForm3"] input[name="EXCA_ISSU_SE_CD"]').val(INQ_PARAMS.PARAMS.EXCA_ISSU_SE_CD);
        $('form[name="prtForm3"] input[name="MFC_BIZRNO"]').val(searchDtl.BIZRNO);
   		
		if(number > 20210609) {     // 21.6.10 일자이후 자원순환보증금관리센터
	   		if(searchList.length > 0){ $('form[name="prtForm"] input[name="SEARCH_LIST1"]').val('Y'); }
			if(searchList2.length > 0){ $('form[name="prtForm"] input[name="SEARCH_LIST2"]').val('Y'); }
			if(searchList3.length > 0){ $('form[name="prtForm"] input[name="SEARCH_LIST3"]').val('Y'); }
			else if( Number(kora.common.null2void(searchDtl.DRVL_BAL_PAY_AMT,"0")) != 0 || Number(kora.common.null2void(searchDtl.DRVL_BAL_ADJ_AMT,"0") ) != 0  ) {
				$('form[name="prtForm"] input[name="SEARCH_LIST3"]').val('Y');
			}
			if(searchList4.length > 0){ $('form[name="prtForm"] input[name="SEARCH_LIST4"]').val('Y'); }
			if(searchList5.length > 0){ $('form[name="prtForm"] input[name="SEARCH_LIST5"]').val('Y'); }
			if(searchList6.length > 0){ $('form[name="prtForm"] input[name="SEARCH_LIST6"]').val('Y'); }
	        if(searchList7.length > 0){ $('form[name="prtForm"] input[name="SEARCH_LIST7"]').val('Y'); }
	        if(searchList8.length > 0){ $('form[name="prtForm"] input[name="SEARCH_LIST8"]').val('Y'); }
	        if(searchList9.length > 0){ $('form[name="prtForm"] input[name="SEARCH_LIST9"]').val('Y'); }
			$('form[name="prtForm"] input[name="BILL_DOC_NO"]').val(searchDtl.BILL_DOC_NO);
	   		...
	   		if(searchList.length > 0){ $('form[name="prtForm3"] input[name="SEARCH_LIST1"]').val('Y'); }
			if(searchList2.length > 0){ $('form[name="prtForm3"] input[name="SEARCH_LIST2"]').val('Y'); }
			if(searchList3.length > 0){ $('form[name="prtForm3"] input[name="SEARCH_LIST3"]').val('Y'); }
			else if( Number(kora.common.null2void(searchDtl.DRVL_BAL_PAY_AMT,"0")) != 0 || Number(kora.common.null2void(searchDtl.DRVL_BAL_ADJ_AMT,"0") ) != 0  ) {
				$('form[name="prtForm3"] input[name="SEARCH_LIST3"]').val('Y');
			}
			if(searchList4.length > 0){ $('form[name="prtForm3"] input[name="SEARCH_LIST4"]').val('Y'); }
			if(searchList5.length > 0){ $('form[name="prtForm3"] input[name="SEARCH_LIST5"]').val('Y'); }
			if(searchList6.length > 0){ $('form[name="prtForm3"] input[name="SEARCH_LIST6"]').val('Y'); }
	        if(searchList7.length > 0){ $('form[name="prtForm3"] input[name="SEARCH_LIST7"]').val('Y'); }
	        if(searchList8.length > 0){ $('form[name="prtForm3"] input[name="SEARCH_LIST8"]').val('Y'); }
	        if(searchList9.length > 0){ $('form[name="prtForm3"] input[name="SEARCH_LIST9"]').val('Y'); }
	 ...
	 */
	 function fn_set_grid2() {
         ...
         layoutStr2.push('  <DataGridColumn dataField="CRCT_WRHS_CFM_DT"  headerText="'+parent.fn_text('crct_wrhs_cfm_dt')+'" width="120" formatter="{dateFmt}"/>');
         layoutStr2.push('  <DataGridColumn dataField="CRCT_RTN_DT"  headerText="'+parent.fn_text('crct_rtn_dt')+'" width="100" formatter="{dateFmt}"/>');
		 layoutStr2.push('</groupedColumns>');
	...
	// 그리드 및 메뉴 리스트 세팅
	 ...
	 /**
		...
		var gridApp4, gridRoot4, dataGrid4, layoutStr4, selectorColumn4;
		var layoutStr4 = new Array();
		
		/**
		 * 메뉴관리 그리드 셋팅
		 */
		 function fn_set_grid4() {
			 
			 rMateGridH5.create("grid4", "gridHolder4", jsVars4, "100%", "100%");
			 
			 layoutStr4.push('<rMateGrid>');
			 layoutStr4.push('	<NumberFormatter id="numfmt" useThousandsSeparator="true"/>');
			 layoutStr4.push('<DataGrid headerColors="[#EFF6FC,#EFF6FC]" verticalAlign="middle" id="dg4" headerHeight="35" horizontalScrollPolicy="auto" horizontalGridLines="true" draggableColumns="true" sortableColumns="true" textAlign="center" >');
			 layoutStr4.push('	<groupedColumns>');   
			 layoutStr4.push('	<DataGridColumn dataField="index" headerText="'+parent.fn_text('sn')+'" itemRenderer="IndexNoItem" textAlign="center" width="50" />');
			 layoutStr4.push('	<DataGridColumn dataField="BIZRNM"  headerText="'+parent.fn_text('mfc_bizrnm')+'" width="200" />');
			 layoutStr4.push('	<DataGridColumn dataField="EXCH_DLIVY_QTY" id="sum1" headerText="'+parent.fn_text('exch_dlivy_qty')+'" width="160" formatter="{numfmt}" textAlign="right" />');
			 layoutStr4.push('	<DataGridColumn dataField="EXCH_DLIVY_GTN" id="sum2" headerText="'+parent.fn_text('exch_dlivy_gtn')+'" width="180" formatter="{numfmt}" textAlign="right" />');
			 layoutStr4.push('	<DataGridColumn dataField="EXCH_WRHS_QTY" id="sum3" headerText="'+parent.fn_text('exch_wrhs_qty')+'" width="160" formatter="{numfmt}" textAlign="right" />');
			 layoutStr4.push('	<DataGridColumn dataField="EXCH_WRHS_GTN" id="sum4" headerText="'+parent.fn_text('exch_wrhs_gtn')+'" width="180" formatter="{numfmt}" textAlign="right" />');
			 layoutStr4.push('	<DataGridColumn dataField="SF_AMT" id="sum5" headerText="'+parent.fn_text('sf_amt')+'" width="120" formatter="{numfmt}" textAlign="right" />');
			 layoutStr4.push('	</groupedColumns>');
			 layoutStr4.push('		<footers>');
			 layoutStr4.push('			<DataGridFooter backgroundColor="#6E7376" color="#FFFFFF">');
			 layoutStr4.push('				<DataGridFooterColumn label="'+parent.fn_text('sum')+'" textAlign="center"/>');
			 layoutStr4.push('				<DataGridFooterColumn/>');
			 layoutStr4.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{sum1}" formatter="{numfmt}" textAlign="right"/>');	
			 layoutStr4.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{sum2}" formatter="{numfmt}" textAlign="right"/>');	
			 layoutStr4.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{sum3}" formatter="{numfmt}" textAlign="right"/>');	
			 layoutStr4.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{sum4}" formatter="{numfmt}" textAlign="right"/>');	
			 layoutStr4.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{sum5}" formatter="{numfmt}" textAlign="right"/>');	
			 layoutStr4.push('			</DataGridFooter>');
			 layoutStr4.push('		</footers>');
			 layoutStr4.push('</DataGrid>');
			 layoutStr4.push('</rMateGrid>');
		...
		// 그리드 및 메뉴 리스트 세팅
		 function gridReadyHandler4(id) {
		     ...
		     gridApp4.setLayout(layoutStr4.join("").toString());

		     var layoutCompleteHandler = function(event) {
		    	 dataGrid4 = gridRoot4.getDataGrid();  // 그리드 객체
		    	 gridApp4.setData(searchList4);
		    }
		    var dataCompleteHandler = function(event) {
		    }
		    
		    gridRoot4.addEventListener("dataComplete", dataCompleteHandler);
...
<input type="hidden" id="searchList4" value="<c:out value='${searchList4}' />"/>
...
<input type="hidden" id="searchList9" value="<c:out value='${searchList9}' />"/>
	...
	<div class="iframe_inner">
			...
			<div class="write_area">
				<div class="write_tbl">
					<table>
						...
						<tbody>
							<tr>
								<th ><span class="tit" id="add_amt_se_txt"></span></th>
								<td>
									<div class="row">
										<div class="txtbox" id="ADD_AMT_SE_NM"></div>
									</div>
								</td>
								<th ><span class="tit" id="add_amt_txt"></span></th>
								<td>
									<div class="row">
							...
							<tr>
								<th ><span class="tit" id="risu_dt_txt"></span></th>
								<td>
									<div class="row">
										<div class="txtbox" id="RISU_DT"></div>
									</div>
								</td>
								<th ><span class="tit" id="risu_rsn_txt"></span></th>
								<td>
									<div class="row">
			...
			<div class="write_area" id="EXCA_ISSU_SE_CD_G" style="display:none">
				<div class="write_tbl">
					<table>
						...
						<tr id="AGTN_TR" style="display:none;">
						...
						<tr id="DRCT_RTRVL_TR" style="display:none;">
        ...
        <section id="rsnSection" class="secwrap mt10">
            <div class="write_area">
                <div class="write_tbl">
                    <table>
                        <colgroup>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="row" id="divInput">
                                    <div class="txtbox" id="MNUL_ISSU_RSN" ></div>&nbsp;
                                </div>
                            </td>
                        </tr>
                    </table>
		...
		<section class="secwrap mt10" id="gridDiv4">
			<div class="h4group">
	...
	<form name="prtForm" id="prtForm">
		...
		<input type="hidden" name="SEARCH_LIST1" value="" />
		<input type="hidden" name="SEARCH_LIST2" value="" />
		<input type="hidden" name="SEARCH_LIST3" value="" />
		<input type="hidden" name="SEARCH_LIST4" value="" />
		<input type="hidden" name="SEARCH_LIST5" value="" />
		<input type="hidden" name="SEARCH_LIST6" value="" />
		<input type="hidden" name="SEARCH_LIST7" value="" />
        <input type="hidden" name="SEARCH_LIST8" value="" />
        <input type="hidden" name="SEARCH_LIST9" value="" />
