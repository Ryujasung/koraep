...
<script type="text/javaScript" language="javascript" defer="defer">
	...
	$(function(){
		...
		////////////////////////
	        ...
	        $("#CUST_SEL").change(function(){
	            ...
	            if(CUST_SEL_CK=="Y"){
	                ...
	                //$('#WHSDL_BIZRNM').attr('style', '');
	              }else if(CUST_SEL_CK=="N"){
	                  $("#WHSDL_BIZRNM").find("option:eq(0)").prop("selected", true); //도매업자 선택 초기상태로
	                  
	                  //$("#WHSDL_BIZRNM").val(""); //판매처번호 빈값으로
	                  
	                  //$('#WHSDL_BIZRNM').attr('style', 'display:none');
	              }
	         });
	        /************************************
	         * 시작날짜  클릭시 - 삭제 변경 이벤트
		...
		$('#title_sub').text('<c:out value="반환수집소정보변경" />');
// 		fn_btnSetting();
 		kora.common.setEtcCmBx2(BankCdList, "", "", $("#AcpBankCdList_SEL"), "ETC_CD", "ETC_CD_NM", "N", "S");
 		kora.common.setEtcCmBx2(whsdl_cd_list, "","", $("#WHSDL_BIZRNM"), "CUST_BIZRID_NO", "CUST_BIZRNM", "N" ,'S');		//도매업자 업체명
...
// 		$('#cho5').text(parent.fn_text('cho'));
	...
	/**
	 ...
	 function fnSetDtlData(data){
		 ...
		 $("#RCS_NO").val(kora.common.null2void(data.RCS_NO));
			 $("#RCS_NM").val(kora.common.null2void(data.RCS_NM));
			 $("#RCS_BIZR_CD").val(data.RCS_BIZR_CD).prop("selected",true);
			$("#URM_YN").val(data.URM_YN).prop("selected",true);
			$("#PNO").val(data.PNO);
			$("#ADDR1").val(data.ADDR1);
			$("#ADDR2").val(data.ADDR2);
			$("#AreaCdList_SEL").val(data.AREA_CD).prop("selected",true);
			$("#WHSDL_BIZRNM").val(kora.common.null2void(data.WHSDL_BIZRNM));
			$("#WHSDL_BIZRNO").val(kora.common.null2void(data.WHSDL_BIZRNO));
			$("#AcpBankCdList_SEL").val(data.ACP_BANK_CD).prop("selected",true);
			$("#ACP_ACCT_NO").val(kora.common.null2void(data.ACP_ACCT_NO));
			
			//$("#FILE_NM").val(kora.common.null2void(data.FILE_NM));
			$("#START_DT").val(kora.common.null2void(data.START_DT));
			$("#END_DT").val(kora.common.null2void(data.END_DT));
			$("#MN_TEL").val(kora.common.null2void(data.MN_TEL));
			$("#MN_HTEL").val(kora.common.null2void(data.MN_HTEL));
			
			$("#LOC_GOV").val(kora.common.null2void(data.LOC_GOV));
			$("#LOC_NM").val(kora.common.null2void(data.LOC_NM));
			$("#LOC_TEL").val(kora.common.null2void(data.LOC_TEL));
			$("#LOC_HTEL").val(kora.common.null2void(data.LOC_HTEL));
			$("#LOC_EMAIL").val(kora.common.null2void(data.LOC_EMAIL));
...
// 		if(!confirm("사업자관리 페이지로 이동하시겠습니까?")) return;
		...
		kora.common.goPageB('/CE/EPCE9000216.do', INQ_PARAMS);
	}
		                ...
		                <div class="row" >
		                    <div class="col"  style="width: 100%">
		                        ...
		                        <div class="box">
									...
									<input type="hidden" id="RCS_NO" name="RCS_NO" />
		                        </div>
		                    </div>
		                </div> <!-- end of row -->
		                 ...
		                 <div class="row"   id="ma3">
		                    <div class="col"  style="width: 100%">
		                        ...
		                        <div class="box" id="CUST_SEL" style="width:150px">
                                            <label class="rdo"><input type="radio" id="selectCust1" name="selectCust" value="N" checked="checked"/><span id="">변경안함</span></label>
                                            <label class="rdo"><input type="radio" id="selectCust2" name="selectCust" value="Y"/><span id="">변경</span></label>
                                        </div>
		                        <div class="box">
		                        	<!-- <input type="text" id="WHSDL_BIZRNM" name="WHSDL_BIZRNM" style="width: 330px;" class="i_notnull"  > -->
		                        	<select id="WHSDL_BIZRNM" name="WHSDL_BIZRNM" style="width: 330px;"></select>
		                        	<span class="notice" style="color: RED;">관리업체를 변경시 변경에 체크 후 변경하실 업체를 선택해주십시오 </span>
		                        </div>
		                    </div>
		                </div> <!-- end of row -->
		                
		                <div class="row"   id="ma4" style="display: none;">
		                    <div class="col"  style="width: 100%">
		                        <div class="tit">관리업체사업자번호</div>  
		                        <div class="box">
		                        	<input type="text" id="WHSDL_BIZRNO" name="WHSDL_BIZRNO" style="width: 330px;" class="i_notnull"  >
		                        </div>
		                    </div>
		                </div> <!-- end of row -->
