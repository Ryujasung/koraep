...
<script type="text/javaScript" language="javascript" defer="defer">
    ...
    $(function() {
     ...
     function fn_init(){
    	 	...
    	 	kora.common.setEtcCmBx2(whsdlList, "","", $("#WHSDL_BIZRNM"), "CUST_BIZRID_NO", "CUST_BIZRNM", "N" ,'T');		 //업체명
			...
			if(page !="P"){
				...
				INQ_PARAMS["SEL_PARAMS"] = input;
			 	...
			 	if($("#MFC_BIZRNM").val() !="" ){//생산자
			 		var arr2  =[];
		   			arr2 = $("#MFC_BIZRNM").val().split(";");	
		   			input["MFC_BIZRID"] = arr2[0];
		   			input["MFC_BIZRNO"] = arr2[1];
			 	}
...
// 				hideLoadingBar(); 
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('				<DataGridColumn dataField="DLIVY_TOT_RT_H" headerText="'+ parent.fn_text('fh_rt')+ '" 	width="100" formatter="{percfmt}" id="tmp1" textAlign="right" />');//가정용비율
			...
			layoutStr.push('				<DataGridColumn dataField="DLIVY_CPR_RTRVL_RT_H" headerText="'+ parent.fn_text('fh_rt')+ '" width="100" formatter="{percfmt}" id="tmp3" textAlign="right" />');//가정용비율
			layoutStr.push('			</DataGridColumnGroup>');
			layoutStr.push('			<DataGridColumn dataField="DLIVY_CPR_RTRVL_RT" headerText="'+ parent.fn_text('dlivy_cpr_rtrvl_rt')+ '" width="100" formatter="{percfmt}" id="tmp4" textAlign="right"  />');//출고대비비율
			layoutStr.push('		</groupedColumns>');
