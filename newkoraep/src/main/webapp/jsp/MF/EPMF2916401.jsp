...
<script type="text/javaScript" language="javascript" defer="defer">
   ...
   //입고관리 조회
		...
		var url ='/MF/EPMF2983931_092.do'	//템프에 저장		
	 	ajaxPost(url, input, function(rtnData){
			if(rtnData.RSLT_CD != "0000"){
				...
				INQ_PARAMS["URL_CALLBACK"] = "/MF/EPMF2916401.do";
				kora.common.goPage('/MF/EPMF2916431.do', INQ_PARAMS);
			}
		});    
	}
	...
	function dtl_link(data){
		...
		var url = "/MF/EPMF2916464.do";
		if(data == '2'){
			url = "/MF/EPMF29164642.do";
		}
		//파라미터에 조회조건값 저장 
		INQ_PARAMS["PARAMS"] = {};
		INQ_PARAMS["PARAMS"] = input;
	...
	function link(data){
		...
		var url = "/MF/EPMF29164883.do";		//실태조사요청정보
			if(data == '1'){
				url = "/MF/EPMF2916488.do";		//증빙파일등록
			}else if(data == '2'){
				url = "/MF/EPMF29164882.do";	//증빙파일조회
			}
    ...
    function rsrc_doc_link(se){
        var idx = dataGrid.getSelectedIndices();
        var input = gridRoot.getItemAt(idx);
        //var url = "/MF/EPMF29164312.do";  //실태조사표
        var url = "/MF/EPMF29164313.do";  //실태조사표
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="SYS_SE_NM" 						headerText="'+ parent.fn_text('reg_se')+ '" width="100"  textAlign="center"  id="tmp3" />');														//등록구분
			layoutStr.push('			<DataGridColumn dataField="WHSDL_BIZRNO" 				headerText="'+parent.fn_text('whsdl')+parent.fn_text('bizrno')+ '"  formatter="{maskfmt1}"   width="110"  textAlign="center"  id="tmp4" />');				//도매업자 사업자 번호
			//layoutStr.push('			<DataGridColumn dataField="ADD_FILE" 						headerText="'+ parent.fn_text('prf_file')+ '" width="100"  textAlign="center"  itemRenderer="HtmlItem" id="tmp5" />');														//등록구분
			layoutStr.push('			<DataGridColumn dataField="REQ_RSN" 							headerText="'+parent.fn_text('rh_req_info')+ '"   width="70"  textAlign="center"  itemRenderer="HtmlItem" id="tmp6" />');				//도매업자 사업자 번호
            layoutStr.push('            <DataGridColumn dataField="RSRC_DOC_KND"                         headerText="'+parent.fn_text('rsrc_doc')+ '"   width="60"  textAlign="center"  itemRenderer="HtmlItem" id="tmp7" />');              //실태조사표
			layoutStr.push('		</groupedColumns>');
