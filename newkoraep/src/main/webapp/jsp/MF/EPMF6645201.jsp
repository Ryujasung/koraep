...
<script type="text/javaScript" language="javascript" defer="defer" >
		...
		$(document).ready(function(){
			...
			//다국어 세팅
			$('#drct_rtrvl_dt').text("<c:out value='${drct_rtrvl_dt}' />");//직접회수일자
			$('#reg_dt2').text("<c:out value='${reg_dt2}' />");//등록일자
			...
			fn_bizrTpCd();
		 	...
		 	//상태
	   	 	kora.common.setEtcCmBx2(stat_cdList, "","", $("#DRCT_RTRVL_STAT_CD"), "ETC_CD", "ETC_CD_NM", "N" ,'T');

	   		//등록구분
	   	 	kora.common.setEtcCmBx2(reg_se_sel, "","", $("#SYS_SE"), "ETC_CD", "ETC_CD_NM", "N" ,'T');
		...
		//직접회수정보 변경
	     function fn_upd(){
	    	 ...
	    	 var chkLst = selectorColumn.getSelectedItems();
				...
				if(item.DRCT_RTRVL_STAT_CD != 'RG' ){
					alertMsg("직접회수등록 상태의 정보만 변경 가능합니다. 다시 한 번 확인하시기 바랍니다.");
		    		return;
				}
				
				INQ_PARAMS["PARAMS"] = item;
				INQ_PARAMS["FN_CALLBACK" ] = "fn_sel";
		 		INQ_PARAMS["URL_CALLBACK"] = "/MF/EPMF6645201.do";
	
		 		kora.common.goPage('/MF/EPMF6645242.do', INQ_PARAMS);
	     }
		...
		function fn_excel(){
			...
			for(i=0; i<columns.length; i++){
				if(columns[i].getDataField() != undefined && columns[i].getDataField() != 'index'){ //순번 제외
					...
					if(columns[i].getDataField() == 'DRCT_RTRVL_DT_PAGE'){// html 태크 사용중 컬럼은 대체
						item['dataField'] = 'DRCT_RTRVL_DT';
					}else{
	...
	function fn_sel(){
		...
		input[ "SEL_TERM_SE"]   				=$("#SEL_TERM_SE").val(); //조회기간구분
		input[ "START_DT"]        				=$("#START_DT").val();     //시작날짜
		input[ "END_DT"]           				=$("#END_DT").val();   		//끝날짜
		input[ "DRCT_RTRVL_STAT_CD"] 	=$("#DRCT_RTRVL_STAT_CD").val();  	// 상태 
		input[ "SYS_SE"]           				=$("#SYS_SE").val();   		//등록구분
...
// 			hideLoadingBar();
	...
	function fn_reg(){
		INQ_PARAMS["FN_CALLBACK" ] = "fn_sel";
		INQ_PARAMS["URL_CALLBACK"] = "/MF/EPMF6645201.do";
		kora.common.goPage('/MF/EPMF6645231.do', INQ_PARAMS);
	}
    	...
    	del_row = new Array();
 		...
 		if(selectorColumn.getSelectedIndices() == "") {
 		 ...
 		 for(var i=0; i<selectorColumn.getSelectedIndices().length; i++) {
			...
			if(item["DRCT_RTRVL_STAT_CD"] !="RG"){
				alertMsg("직접회수등록 상태의 정보만 삭제 가능합니다");
				return;
			}
			del_row.push(item);
 		 }
...
/****************************************** 그리드 셋팅 시작***************************************** */
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="DRCT_RTRVL_REG_DT"  headerText="'+ parent.fn_text('reg_dt2')+'" textAlign="center" width="150" formatter="{datefmt}"/>');//등록일자
			layoutStr.push('			<DataGridColumn dataField="DRCT_RTRVL_DT_PAGE" itemRenderer="HtmlItem"  headerText="'+ parent.fn_text('drct_rtrvl_dt')+'" textAlign="center" width="150" />');//직접회수일자
			...
			layoutStr.push('			<DataGridColumn dataField="DRCT_RTRVL_QTY" id="num1"  headerText="'+ parent.fn_text('drct_rtrvl_qty')+ '" textAlign="right" width="100" formatter="{numfmt}"/>');//직접회수량
			layoutStr.push('			<DataGridColumn dataField="DRCT_PAY_GTN" id="num2"  headerText="'+ parent.fn_text('drct_pay_dps')+ '" textAlign="right" width="150" formatter="{numfmt}"/>');//직접지급보증금
			layoutStr.push('			<DataGridColumn dataField="DRCT_RTRVL_STAT_NM"  headerText="'+ parent.fn_text('stat')+ '" textAlign="center" width="120" id="tmp1" />');//상태
	...
	//상세조회 링크
	 function link() {
		...
		INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
		INQ_PARAMS["URL_CALLBACK"] = "/MF/EPMF6645201.do";
		kora.common.goPage('/MF/EPMF6645264.do', INQ_PARAMS);
	}
	...
	<body>
  	...
  	<div class="iframe_inner"  id="testee" >
				...
				<div class="srcharea" id="sel_params">
					...
					<input type="hidden" id="grid_info_list" value="<c:out value='${grid_info}' />" />
						...
						<div class="col" >
							...
							<div class="box">
								<select style="width: 180px;" id="SEL_TERM_SE" >
										<option value="dt" selected="selected">직접회수일자</option>
						...
						<div class="col" >
							...
							<div class="box">
								<select style="width: 180px;" id="DRCT_RTRVL_STAT_CD"></select>
							</div>
						</div>
						
					</div>
					<div class="row">
						<div class="col" style="width:363px">  <!-- 생산자 선택 -->
						...
						<div class="col">
							<div class="tit" id="reg_se"style=""></div>  <!--등록 구분 -->
							<div class="box">
								<select style="width: 180px;" id="SYS_SE" ></select>
							</div>
						</div>
		...
		<div class="boxarea mt10">
			<!-- 그리드 셋팅 -->
			<div id="gridHolder" style="height: 615px; background: #FFF;"></div>
