...
<script type="text/javaScript" language="javascript" defer="defer">
  	 		...
  	 		if($("#EXCA_STD_CD_SE").val() !=""){
	  			...
	  			ajaxPost(url, input, function(rtnData) {
	  				if ("" != rtnData && null != rtnData) {   
	  					 ...
	  					 kora.common.setEtcCmBx2(rtnData.mfc_bizrnmList, "","", $("#MFC_BIZRNM"), "BIZRID_NO", "BIZRNM", "N");			//생산자
	  					 fn_mfc_bizrnm(); //생산자 변경이벤트
	  				}else{
	  					 alertMsg("error");
	  				}
	  			},false);
   ...
   function fn_mfc_bizrnm(){
		var url = "/MF/EPMF4738701_19.do" 
		var input ={};
		console.log("값"+$("#MFC_BIZRNM").val())
   ...
   //입고정정  조회
	...
	function fn_upd_chk(data) {
	    ...
	    if(data=="1"){
	    	alertMsg = "선택하신 입고정정내역이 모두 정정확인 상태로 변경됩니다.\n\n계속 진행하시겠습니까?";
	    }else if(data=="2"){
	    	alertMsg = "선택하신 입고정정내역이 모두 정정반려 상태로 변경됩니다.\n\n계속 진행하시겠습니까?";
	    }else if(data=="3"){
	    	alertMsg = "선택하신 정정확인 내역이 모두 확인취소 됩니다.\n\n계속 진행하시겠습니까?";
	    }
	    btn_upd_val = data//css때문에 value값 넘기지 못해서 전역형으로..값 넘기기
		confirm(alertMsg,"fn_upd");
	}
	...
	function dtl_link(data){
		...
		var url = "/MF/EPMF29164642.do"; 	//입고정보상세조회
		if(data == '2'){
			url = "/MF/EPMF4738764.do";	//입고정정내역조회
		}
		//파라미터에 조회조건값 저장 
		INQ_PARAMS["PARAMS"] = {};
		INQ_PARAMS["PARAMS"] = input;
		INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
		INQ_PARAMS["URL_CALLBACK"] = "/MF/EPMF4738701.do";
	...
	function  fn_data_chk(){
		...
		for(var i=0; i<std_mgnt_list.length;i++){
			 if(std_mgnt_list[i].EXCA_STD_CD == arr4[0]){							//정산기간코드에서
				 ...
				 if(std_mgnt_list[i].CET_FYER_EXCA_YN == 'Y'){
	...
	function fn_page(){
			...
			INQ_PARAMS["URL_CALLBACK"] = "/MF/EPMF4738701.do";
			INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
			kora.common.goPage('/MF/EPMF4738742.do', INQ_PARAMS); 
	}
	...
	function fn_page2(data){
		...
		if(data =="AR"){
			url = "/MF/EPMF47387312.do"; //정정일괄등록 페이지 이동
		}else{
			url = "/MF/EPMF47387642.do"; //입고내역선택 페이지 이동
		}
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="WRHS_CFM_DT" 		headerText="'+ parent.fn_text('wrhs_cfm_dt')+ '"	width="120"  textAlign="center"   itemRenderer="HtmlItem" />');			//정정입고확인일자
			...
			layoutStr.push('			</DataGridColumnGroup>');
			layoutStr.push('			<DataGridColumn dataField="CRCT_RTN_DT" headerText="'+ parent.fn_text('crct_rtn_dt')+ '" width="100"  textAlign="center"  formatter="{datefmt2}" id="tmp1" />');													//생산자  직매장
			layoutStr.push('			<DataGridColumn dataField="CRCT_WRHS_CFM_DT" headerText="'+ parent.fn_text('crct_wrhs_cfm_dt')+ '" width="120"  textAlign="center"  formatter="{datefmt2}" id="tmp2" />');													//생산자  직매장
			layoutStr.push('		</groupedColumns>');
