...
<script type="text/javaScript" language="javascript" defer="defer">
   ...
   //자동계산
    ...
    function  fn_init(){
		...
		$('#rtn_doc_no').text(parent.fn_text('rtn_doc_no')); 					//반환문서번호
		 ...
		 //데이터 넣기
   		 ...
   		 $("#RTN_REG_DT")   	  		.text(kora.common.formatter.datetime(INQ_PARAMS.PARAMS.RTN_REG_DT, "yyyy-mm-dd"));		//반환일자
	...
	//빈용기 구분 선택시
	function fn_prps_cd(){
		var url = "/MF/EPMF2910142_19.do" 
		var input ={};
		if( $("#PRPS_CD").val() !=""){
				input["CUST_BIZRID"] 		= 	gridList2[0].WHSDL_BIZRID		//도매업자아이디                     
				input["CUST_BIZRNO"] 		= 	gridList2[0].WHSDL_BIZRNO		//도매업자사업자번호                   
				input["CUST_BRCH_ID"] 	= 	gridList2[0].WHSDL_BRCH_ID		//도매업자 지점 아이디             
				input["CUST_BRCH_NO"] 	= 	gridList2[0].WHSDL_BRCH_NO	//도매업자 지점 번호                      
				input["MFC_BIZRID"] 		= 	gridList2[0].MFC_BIZRID			//생산자 아이디             
				input["MFC_BIZRNO"] 		= 	gridList2[0].MFC_BIZRNO			//생산자 사업자번호           
				input["MFC_BRCH_ID"] 		= 	gridList2[0].MFC_BRCH_ID			//생산자 직매장/공장 아이디      
				input["MFC_BRCH_NO"] 	= 	gridList2[0].MFC_BRCH_NO		//생산자 직매장/공장 번호               
				input["RTN_DT"] 				=	gridList2[0].RTN_DT					//반화일자
...
//소매수수료 적용여부 ㅇㄷ----------------------------------------------------------------------------------------------------
	...
	insRow = function(gbn) {
		...
		//반환내역에 없는 빈용기 입력시
		for(var k=0;k<gridList2.length;k++){
				if(gridList2[k].CTNR_CD ==ctnrCd){
					flag =true;
				}
		}//end of for
		if(!flag){
		...
		for(var i=0; i<ctnr_nm.length; i++){
			if(ctnr_nm[i].CTNR_CD == ctnrCd) {
			    ...
			    input["AMT_TOT"] 					=	input["CFM_GTN"]+ input["CFM_WHSL_FEE"] + input["CFM_WHSL_FEE_STAX"]+ input["CFM_RTL_FEE"]; //총합계
			    input["WHSDL_BIZRID"] 			= 	gridList2[0].WHSDL_BIZRID		//도매업자아이디
			    input["WHSDL_BIZRNO"] 			= 	gridList2[0].WHSDL_BIZRNO		//도매업자사업자번호
			    input["WHSDL_BRCH_ID"] 			= 	gridList2[0].WHSDL_BRCH_ID		//도매업자 지점 아이디
			    input["WHSDL_BRCH_NO"] 		= 	gridList2[0].WHSDL_BRCH_NO	//도매업자 지점 번호
			    input["MFC_BIZRID"] 				= 	gridList2[0].MFC_BIZRID			//생산자 아이디
			    input["MFC_BIZRNO"] 				= 	gridList2[0].MFC_BIZRNO			//생산자 사업자번호
			    input["MFC_BRCH_ID"] 				= 	gridList2[0].MFC_BRCH_ID			//생산자 직매장/공장 아이디
			    input["MFC_BRCH_NO"] 			= 	gridList2[0].MFC_BRCH_NO		//생산자 직매장/공장 번호
			    input["RTN_DOC_NO"]				= 	gridList2[0].RTN_DOC_NO	   		 // 반환문서번호
			    input["WRHS_DOC_NO"]			= 	gridList2[0].WRHS_DOC_NO	   	 // 입고문서번호
			    input["RSRC_DOC_NO"]				= 	gridList2[0].RSRC_DOC_NO	   	 // 실태조사문서번호
			   	input["RTN_DT"]						=	gridList2[0].RTN_DT					//반화일자
	...
	function fn_reg(){
		...
		var vrsb_qty = 0
		var collection = gridRoot2.getCollection();
		var rtn_stat_cd =INQ_PARAMS.PARAMS.RTN_STAT_CD; 
		...
		//입고문서있는경우
		if(INQ_PARAMS.PARAMS.WRHS_DOC_NO !=null){
			url = "/MF/EPMF2916431_21.do"; 
		//입고문서 없을경우
		}else if(INQ_PARAMS.PARAMS.WRHS_DOC_NO ==null){
			url = "/MF/EPMF2916431_09.do"; 
		}
		
		for(var i=0;i<collection.getLength(); i++){
				var tmpData 	= gridRoot2.getItemAt(i);
				tmpData["RTN_STAT_CD"]		="CC";
				tmpData["SYS_SE"]				='W';		//시스템구분	
				if(INQ_PARAMS.PARAMS.WRHS_DOC_NO !=null){	//입고문서있을경우 최초등록자 아이디
					tmpData["REG_PRSN_ID"]		= cfm_gridList2[0].REG_PRSN_ID
					tmpData["REG_DTTM"]			= cfm_gridList2[0].REG_DTTM
				}
		...
		data["list_ori"] = JSON.stringify(gridList2);
     ...
     //취소
  	function fn_cnl(){
    	 console.log(JSON.stringify(INQ_PARAMS));
    	var	url		="/MF/EPMF2983931_04.do";  
		var	input	={};
		input["RTN_DOC_NO"]	= 	gridList2[0].RTN_DOC_NO	   		 // 반환문서번호 템프증빙사진 삭제
  		ajaxPost(url, input, function(rtnData){},false); 
	...
	function link(){
		...
		window.parent.NrvPub.AjaxPopup('/MF/EPMF29839882.do', pagedata);
		
		
/* 
		var url ='/MF/EPMF2916431_092.do'	//템프에 저장		
		showLoadingBar()
	 	ajaxPost(url, input, function(rtnData){
			if(rtnData.RSLT_CD != "0000"){
				alertMsg(rtnData.RSLT_MSG);
  ...
  //선택한 행 입력창에 값 넣기
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr2.push('			<DataGridColumn dataField="RTN_QTY"		headerText="'+ parent.fn_text('rtn_qty')+'" 	width="70"		textAlign="center"  formatter="{numfmt}"/>');				//반환량
			...
			layoutStr2.push('				<DataGridColumn dataField="CFM_WHSL_FEE_STAX"    headerText="'+ parent.fn_text('stax')+ '"	width="100" 	formatter="{numfmt}" 	textAlign="right"  id="num6"/>');//도매부가세
			layoutStr2.push('			</DataGridColumnGroup>');
			...
			layoutStr2.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{num6}" formatter="{numfmt1}" textAlign="right"/>');	//도매부가세
...
<body>
    <div class="iframe_inner" id="testee" >
			...
			<input type="hidden" id="gridList2" value="<c:out value='${gridList}' />" />
		...
		<section class="secwrap" >
				<div class="h4group" >
					<h5 class="tit"  style="font-size: 16px; margin-top: 10px" id="wrhs_cfm_data"><h5>
				</div>
				<div class="srcharea"  id="divInput" > 
					<div class="row">
						<div class="col">
							...
							<div class="box">
								<select id="CTNR_SE" style="width: 41%" class="i_notnull" ></select> <!-- 구병/신병 -->
								<select id="PRPS_CD" style="width: 57%" class="i_notnull" ></select><!-- 유흥 / 가정 / 직접 -->
							</div>
						</div>
						<div class="col" >
						...
						<div class="col">
							...
							<div class="box">
								  <input type="text"  id="RTN_QTY" style=""   format="number" maxlength="8"  disabled="disabled"/>
							</div>
						</div>
					</div> <!-- end of row -->
					<div class="row">
						...
						<div class="col" style="width: 65%">
							...
							<div class="box"  style="width:84%">
									<select id="RMK_SELECT" style="width: 29%"  disabled="disabled"></select>
									<input type="text"  id="RMK" style="width: 69%; max-width: 610px" maxByteLength="30"  disabled="disabled" />
							</div> 
						</div>
					</div> <!-- end of row -->
					<div class="row">
