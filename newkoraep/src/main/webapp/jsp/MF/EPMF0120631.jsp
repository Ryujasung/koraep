...
<script src="/js/kora/paging_common.js"></script>
	...
	<script type="text/javaScript" language="javascript" defer="defer">
		...
		function fn_reg(){
			...
			window.parent.NrvPub.AjaxPopup('/MF/EPMF0120675.do', pagedata);
			
		}
		
		function fn_reg_exec(){

			var url  = "/MF/EPMF0120631_31.do";
			...
			for(var i=0; i<selectorColumn.getSelectedIndices().length; i++) {
				...
				item["CUST_BRCH_ID"] = item["BRCH_ID"];
				item["CUST_BRCH_NO"] = item["BRCH_NO"];
				item["CUST_BIZRID"]  = item["BIZRID"];
				item["CUST_BIZRNO"]  = item["BIZRNO"];
				item["BIZR_TP_CD_MFC"] = mfc_bizrnm_sel[0]["BIZR_TP_CD"];  
			...
			ajaxPost(url, data, function(rtnData){
				if ("" != rtnData && null != rtnData) {
					...
					alertMsg("거래처 등록 및 기준취급수수료 정보 생성이 완료되었습니다.", 'fn_sel');
				} else {
					alertMsg("error");
				}
			});
		...
		function fn_sel(){
			...
			if(input["CUST_BIZRNO_SEL"].length == 0) {
                alertMsg("거래처 사업자번호를 입력하시기 바랍니다.");
                return;
			}
			else if(input["CUST_BIZRNO_SEL"].length != 10){
                alertMsg("거래처 사업자번호 10자리를 입력하시기 바랍니다.");
                return;
            }
	...
	<div class="iframe_inner">
		...
		<section class="secwrap" >
			...
			<div class="boxarea mt10">
				<div id="gridHolder" style="height:104px;"></div>
