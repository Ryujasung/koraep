...
<script type="text/javaScript" language="javascript" defer="defer">
   ...
   //수기입고정정  조회
	...
	function dtl_link(data){
		...
		if(data == '2'){
			url = "/MF/EPMF4705664.do";	 //수기입고정정내역조회
		}
		//파라미터에 조회조건값 저장 
		INQ_PARAMS["PARAMS"] = {};
		INQ_PARAMS["PARAMS"] = input;
		INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
		INQ_PARAMS["URL_CALLBACK"] = "/MF/EPMF4705601.do";
	...
	function fn_page(){
			...
			if(input["MNUL_EXCA_SE"] == 'M'){ //수기등록
				url = '/MF/EPMF47056422.do';
			}else{
				url = '/MF/EPMF4705642.do';
			}
	...
	function fn_page2(data){
		...
		//진행중인 정산기간 여부 체크
		ajaxPost("/MF/EPMF4705601_195.do", {}, function(rtnData) {
   				if ("" != rtnData && null != rtnData) {   
		...
		if(data =="AR"){
			url = "/MF/EPMF47056312.do"; //수기정산등록 페이지 이동
		}else{
			url = "/MF/EPMF47056642.do"; //정정건재등록 페이지 이동
		}
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="MNUL_EXCA_SE_NM"   			headerText="'+ parent.fn_text('se')+ '" 		width="80"  textAlign="center"  />');												//생산자  직매장
			...
			layoutStr.push('				<DataGridFooterColumn/>');
			layoutStr.push('				<DataGridFooterColumn/>');
			layoutStr.push('				<DataGridFooterColumn labelJsFunction="totalsum1"  formatter="{numfmt}" textAlign="right"/>');	
			layoutStr.push('				<DataGridFooterColumn labelJsFunction="totalsum2"  formatter="{numfmt}" textAlign="right"/>');	
			layoutStr.push('				<DataGridFooterColumn labelJsFunction="totalsum3"  formatter="{numfmt1}" textAlign="right"/>');	
			layoutStr.push('				<DataGridFooterColumn labelJsFunction="totalsum4"  formatter="{numfmt1}" textAlign="right"/>');	
			layoutStr.push('				<DataGridFooterColumn labelJsFunction="totalsum5"  formatter="{numfmt1}" textAlign="right"/>');	
			layoutStr.push('				<DataGridFooterColumn labelJsFunction="totalsum6"  formatter="{numfmt}" textAlign="right"/>');	
			layoutStr.push('				<DataGridFooterColumn labelJsFunction="totalsum7"  formatter="{numfmt}" textAlign="right"/>');	
			layoutStr.push('				<DataGridFooterColumn labelJsFunction="totalsum8"  formatter="{numfmt}" textAlign="right"/>');	
			layoutStr.push('				<DataGridFooterColumn labelJsFunction="totalsum9"  formatter="{numfmt}" textAlign="right"/>');	
			layoutStr.push('				<DataGridFooterColumn labelJsFunction="totalsum10"  formatter="{numfmt1}" textAlign="right"/>');	
			layoutStr.push('				<DataGridFooterColumn/>');
			layoutStr.push('				<DataGridFooterColumn/>');
