			...
			if(rtnData != null && rtnData != ""){
				...
				if(chartYn == "Y"){ //조회버튼 클릭 시 챠트조회
					...
					for(var i in rtnData.searchList){
						dlivyQty += Number(rtnData.searchList[i].D_TOTAL);
						cfmQty += Number(rtnData.searchList[i].R_TOTAL);
					}
...
// 			kora.common.hideLoadingBar(dataGrid, gridRoot);// 그리드 loading bar off
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('				<DataGridColumn dataField="D_HOME" headerText="'+parent.fn_text('fh')+'" id="dg1col0" width="120" formatter="{numfmt}" textAlign="right" />');
			layoutStr.push('				<DataGridColumn dataField="D_SHOP" headerText="'+parent.fn_text('fb')+'" id="dg1col1" width="120" formatter="{numfmt}" textAlign="right" />');
			layoutStr.push('				<DataGridColumn dataField="D_TOTAL" headerText="'+parent.fn_text('dlivy_sum')+'" id="dg1col2" width="120" formatter="{numfmt}" textAlign="right" />');
			layoutStr.push('			</DataGridColumnGroup>');
			layoutStr.push('			<DataGridColumnGroup headerText="'+parent.fn_text('rtrvl_b')+'">');
			layoutStr.push('				<DataGridColumn dataField="R_HOME" headerText="'+parent.fn_text('fh')+'" id="dg1col3" width="120" formatter="{numfmt}" textAlign="right" />');
			layoutStr.push('				<DataGridColumn dataField="R_DRCT" headerText="'+parent.fn_text('drct_rtn')+'" id="dg1col4" width="120" formatter="{numfmt}" textAlign="right" />');
			layoutStr.push('				<DataGridColumn dataField="R_SHOP" headerText="'+parent.fn_text('fb')+'" id="dg1col5" width="120" formatter="{numfmt}" textAlign="right" />');
			layoutStr.push('				<DataGridColumn dataField="R_TOTAL" headerText="'+parent.fn_text('rtrvl_sum')+'" id="dg1col6" width="120" formatter="{numfmt}" textAlign="right" />');
			layoutStr.push('			</DataGridColumnGroup>');
			layoutStr.push('			<DataGridColumn dataField="RATE" headerText="'+parent.fn_text('rtrvl_rt')+'&lt;br&gt;(B/A)" width="120" formatter="{percfmt}" textAlign="right"/>');
			layoutStr.push('		</groupedColumns>');
	...
	layoutStrChart.push('             <CategoryAxis categoryField="MFC_BIZRNM"/>');
	layoutStrChart.push('       </horizontalAxis>');
	layoutStrChart.push('        <verticalAxis>');
	layoutStrChart.push('           <LinearAxis/>');
	layoutStrChart.push('       </verticalAxis>');
	layoutStrChart.push('          <series>');
	layoutStrChart.push('             <Column3DSeries labelPosition="outside" yField="D_TOTAL" displayName="출고량" outsideLabelYOffset="-10" showTotalLabel="true" showValueLabels="[]" halfWidthOffset="0" itemRenderer="CylinderItemRenderer">');
	layoutStrChart.push('                  <showDataEffect>');
	layoutStrChart.push('                     <SeriesInterpolate/>');
	layoutStrChart.push('                </showDataEffect>');
	layoutStrChart.push('            </Column3DSeries>');
	layoutStrChart.push('            <Column3DSeries labelPosition="outside" yField="R_TOTAL" displayName="회수량" outsideLabelYOffset="-10" showTotalLabel="true" showValueLabels="[]" halfWidthOffset="0" itemRenderer="CylinderItemRenderer">');
	layoutStrChart.push('                <showDataEffect>');
	layoutStrChart.push('                     <SeriesInterpolate/>');
	layoutStrChart.push('                </showDataEffect>');
	layoutStrChart.push('            </Column3DSeries>');
	...
	// 차트 데이터
	var chartData = [{MFC_BIZRNM:"", D_TOTAL:0, R_TOTAL:0}];
