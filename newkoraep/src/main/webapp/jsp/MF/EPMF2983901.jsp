...
<script type="text/javaScript" language="javascript" defer="defer">
    ...
    $(function() {
    	...
    	/*상세조회 바로이동*/
		var RTN_DOC_NO = "<c:out value='${RTN_DOC_NO}' />";
		var WRHS_DOC_NO = "<c:out value='${WRHS_DOC_NO}' />";
		if( RTN_DOC_NO != ''){
			//파라미터에 조회조건값 저장 
    		var input = {};
			input.RTN_DOC_NO = RTN_DOC_NO;
			input.WRHS_DOC_NO = WRHS_DOC_NO;
    		INQ_PARAMS["PARAMS"] = input;
    		INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
    		INQ_PARAMS["URL_CALLBACK"] = "/MF/EPMF2983901.do";
    		kora.common.goPage('/MF/EPMF2983964.do', INQ_PARAMS);
		}
		/*상세조회 바로이동*/
   ...
   function fn_reg(){
		...
		var url = "/MF/EPMF2983988.do";		//실태조사요청 (생산자);
		...
		if(selectorColumn.getSelectedIndices() == "") {
			...
			return;
		}else if(parent_item["RTN_STAT_CD"] !="RG" ){
			alertMsg("반환등록 상태인경우에만 조사확인요청이 가능합니다.");
			return;
		}
	...
	function fn_page() {
		...
		var url ='/MF/EPMF2983931_092.do'	//템프에 저장		
	 	ajaxPost(url, input, function(rtnData){
			if(rtnData.RSLT_CD != "0000"){
				alertMsg(rtnData.RSLT_MSG);
				return;
				...
				INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
				INQ_PARAMS["URL_CALLBACK"] = "/MF/EPMF2983901.do";
				kora.common.goPage('/MF/EPMF2983931.do', INQ_PARAMS);
			}
		},false);    
	...
	function link2(){
		...
		INQ_PARAMS["FN_CALLBACK"] = "fn_sel";
		INQ_PARAMS["URL_CALLBACK"] = "/MF/EPMF2983901.do";
		kora.common.goPage('/MF/EPMF2910164.do', INQ_PARAMS);
	}
	...
	function fn_excel(){
		...
		for(i=0; i<columns.length; i++){
			if(columns[i].getDataField() != undefined && columns[i].getDataField() != 'index'){ //순번 제외
				...
				}else if(columns[i].getDataField() == 'RTN_DOC_NO_PAGE'){// html 태크 사용중 컬럼은 대체 
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="RTN_DOC_NO_PAGE" headerText="'+ parent.fn_text('rtn_doc_no')+ '"  textAlign="center" width="120"   itemRenderer="HtmlItem"/>');//반환문서번호
			...
			layoutStr.push('			<DataGridColumn dataField="CAR_NO" headerText="'+ parent.fn_text('car_no')+ '" width="100"  textAlign="right"  id="tmp3" />');//차량번호
			layoutStr.push('			<DataGridColumn dataField="SYS_SE_NM" headerText="'+ parent.fn_text('reg_se')+ '" width="100"  textAlign="center"  id="tmp4" />');//등록구분
			layoutStr.push('			<DataGridColumn dataField="WHSDL_BIZRNO" headerText="'+parent.fn_text('whsdl_bizrno')+ '" formatter="{maskfmt1}" width="120" textAlign="center"  id="tmp5" />');//도매업자 사업자 번호
