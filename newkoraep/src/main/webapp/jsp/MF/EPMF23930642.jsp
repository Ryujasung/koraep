...
<script type="text/javaScript" language="javascript" defer="defer">
    ...
    //인쇄
  	...
  	//용기별합계
   	function fn_pnt2(){
		...
		if(number > 20210610) {     // 21.6.10 일자이후 자원순환보증금관리센터
	...
	function fn_cnl(){
		...
		if(searchDtl.ADD_ISSU_YN == 'Y'){
    		alertMsg("보증금고지서 취소 요청 후 취급수수료 고지서를 취소 요청하시기 바랍니다.\n(순서:보증금 고지서 취소요청 및 완료 -> 취급수수료 고지서 취소요청)");
 			return false;
 		}
	...
	function fn_page2(){
		...
		INQ_PARAMS["URL_CALLBACK"] = "/MF/EPMF23930642.do";
		kora.common.goPage('/MF/EPMF29164642.do', INQ_PARAMS);
	}
	 ...
	 */
	 function fn_set_grid() {
		 ...
		 layoutStr.push('	<DataGridColumn dataField="WRHS_CFM_DT_PAGE"  headerText="'+parent.fn_text('wrhs_cfm_dt')+'" width="110" itemRenderer="HtmlItem" />');
		 layoutStr.push('	<DataGridColumn dataField="WRHS_QTY" id="sum1" headerText="'+parent.fn_text('wrhs_qty')+'" width="120" formatter="{numfmt}" textAlign="right"/>');
		 layoutStr.push('	<DataGridColumn dataField="WRHS_FEE" id="sum2" headerText="'+parent.fn_text('fee')+'" width="120" formatter="{numfmt}" textAlign="right"/>');
		 layoutStr.push('	<DataGridColumn dataField="WRHS_FEE_STAX" id="sum3" headerText="'+parent.fn_text('stax')+'" width="120" formatter="{numfmt}" textAlign="right"/>');
		 layoutStr.push('	<DataGridColumn dataField="WRHS_GTN" id="sum4" headerText="'+parent.fn_text('refn_gtn')+'" width="120" formatter="{numfmt}" textAlign="right"/>');
		 layoutStr.push('</groupedColumns>');
