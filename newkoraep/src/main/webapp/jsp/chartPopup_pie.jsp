...
function chartReadyHandler2(id){
	...
	document.getElementById(id).setData(opener.chartData2);
}
...
var layoutStr = 
				'<rMateChart backgroundColor="#FFFFFF"  borderStyle="none">'
					+'<Options>'
						...
						+'<SubCaption text="'+opener.textData['stdYear']+'"/>'
					...
					/* Pie3DChart는 Series를 두개이상 넣으면 stacked으로 됩니다 */
						+'<series>'
...
var layoutStr2 = 
					'<rMateChart backgroundColor="#FFFFFF"  borderStyle="none">'
						+'<Options>'
							+'<Caption text="'+opener.textData['title2']+'"/>'
							+'<SubCaption text="'+opener.textData['stdYear']+'"/>'
							+'<Legend useVisibleCheck="true"/>'
						+'</Options>'
						+'<NumberFormatter id="numFmt"/>'
...
<body>
	...
	<div>
		<button type="button" class="btn36 c2" style="width: 100px;position:absolute;z-index:9;margin:10px;top:320px" onclick="saveAsImage('chart1')" >이미지저장</button>
	</div>
	<div>
		<button type="button" class="btn36 c2" style="width: 100px;position:absolute;z-index:9;margin:10px;top:690px" onclick="saveAsImage('chart2')" >이미지저장</button>
	</div>
	...
	<div class="wrapper" style="margin-left:60px">
		...
		<!-- 
		<div class="header">
			<div class="headerTitle" id="title"></div>
		</div>
		  -->
		...
		<div id="content" style="height:360px;margin-top:10px">
			<!-- 차트가 삽입될 DIV -->
			<div id="chartHolder2" class="chartHolder"></div>
		</div>
