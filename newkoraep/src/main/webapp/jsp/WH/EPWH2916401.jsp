...
<script type="text/javaScript" language="javascript" defer="defer">
   ...
   //입고관리 조회
	...
	function link(data){
		...
		console.log(parent_item.RSRC_DOC_NO);
		var pagedata = window.frameElement.name;
		var url = "/WH/EPWH29164883.do";		//실태조사요청정보
			if(data == '1'){
				url = "/WH/EPWH2916488.do";		//증빙파일등록
			}else if(data == '2'){
				url = "/WH/EPWH29164882.do";	//증빙파일조회
			}
    ...
    function rsrc_doc_link(se){
        var idx = dataGrid.getSelectedIndices();
        var input = gridRoot.getItemAt(idx);
        var url = "/WH/EPWH29164313.do"; //실태조사표
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="WHSDL_BIZRNO" headerText="'+parent.fn_text('whsdl')+parent.fn_text('bizrno')+ '" id="tmp4" formatter="{maskfmt1}"  width="110"  textAlign="center" />');//도매업자 사업자 번호
			//layoutStr.push('			<DataGridColumn dataField="ADD_FILE" headerText="'+ parent.fn_text('prf_file')+ '" width="100"  textAlign="center"  itemRenderer="HtmlItem"/>');//등록구분
			layoutStr.push('			<DataGridColumn dataField="REQ_RSN" headerText="'+parent.fn_text('rh_req_info')+ '"   width="70"  textAlign="center" id="tmp5" itemRenderer="HtmlItem"/>');//도매업자 사업자 번호
		    layoutStr.push('            <DataGridColumn dataField="RSRC_DOC_KND" headerText="'+parent.fn_text('rsrc_doc')+ '"   width="60"  textAlign="center" id="tmp6" itemRenderer="HtmlItem"/>');//도매업자 사업자 번호
			layoutStr.push('		</groupedColumns>');
