...
<%@include file="/jsp/include/common_page.jsp" %>
	...
	<script type="text/javaScript" language="javascript" defer="defer">
		...
		$(document).ready(function(){
			...
			kora.common.setEtcCmBx2(bizrTpList, "", "R1", $("#BIZR_TP_CD"), "ETC_CD", "ETC_CD_NM", "N", "S");
	    	...
	    	var err_msg2 = "";
	    	 ...
	    	 for(var i=0; i<rtnData.length ;i++) {
	     	 ...
	     	 for(var i=0; i<rtnData.length ;i++) {
	     		...
	     		//input["WHSDL_BRCH_NO"] = $('#WHSDL_BRCH option:selected').val().split(";")[1];  // rtnData[i].지점번호;   // 새로운 값으로
	     		//input["BIZR_TP_NM"]	   = $('#BIZR_TP_CD option:selected').text(); // rtnData[i].거래처구분; // 새로운 값으로
	     		//input["BIZR_TP_CD"]    = $('#BIZR_TP_CD option:selected').val();
	     		input["BIZRNM"]		   = rtnData[i].거래처명;
	     		input["BIZRNO"]		   = rtnData[i].거래처사업자번호;
	     		
	     		//console.log("ss: " + JSON.stringify(input));
	     		
	     		//사업자번호 체크
	     		if(!kora.common.gfn_bizNoCheck(input["BIZRNO"])){
	     			err_cnt++
	     			//console.log("dd : " + err_cnt);
	     			//console.log("aa : " + kora.common.gfn_bizNoCheck(input["WHSDL_BIZRNO"]));
	     			//console.log("bb : " + kora.common.gfn_bizNoCheck(input["BIZRNO"]));
					continue;
				}
	     		
	     		var url = "/WH/EPWH0121831_192.do";
	     		ajaxPost(url, input, function(rtnData) {
    				if ("" != rtnData && null != rtnData) {   
	    					if(rtnData.selList != undefined && rtnData.selList != null && rtnData.selList.length !=0){	// 쿼리상 데이터가 있을경우에만
	    						
	    						if(rtnData.selList[0].DATA_YN == "Y"){//데이터가 존재
	    							flag =true;
	    							dup_cnt++;
	    						}else{
								    ...
								    for(var i=0; i<collection.getLength(); i++) {
								    	...
								    	if( gridData.WHSDL_BIZRNO == rtnData.selList[0].WHSDL_BIZRNO
									    	 && gridData.WHSDL_BRCH_NO == rtnData.selList[0].WHSDL_BRCH_NO
									    	 && gridData.BIZRNO == rtnData.selList[0].BIZRNO
								    	 ){
								    		flag =true;
								    		dup_cnt++;
										}
		...
		function fn_change(){
		    ...
		    //input["BIZRID_NO"] =$("#WHSDL_BIZR").val();
       	    ...
       	    ajaxPost(url, input, function(rtnData) {
   				if (null != rtnData) {   
   					kora.common.setEtcCmBx2(rtnData, "",brchIdNo, $("#WHSDL_BRCH"), "BRCH_ID_NO", "BRCH_NM", "N" ,'S');
   				} else {
   					alertMsg("error");
   				}
    		}, false);
			...
			<div class="srcharea" id="params">
				<div class="row" style="display:none">
					...
					<div class="col">
						...
						<div class="box">
							<select id="WHSDL_BRCH" name="WHSDL_BRCH" style="width: 179px" ></select>
						</div>
					</div>
					<div class="col">
