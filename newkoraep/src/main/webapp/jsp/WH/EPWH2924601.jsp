	...
	function fn_whsdl_bizrnm(){
		...
		if($("#WHSDL_BIZRNM").val() !=""){
			...
			//	$("#WHSDL_BIZRNM").select2("val","");
				ajaxPost(url, input, function(rtnData) {
						if ("" != rtnData && null != rtnData) {   
							kora.common.setEtcCmBx2(rtnData.brch_cd_List, "","", $("#WHSDL_BRCHNM"), "BRCH_ID_NO", "BRCH_NM", "N" ,'');				//지점
						}else{
								 alertMsg("error");
						}
				},false);
		}else{
			kora.common.setEtcCmBx2([], "","", $("#WHSDL_BRCHNM"), "BRCH_ID_NO", "BRCH_NM", "N" ,'');				//지점
		}
   }
...
//      		hideLoadingBar();// 그리드 loading bar on
	...
	function link(){
		...
		INQ_PARAMS["URL_CALLBACK"] = "/WH/EPWH2924601.do";
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="RTRVL_STAT_NM" headerText="'+ parent.fn_text('stat')+ '" width="100" textAlign="center" />');//상태
			layoutStr.push('			<DataGridColumn dataField="PRPS_NM" headerText="'+ parent.fn_text('prps_cd')+ '" width="80" textAlign="center" />');//지역
