...
<%@include file="/jsp/include/common_page.jsp" %>
	...
	<script type="text/javaScript" language="javascript" defer="defer">
        ...
        function fn_pnt(){
            ...
            var date2 = new Date($("#END_DT_SEL").val());
            if(date1 >= date2){
            	 $("#CRF_NAME").val("EPGMPLDPt_kora.crf");
            }else{
            	 $("#CRF_NAME").val("EPGMPLDPt_cosmo.crf");
            }
            ...
            if("" != $("#MFC_BIZRNM_SEL option:selected").val()) {
        	    $('form[name="prtForm"] input[name="RD_MFC_BIZRID"  ]').val($("#MFC_BIZRNM_SEL option:selected").val().split(";")[0]);
        	    $('form[name="prtForm"] input[name="RD_MFC_BIZRNO"  ]').val($("#MFC_BIZRNM_SEL option:selected").val().split(";")[1]);
            }
            else {
        	    $('form[name="prtForm"] input[name="RD_MFC_BIZRID"  ]').val("");
        	    $('form[name="prtForm"] input[name="RD_MFC_BIZRNO"  ]').val("");
            }
		...
		function fn_page2(){
			...
			for(var i=0; i<selectorColumn.getSelectedIndices().length; i++) {
				...
				if(item.PAY_STAT_CD != 'L'){
					alertMsg('지급예정 항목만 연계 자료 생성이 가능합니다.');
					return;
				}
			}
...
<body>
    ...
    <form name="prtForm" id="prtForm">
        ...
        <input type="hidden" id="RD_MFC_BIZRID" name="RD_MFC_BIZRID" value="" />
        <input type="hidden" id="RD_MFC_BIZRNO" name="RD_MFC_BIZRNO" value="" />
