...
<script src="/js/kora/paging_common.js"></script>
	...
	<script type="text/javaScript" language="javascript" defer="defer">
		...
		$(document).ready(function(){
			...
			kora.common.setEtcCmBx2(bizrList, "", "", $("#WHSDL_BIZR_SEL"), "BIZRID_NO", "BIZRNM", "N");
			...
			kora.common.setEtcCmBx2(bizrTpList, "", "", $("#BIZR_TP_CD_SEL"), "ETC_CD", "ETC_CD_NM", "N");
		...
		//수수료증빙자료 삭제 confirm
	   	function fn_del_chk(){
			...
			if(parent_item.list.FILE_NM_ORI == null){
				alertMsg("삭제할 정보가 없습니다.");
				return;
			};
	  ...
	  //취급정보 삭제
	   	function fn_del(){
	        ...
	        input["RTL_CUST_BIZRID"] = data["RTL_CUST_BIZRID"];       
	        input["RTL_CUST_BIZRNO"]   = data["RTL_CUST_BIZRNO"];
		...
		function fn_reg(){
			...
			INQ_PARAMS["URL_CALLBACK"] = "/WH/EPWH0121801.do";
			kora.common.goPage('/WH/EPWH0121831.do', INQ_PARAMS);
		}
		/**
		 * 등록팝업 이동
		 */
		function fn_upload(){
			var selector = gridRoot.getObjectById("selector");
			var chkLst = selector.getSelectedItems();
			console.log(chkLst.FILE_NM_ORI);
			if(chkLst.length < 1){
				alertMsg("선택된 내역이 없습니다.");
				return;
			}
			if(chkLst.length > 1){
				alertMsg("하나만 선택하셔야 합니다.");
				alertMsg("다시 등록하실려면 삭제 후 등록해주세요.");
				return;
			}
...
//     	 	console.log('389'+input);
// 			parent_item =input;
    		var url = "/CE/EPCE01218881.do";	
		 ...
		 */
		 function fn_set_grid() {
			 ...
			 layoutStr.push('	<DataGridColumn dataField="WHSDL_BRCH_NM"  headerText="'+parent.fn_text('brch_nm')+'" width="15%"  visible="false"/>');
			 layoutStr.push('	<DataGridColumn dataField="AREA_NM"  headerText="'+parent.fn_text('area')+'" width="7%" visible="false"/>');
			 layoutStr.push('	<DataGridColumn dataField="BIZR_TP_CD_NM"  headerText="'+parent.fn_text('bizr_tp_cd')+'" width="10%"  visible="false"/>');
			 ...
			 layoutStr.push('	<DataGridColumn dataField="UPLOAD_YN"  headerText="'+parent.fn_text('upload_yn')+'" width="5%"/>');
			 layoutStr.push('	<DataGridColumn dataField="FILE_NM"	 	headerText="'+ parent.fn_text('prf_file') + '" width="20%"	itemRenderer="HtmlItem" textAlign="center"/>');	//증빙자료
			 layoutStr.push('</columns>');
		...
		function fn_pop(){
			...
			var pagedata = window.frameElement.name;
			window.parent.NrvPub.AjaxPopup('/WH/EPWH0121888.do', pagedata);
		}
