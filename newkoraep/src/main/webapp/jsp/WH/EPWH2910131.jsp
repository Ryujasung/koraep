...
<script type="text/javaScript" language="javascript" defer="defer">
     ...
     $(function() {
    	...
    	kora.common.setEtcCmBx2(whsl_se_cd, "","", $("#WHSL_SE_CD"), "ETC_CD", "ETC_CD_NM", "N");//도매업자구분
    	//$("#ENP_NM").select2();
    	fn_whsl_se_cd();
    	//fn_init();
    	...
    	if($("#WHSL_SE_CD").val() !=""){
           	ajaxPost(url, input, function(rtnData) {
 				if ("" != rtnData && null != rtnData) {
 					...
 					kora.common.setEtcCmBx2(rtnData.enp_nmList, "","", $("#ENP_NM"), "BIZRID_NO", "BIZRNM", "N");	//업체명
 					fn_enp_nm();
 				}else{
 					alertMsg("error");
 				}
 			...
 			arr=[];
		   	...
		   	input["BIZR_TP_CD"] =$("#WHSL_SE_CD").val();
       	   	ajaxPost(url, input, function(rtnData) {
   				if ("" != rtnData && null != rtnData) {   
   					 //본인지점 자동선택
   					 kora.common.setEtcCmBx2(rtnData.brch_nmList, "",rtnData.brchIdNo, $("#BRCH_NM"), "BRCH_ID_NO", "BRCH_NM", "N","S");//지점
   					 fn_brch_nm();
   				}else{   
   					alertMsg("error");   
   				}   
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	function fn_reg2(){
        ...
        if(kora.common.format_noComma(kora.common.null2void($("#BOX_QTY").val(),0)) < 0) {
            alertMsg("상자를 확인하십시요.", "kora.common.cfrmDivChkValid_focus");
            chkTarget = $("#BOX_QTY");
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('				<DataGridColumn dataField="RTN_WHSL_FEE_STAX" headerText="'+ parent.fn_text('stax')+ '"	width="150" textAlign="right"  formatter="{numfmt}" id="num5"/>');//도매부가세
			layoutStr.push('			</DataGridColumnGroup>');
			layoutStr.push('			<DataGridColumn dataField="AMT_TOT"	headerText="'+ parent.fn_text('amt_tot')+ '" width="150" textAlign="right"  formatter="{numfmt}" id="num8"/>');//금액합계(원)
			...
			layoutStr.push('				<DataGridFooterColumn summaryOperation="SUM" dataColumn="{num8}" formatter="{numfmt1}" 	textAlign="right"/>');	//소매수수료
...
<body>
    <div class="iframe_inner"  >
            ...
            <input type="hidden" id="bizr_tp_cd" value="<c:out value='${sBizrTpCd}' />" />
			<div class="h3group">
			...
			<section class="secwrap" style="display:none"> <!-- 히든처리 -->
		...
		<section class="secwrap" >
				<div class="srcharea" id="divInput" > 
					...
					<div class="row">
						<div class="col">
							<div class="tit" id="car_no"></div>  <!-- 차량번호 --> 
							<div class="box" style="width: 52%">
