...
//     	    kora.common.setEtcCmBx2(stat_cd_list, "","", $("#RTRVL_STAT_CD"), "ETC_CD", "ETC_CD_NM", "N" ,'T');							//상태
			...
			$("#RTRVL_DT").val(INQ_PARAMS.RTRVL_DT);
	  		$("#CUST_BIZRNM").val(INQ_PARAMS.CUST_BIZRNM);
	  		$("#CUST_BIZRNO").val(INQ_PARAMS.CUST_BIZRNO);
	  		$("#WHSDL_BIZRNM").val(INQ_PARAMS.WHSDL_BIZRNM);
	  		$("#WHSDL_BIZRNO").val(INQ_PARAMS.WHSDL_BIZRNO);
	  		$("#AREA_CD").val(INQ_PARAMS.AREA_CD);
			...
			if(whsdl_cd_list[0] == null){
				alert('P-BOX 정보등록이 불가능한 도매업자입니다.');
				location.href = "/WH/EPWH9000101.do";
				
			}else{
		  $("#WHSDL_BIZRNM").val(whsdl_cd_list[0].BIZRNM);
		  $("#WHSDL_BIZRNO").val(kora.common.setDelim(whsdl_cd_list[0].BIZRNO_DE, "999-99-99999"));
			//$("#WHSDL_BIZRNM option:eq(1)").prop("selected", true);
			fn_whsdl_bizrnm();
			}
     }
     ...
     function fn_whsdl_bizrnm(){
  			...
  			var arr=[];
  			if($("#WHSDL_BIZRNM").val() !=""){
  					arr = whsdl_cd_list[0].BIZRID_NO.split(";"); 
  					input["BIZRID"] 	= arr[0];
  					input["BIZRNO"] 	= arr[1];
...
// 			$('#CUST_BIZRNO').focus();
// 			return;
		//}else if($("#RTL_CUST_BIZRNM").val() == ""){
			//alertMsg("소매업자를 입력해주세요.");
			//return;
		//}
	 	}else if(kora.common.format_noComma(kora.common.null2void($("#WHSDL_BIZRNM").val(),0))  < 1) {
            alertMsg("도매업자업체명을(를) 선택하십시요.", "kora.common.cfrmDivChkValid_focus");
	 	...
	 	/* else if(kora.common.format_noComma(kora.common.null2void($("#AreaCdList_SEL").val(),0))  < 1) {
            alertMsg("지역을(를) 선택하십시요.", "kora.common.cfrmDivChkValid_focus");
            chkTarget = $("#AreaCdList_SEL");
            return;
        }  */
        else if(kora.common.format_noComma(kora.common.null2void($("#DLIVY_QTY").val(),0))  < 0) {
            alertMsg("출고량을(를) 입력하십시요.", "kora.common.cfrmDivChkValid_focus");
            chkTarget = $("#DLIVY_QTY");
            return;
        } else if(kora.common.format_noComma(kora.common.null2void($("#RTN_QTY").val(),-1))  < 0) {
            alertMsg("회수량을(를) 입력하십시요.", "kora.common.cfrmDivChkValid_focus");
            chkTarget = $("#RTN_QTY");
            return;
        } else if(kora.common.format_noComma(kora.common.null2void($("#DST_QTY").val(),-1))  < 0) {
            alertMsg("파손(분실)량을(를) 입력하십시요.", "kora.common.cfrmDivChkValid_focus");
            chkTarget = $("#DST_QTY");
            return;
        } 
	 	/* else if($("#AreaCdList_SEL").val() == ""){
			alertMsg("지역을 선택해주세요.");
			return;
		} */
		var input 	=	insRow("A");
// 		alert("3");
	...
	function fn_upd(){
 	...
 	else if(kora.common.format_noComma(kora.common.null2void($("#DLIVY_QTY").val(),0))  < 1) {
        alertMsg("출고량을(를) 입력하십시요.", "kora.common.cfrmDivChkValid_focus");
        chkTarget = $("#DLIVY_QTY");
        return;
    } else if(kora.common.format_noComma(kora.common.null2void($("#RTN_QTY").val(),-1))  < 0) {
...
// 			}
		...
		/* 	var arr2 = $("#WHSDL_BRCH_NM").val().split(";"); 												//도매업자 지점
			...
			if($("#WHSDL option:selected").val() != ""){
				 input["CUST_BIZRNM"] 			= 	 $("#WHSDL option:selected").text();				//회수처
			     input["CUST_BIZRNO"] 		= 	$("#WHSDL option:selected").val().replace(/-/g,"");						//회수처사업자번호
			}else{
				input["CUST_BIZRNM"] 			= 	 $("#RTL_CUST_BIZRNM").val();  				//회수처
			    input["CUST_BIZRNO"] 		= 	$("#RTL_CUST_BIZRNO").val().replace(/-/g,"");  						//회수처사업자번호
			}
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			//layoutStr.push('			<DataGridColumn dataField="BOX_SE_CD"  		headerText="상태코드" 				width="120" 	textAlign="center" 		  />');		//회수량
 			layoutStr.push('			<DataGridColumn dataField="AREA_CD"  	visible="false"	headerText="지역" 				width="120" 	textAlign="center" 		  />');
			...
			<section class="secwrap">
				...
				<div class="row">
						<div class="col">
							...
							<div class="box">
								<input type="text" id="WHSDL_BIZRNM"  name="WHSDL_BIZRNM" style="width: 300px"  disabled="disabled"/>
					...
					<div class="srcharea"  id="divInput" > 
							<div class="row">
                                    ...
                                    <div class="col" id="cust_n1" style="display:none">
                                        <!-- <div class="tit" id="" style="width: 109px">소매업체명</div> -->  
                                        <div class="box">
                                            <input type="text" id="RTL_CUST_BIZRNM"  maxByteLength="100" style="width: 250px"/>
                                        </div>
                                    </div>
                                    <div class="col" id="cust_n2" style="display:none" >
                                        <div class="tit" id="" style="width: 120px">소매업체 사업자번호</div>   
                                        <div class="box">
                                            <input type="text" id="RTL_CUST_BIZRNO" style="width: 250px"  maxlength="10"/>
                                        </div>
                                    </div>
                                    
                                    <!-- <div class="col" id="cust_n1" style="display: none;">
                                        <div class="box">
                                        	<input type="text" id="WHSDL2"  style="width: 179px" /> 
                                            <select id="WHSDL" style="width: 179px" ></select>
                                        </div>
                                    </div> -->
								    <!-- <div class="col">
										<div class="tit" id="" style="width: 100px">PBOX공급량</div>  빈용기명(소매)
										<div class="box">
											<select id="RTRVL_STAT_CD" style="width: 200px" class="i_notnull" ></select>
											<input type="text" id="RMG_QTY" style="text-align:right;width: 179px"  format="minus"   class="i_notnull"  maxlength="8"/>
										</div>
									</div> -->
									
									<!-- <div class="col" style="">
										<div class="tit" id="" style="width: 119px">회수량</div>  소매수수료
										<div class="box" style="" >
							...
							<div class="row">
									<div class="col" style="display: none;">
										<div class="tit" >지역</div>  <!-- 반환대상 직매장/공장 -->
										<div class="box" style="">
                           ...
                           <!--  <div class="row">
								...
								<div style="float:right ;margin-top: 10px; margin-bottom: 10px;" id="CR" >
