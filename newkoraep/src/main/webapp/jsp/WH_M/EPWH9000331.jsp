...
<script type="text/javaScript" language="javascript" defer="defer">
     ...
     $(function() {
    	...
    	ctnr_seList 				= jsonObject($("#ctnr_seList").val());		//빈용기 구분
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	function fn_reg2(){
		...
		/* if( $("#WHSL_SE_CD").val()      !=""   && $("#ENP_NM").val()     != ""   && $("#BRCH_NM").val()  != ""  ) {
		 ...
		 if(kora.common.format_noComma(kora.common.null2void($("#RCS_NM").val(),0))  < 1) {
            alert("반환수집소을(를) 선택하십시요.", "kora.common.cfrmDivChkValid_focus");
            chkTarget = $("#RCS_NM");
		 ...
		 if(kora.common.format_noComma(kora.common.null2void($("#CPCT_CD").val(),0))  < 1) {
	            alert("용량(신/구병)을(를) 선택하십시요.", "kora.common.cfrmDivChkValid_focus");
	            chkTarget = $("#CPCT_CD");
		 ...
		 if(kora.common.format_noComma(kora.common.null2void($("#RTN_QTY").val(),0))  < 1) {
	            alert("회수량을(를) 입력하십시요.", "kora.common.cfrmDivChkValid_focus");
	            chkTarget = $("#RTN_QTY");
		 ...
		 if(kora.common.format_noComma(kora.common.null2void($("#RTN_GTN").val(),0))  < 1) {
	            alert("보증금을(를) 입력하십시요.", "kora.common.cfrmDivChkValid_focus");
	            chkTarget = $("#RTN_GTN");
			...
			ajaxPost(url, data, function(rtnData){
				if(rtnData != null && rtnData != ""){
						if(rtnData.RSLT_CD =="A003"){ // 중복일경우
							...
							alert("해당 반환수집소 계약일 내의 회수 일자만 선택 가능합니다. ");
						}else if(rtnData.RSLT_CD =="0000"){
							alert(rtnData.RSLT_MSG);
							fn_cnl();
						}else if(rtnData.RSLT_CD !="0000"){
						    alert(rtnData.ERR_CTNR_NM);
						}
				}else{
						alertMsg("error");
				}
	 ...
	 */
	 function fnSetGrid1(reDrawYn) {
			...
			layoutStr.push('			<DataGridColumn dataField="RTN_GTN" labelJsFunction="convertItem" itemRenderer="HtmlItem" headerText="보증금'+"&lt;br&gt;(회수량(개)"+ ')" width="30%"/>');
			layoutStr.push('		</columns>');
	...
	function convertItem(item, value, column) {
		...
		}else if(dataField == "RTN_GTN"){
			return kora.common.format_comma(item["RTN_GTN"]) + "</br>(" + kora.common.format_comma(item["RTN_QTY"]) + ")";
		}else {
			return "";
		}
	}
...
<body>
    ...
    <div class="wrap"  >
		...
		<div id="container">
			...
			<div id="subvisual">
				<h2 class="tit"  id="title_sub">반환수집소내역작성</h2>
				<button class="btn_back" id="btn_cl"><span class="hide">뒤로가기</span></button>
			</div><!-- id : subvisual -->
			...
			<div id="contents" >
				...
				<div class="boxed v2">
							<div class="sort"  id="ctnr_nm2_txt">반환수집소</div>  <!--반환대상생산자-->
							<div class="cont"">
								<!-- <input type="text"  id="RCS_NM" class="i_notnull" style="x"/> -->
								<select class="i_notnull" id="RCS_NM" name="RCS_NM" style="width: 450px;"></select>
							</div>
				...
				<div class="box_wrap"> 
						<div class="boxed v2">
							<div class="sort" id="rtrvl_dt2_txt">회수일자</div>  <!-- 반환일자 -->
							<div class="cont">
						...
						<div class="boxed v2 mt10">
							<div class="sort">용량(신/구병)</div>  <!-- 빈용기구분 -->
							<div class="cont">
						...
						<div class="boxed v2 mt10">
							<div class="sort" >용도</div>  <!-- 빈용기구분 -->
							<div class="cont">
								<select id="PRPS_CD"   class="i_notnull">
									<option value="1">가정용(1)</option>
									<option value="0">유흥용(0)</option>
								</select>
							</div>
						</div>
					</div>	
						...
						<div class="box_wrap"> 
						<div class="boxed v2">
							<div class="sort"  >회수량</div>  <!-- 차량번호 --> 
							<div class="cont" >
								<input type="number"  placeholder="직접입력" style="text-align: right;" maxlength="10"  id="RTN_QTY"  class="i_notnull" />
							</div>
						</div>
						
						
						<div class="boxed v2 mt10">
							<div class="sort" >보증금</div>  <!-- 반환량(개) -->
							<div class="cont">
								  <input type="number" placeholder="직접입력" style="text-align: right;" maxlength="10"  id="RTN_GTN"   class="i_notnull" />
							</div>
						</div>
						</div>
					<div class="btn_wrap mt10 line">					
					<div class="fl_c">
						<button type="button" class="btnCircle c2"  id="btn_upd">변경</button>
						<button type="button" class="btnCircle c1"  id="btn_del">삭제</button>
						<button type="button" class="btnCircle c3" id="btn_reg2">추가</button>			
					</div>
					</div>
				</div>
			...
			<div class="tblbox">
				...
				<div class="btn_wrap" style="height:50px">
					<button class="btnCircle c1" style="width: 100px;" id="btn_reg">등록</button>			
				</div>
			</div>
			</div><!-- id : contents -->
			</div><!-- id : container -->
