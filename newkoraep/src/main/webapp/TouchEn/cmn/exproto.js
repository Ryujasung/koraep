...
var TOUCHENEX_CONST = {
	...
	"exDivName"		: "exDiv",
	"exIframeName"	: "ex_ifr",
	"exScriptName"	: "ex_script",
	"tmpExtArr"		: [],
	"json2Path"		: touchenexBaseDir + "/cmn/json2.js",
	"blankPath"		: touchenexBaseDir + "/cmn/blank.html"
};
...
var TOUCHENEX_CHECK = {
			...
			for(var i = 0; i < infoCnt; i++){
				...
				if(pluginInfoArr[i].checkSpec){
					if(!TOUCHENEX_UTIL.chkOS(TOUCHENEX_CONST.OSInfo, pluginInfoArr[i].reqSpec.OS)){
						exlog("_CHECK.chek", "not support OS");
					...
					if(!TOUCHENEX_UTIL.chkBrowser(TOUCHENEX_CONST.browserInfo, pluginInfoArr[i].reqSpec.Browser)){
						exlog("_CHECK.chek", "not support Browser");
	...
	extensionVersionCheck : function( updateInfo ) {
		...
		if(!eventEXListener){
			eventEXListener = function(event) {
				alert(pluginInfo.exPluginName + " extension 설치를 완료 하였습니다.");
			};
			window.addEventListener('__'+ pluginInfo.exProtocolName + '_extension_installed__', eventEXListener);
		}
		try{
			if(updateInfo == "-1") {
			...
			exlog("_CHECK.extensionVersionCheck", "");
		}
		try{
		...
		if(TOUCHENEX_UTIL.diffVersion(updateInfo, extensionVersion)) {
			...
			window.removeEventListener('__'+ pluginInfo.exProtocolName + '_extension_installed__', eventEXListener);
...
var TOUCHENEX_EX = function( property ) {
	...
	this.exDivName		= property.exDivName ? property.exDivName : TOUCHENEX_CONST.exDivName;
	this.exFormName		= property.exFormName;
	this.exFormDataName	= property.exFormDataName;
	this.exIframeName	= property.exIframeName ? property.exIframeName : TOUCHENEX_CONST.exIframeName;
	this.exScriptName	= property.exScriptName ? property.exScriptName : TOUCHENEX_CONST.exScriptName;
	...
	this.InvokeEX = function( id, callback, reqJSON){
		...
		var frm = document.getElementById(this.exFormName);
		frm.target = this.exIframeName;
		frm.action = this.createEXURL(id, this.exPluginName + ".InvokeCallback");
		var node = document.getElementById(this.exFormDataName);
		...
		if(response){
			try{
				...
				if(status == "TRUE") {	// success
					...
					// remove exInterfaceArr info
					for(var i=0; i < this.exInterfaceArr.length; i++){
						if(this.exInterfaceArr[i]){
							var arrObj = this.exInterfaceArr[i];
							if(arrObj.id == id){
								//exlog(this.exPluginName + ".InvokeCallback exInterfaceArr delete", arrObj);
...
var TOUCHENEX_UTIL = {
	...
	getOSInfo : function(){
    	...
    	if (tp == "Win32" || tp == "Win64"){
    		...
    		if((tp.search("iPhone") >= 0) || (tp.search("iPad") >= 0) || (tp.search("iPod") >= 0)){
    			result.platform = "UNKNOWN";
    		} else {
	...
	chkCrossEXDiv : function( exObj ) {
	
		var chk = document.getElementById(exObj.exDivName);
		if(!chk) {
			var exDiv = document.createElement("div");
			exDiv.id = exObj.exDivName;
			exDiv.style.display = "none";
			
			var exIframe = document.createElement("iframe");
			exIframe.id = exObj.exIframeName;
			exIframe.name = exObj.exIframeName;
			...
			for(var i = 0; i < exObj.pluginInfo.length ; i++){
				var exForm = document.createElement("form");
				exForm.method = "POST";
				exForm.id = exObj.pluginInfo[i].exFormName;
				exForm.name = exObj.pluginInfo[i].exFormName;
				
				var exNode = document.createElement("input");
				exNode.type = "hidden";
				exNode.id = exObj.pluginInfo[i].exFormDataName;
				exNode.name = exObj.pluginInfo[i].exFormDataName;
