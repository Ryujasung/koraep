...
import egovframework.mapper.wh.ep.EPWH8149001Mapper;
 ...
 *
 */
@Service("epwh8149001Service")
public class EPWH8149001Service {
	
	@Resource(name="epwh8149001Mapper")
	private EPWH8149001Mapper epwh8149001Mapper;
	 ...
	 * @
	 */
	public ModelMap epwh8149001(ModelMap model, HttpServletRequest request)  {
		...
		//공지사항 총 게시글 수 조회
		List<?> totalCnt = epwh8149001Mapper.epwh8149001(map);
		...
		String title = commonceService.getMenuTitle("EPWH8149001");
	 ...
	 * @
	 */
	public HashMap<String, Object> epwh8149001_select(Map<String, String> data, HttpServletRequest request)  {
		...
		//공지사항 검색 게시글 수 조회
		List<?> totalCnt = epwh8149001Mapper.epwh8149001(data);
		
		//공지사항 조회
		List<?> list = epwh8149001Mapper.epwh8149001_select1(data);
		
		//첨부파일 있는지 조회
		List<?> fileList = epwh8149001Mapper.epwh8149001_select2(data);
		...
		for(int i=0;i<list.size();i++){
			...
			if(0 < fileList.size()){
				...
				for(int j=k;j<fileList.size();j++){
					...
					if(String.valueOf(rtn.get("NOTI_SEQ")).equals(String.valueOf(rtnFile.get("NOTI_SEQ")))){
						sTmpSbj += "&nbsp;&nbsp;<img style='vertical-align:middle' src='/images/util/attach_ico.png'/>" ;
						k++;
						break;
					}
				}
			...
			System.out.println("SBJ : " + sTmpSbj);
			rtn.put("SBJ", sTmpSbj);
		}
