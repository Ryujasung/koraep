 ...
 *
 */
@Service("epce0150201Service")
public class EPCE0150201Service {
	 ...
	 * @
	 */
	  public ModelMap epce0150201_select(ModelMap model, HttpServletRequest request) {
		  ...
		  try {
			...
			List<?> stat_cd_list				= epce0150201Mapper.epce0150201_select3();			//활동 비활동
			...
			String   title							= commonceService.getMenuTitle("EPCE0150201");		//타이틀
	  ...
	  /**
		 ...
		 * @
		 */
		  public ModelMap epce0150231_select(ModelMap model, HttpServletRequest request) {
			  ...
			  try {
					...
					String   title					= commonceService.getMenuTitle("EPCE0150231");		//타이틀
					model.addAttribute("mfc_bizrnmList", util.mapToJson(mfc_bizrnmList));		
					model.addAttribute("area_cd_list", util.mapToJson(area_cd_list));	
		  ...
		  /**
			 ...
			 * @
			 */
			  public ModelMap epce0150242_select(ModelMap model, HttpServletRequest request) {
				  ...
				  try {
						...
						if(jmap != null){
							...
							HashMap<String, String> map = (HashMap<String, String>)epce0150201Mapper.epce0150231_select2(jmap);
						...
						List<?> area_cd_list = commonceService.getCommonCdListNew("B010"); //지역
						String  title = commonceService.getMenuTitle("EPCE0150242"); //타이틀	
						model.addAttribute("area_cd_list", util.mapToJson(area_cd_list));	
						model.addAttribute("titleSub", title);
		 ...
		 * @
		 */
		public HashMap epce0150201_select2(Map<String, String> inputMap, HttpServletRequest request) {
		 ...
		 * @
		 */
		public HashMap epce0150201_select3(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    		try {
					rtnMap.put("selList", util.mapToJson(epce0150201Mapper.epce0150201_select  (inputMap)));
				}catch (IOException io) {
					io.getMessage();
				}catch (SQLException sq) {
					sq.getMessage();
	    		...
	    		rtnMap.put("totalCnt", epce0150201Mapper.epce0150201_select_cnt(inputMap));
	    	return rtnMap;    	
	    }
		 ...
		 * @
		 */
		public String epce0150201_excel(HashMap<String, String> data, HttpServletRequest request) {
			
			String errCd = "0000";
			try {
						
				List<?> list = epce0150201Mapper.epce0150201_select(data);
				//엑셀파일 저장
		 ...
		 * @
		 */
		public String epce0150201_update(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
			String errCd = "0000";
			...
			if (list != null) {
				try {
					for(int i=0; i<list.size(); i++){
						...
						int stat = epce0150201Mapper.epce0150201_select2(map); //상태 체크
						 if(stat>0){
							throw new Exception("A012"); //상태정보가 변경되어있습니다. 다시 한 번 확인하시기 바랍니다.
						 }
						 map.put("REG_PRSN_ID", vo.getUSER_ID());  				//등록자
						 epce0150201Mapper.epce0150201_update(map); 		//상태변경
					}
				}catch (IOException io) {
					io.getMessage();
				}catch (SQLException sq) {
 ...
 ****************************************************************************************************************************************************************************************/
		 ...
		 * @
		 */
		public HashMap<String, Object> epce0150231_select(Map<String, String> data) {
			...
			try {
				map.put("grpList", util.mapToJson(epce0150201Mapper.epce0150231_select(data)));
			}catch (IOException io) {
				io.getMessage();
			}catch (SQLException sq) {
				sq.getMessage();
		 ...
		 * @
		 */
		public String epce0150231_insert(HashMap<String, String> data, HttpServletRequest request) throws Exception {
			...
			try {
					...
					int ck = epce0150201Mapper.epce0150231_select3(data);
					if(ck > 0){
					...
					epce0150201Mapper.epce0150231_insert(data);	//등록 처리
		 ...
		 * @
		 */
		public String epce0150242_update(HashMap<String, String> data, HttpServletRequest request) throws Exception {
			...
			try {
					...
					epce0150201Mapper.epce0150242_update(data);	//수정
 ...
 ****************************************************************************************************************************************************************************************/
		 ...
		 * @
		 */
		  public ModelMap epce0150288_select(ModelMap model, HttpServletRequest request) {
			  
			  try {
					List<?> area_cd_list			= commonceService.getCommonCdListNew("B010");		//지역
					String   title						= commonceService.getMenuTitle("EPCE0150288");		//타이틀
					model.addAttribute("area_cd_list", util.mapToJson(area_cd_list));	
					model.addAttribute("titleSub", title);
				}catch (IOException io) {
					io.getMessage();
		  ...
		  /**
			 ...
			 * @
			 */
			public String epce0150288_update(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
				String errCd = "0000";
				Map<String, String> map;
				List<?> list = JSONArray.fromObject(inputMap.get("list"));
				HttpSession session = request.getSession();
				...
				if (list != null) {
					try {
						for(int i=0; i<list.size(); i++){
							map = (Map<String, String>) list.get(i);
							map.put("REG_PRSN_ID", vo.getUSER_ID());  					//등록자
							epce0150201Mapper.epce0150288_update(map); 		//상태변경
						}
					}catch (IOException io) {
						io.getMessage();
					}catch (SQLException sq) {
	...
	public String epce0150231_3_check(HashMap<String, String> data) {
		String rtn = "Y";
		...
		int cnt = epce0150201Mapper.epce0150231_select3(data);
		if(cnt > 0) rtn = "N";
