...
import egovframework.mapper.ce.ep.EPCE2916431Mapper;
 ...
 *
 */
@Service("epce2916431Service")
public class EPCE2916431Service {
	
	
	@Resource(name="epce2916431Mapper")
	private EPCE2916431Mapper epce2916431Mapper;  //실태조사 입고내역서조정 Mapper
	...
	private CommonCeService commonceService;  //공통
		 ...
		 * @
		 */
		  public ModelMap epce2916431_select(ModelMap model, HttpServletRequest request) {
				...
				try {
					String title	= commonceService.getMenuTitle("EPCE2916431");		//타이틀
					...
					List<?> iniList				= epce2916431Mapper.epce2916431_select(map);		//상세내역 조회
					selCnt 						= epce2916431Mapper.epce2916431_select6(map); 	
					if(selCnt>0){
						cfm_gridList			= epce2916431Mapper.epce2916431_select7(map);		//그리드쪽 조회 입고정보 있을 경우 
					}
					List<?>  gridList			= epce2916431Mapper.epce2916431_select2(map);		//그리드쪽 조회  
			 ...
			 * @
			 */
			public HashMap epce2916431_select2(Map<String, String> inputMap, HttpServletRequest request) {
			...
			public String epce2916431_insert(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
					...
					if (list != null) {
						try {
							...
							int stat = epce2916431Mapper.epce2916431_select4(check); //상태 체크
							 if(stat>0){
								throw new Exception("A009"); 
							 }
							 int sel = epce2916431Mapper.epce2916431_select5(check); // 수량 체크
							 if(sel != 1){
								 throw new Exception("A008"); 
							 }
			
							 List<?>  gridList	= util.mapToJson(epce2916431Mapper.epce2916431_select2(check));// 입고 정보 초기 값 데이터 비교
							...
							for(int i=0; i<list.size(); i++){
								...
								map.put("REG_PRSN_ID", vo.getUSER_ID());  											//등록자
							 	...
							 	if(!keyCheck){
										 ...
										 epce2916431Mapper.epce2916431_insert(map); 							//	입고마스터	  등록
										 epce2916431Mapper.epce2916431_update3(map);						// 실태조사확인상태 up 
								 		 list2.add(map);
								 		 keyCheck = true;
							 	}	
							 	//detail  
							 	epce2916431Mapper.epce2916431_insert2(map); 			// 입고상세
							 	epce2983931Mapper.epce29839882_insert3(map);		//  증빙사진 첨부 up
								 
							}//end of for
							
						 	for(int j=0 ;j<list2.size(); j++){
						 		Map<String, String> map = (Map<String, String>) list2.get(j);
						 		epce2916431Mapper.epce2916431_update(map);			//입고 마스터 총값 넣기
						 		epce2916431Mapper.epce2916431_update2(map); 		//반환 마스터  상태변경
			 ...
			 */
		    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
			public String epce2916431_update(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
					HttpSession session = request.getSession();
					UserVO vo = (UserVO) session.getAttribute("userSession");
							...
							for(int i=0; i<list.size(); i++){
								...
								map.put("UPD_PRSN_ID", vo.getUSER_ID());  					//로그인 수정 등록자 ,   REG_PRSN_ID : 최초등록자
							 	if(!keyCheck){
								 		...
								 		epce2916431Mapper.epce2916431_delete(map);		// 입고상세 삭제
								 		epce2916431Mapper.epce2916431_update4(map); 	//입고 마스터  상태변경및 입고확인입력
								 		epce2916431Mapper.epce2916431_update2(map); 	//반환 마스터  상태변경및 입고확인입력
								 		epce2916431Mapper.epce2916431_update3(map);	//실태조사확인상태
							 	}	 
							 	//detail
							 	epce2916431Mapper.epce2916431_insert2(map); 				// 입고상세
							 	epce2983931Mapper.epce29839882_insert3(map); 			// 증빙사진 등록  UPD_PRSN_ID-로그인자 
							}//end of for
						 	for(int j=0 ;j<list2.size(); j++){
						 		Map<String, String> last_map = (Map<String, String>) list2.get(j);
						 		epce2916431Mapper.epce2916431_update(last_map);		//입고 마스터 총값 넣기
