...
import egovframework.mapper.ce.ep.EPCE4705601Mapper;
 ...
 *
 */
@Service("epce4705601Service")
public class EPCE4705601Service {
	
	@Resource(name="epce4705601Mapper")
	private EPCE4705601Mapper epce4705601Mapper;  //수기입고정정  Mapper
	 ...
	 * @
	 */
	  public ModelMap epce4705601_select(ModelMap model, HttpServletRequest request) {
			...
			try {
						...
						model.addAttribute("titleSub", commonceService.getMenuTitle("EPCE4705601"));										//타이틀
				} catch (Exception e) {
	  ...
	  /**
		 ...
		 * @
		 */
		public HashMap epce4705601_select5(Map<String, String> inputMap, HttpServletRequest request) {
		 ...
		 * @
		 */
		public HashMap epce4705601_select6(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    	try {
	    		rtnMap.put("chkCnt", epce4705601Mapper.epce4705601_select7(inputMap));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
			}	 	 
		 ...
		 * @
		 */
		public HashMap epce4705601_select7(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
		 ...
		 * @
		 */
		public HashMap epce4705601_select2(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
		 ...
		 * @
		 */
		public HashMap epce4705601_select3(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    		try {
		 ...
		 * @
		 */
		public HashMap epce4705601_select4(Map<String, Object> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    		try {
	    			List<?> list = JSONArray.fromObject(inputMap.get("MFC_BIZRNM_RETURN"));
	    			inputMap.put("MFC_BIZRNM_RETURN", list);  
					rtnMap.put("selList", util.mapToJson(epce4705601Mapper.epce4705601_select4(inputMap)));
					rtnMap.put("totalList", epce4705601Mapper.epce4705601_select4_cnt(inputMap));
				} catch (Exception e) {
					// TODO Auto-generated catch block
					org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
				}	  
		 ...
		 * @
		 */
		public String epce4705601_excel(HashMap<String, Object> data, HttpServletRequest request) {
			...
			try {
				...
				List<?> list = epce4705601Mapper.epce4705601_select4(data);
		 ...
		 * @
		 */
		public String epce4705601_update(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
			String errCd = "0000";
			...
			if (list != null) {
				try {
					for(int i=0; i<list.size(); i++){
						...
						int stat = epce4705601Mapper.epce4705601_select6(map); //상태 체크
						 if(stat>0){
							throw new Exception("A012"); //상태정보가 변경되어있습니다. 다시 한 번 확인하시기 바랍니다.
						 }
						 map.put("REG_PRSN_ID", vo.getUSER_ID());  		//등록자
						 epce4705601Mapper.epce4705601_update(map);//수기입고정정  정정확인,정정반려,확인취소 상태 변경
					}
				}catch (Exception e) {
		 ...
		 * @
		 */
        public ModelMap epce4705664_select(ModelMap model, HttpServletRequest request) {
			...
			String   title					= commonceService.getMenuTitle("EPCE4705664");		//타이틀
			List<?> iniList				= epce4705601Mapper.epce4705664_select(map);		//상세내역 조회
			List<?> cfm_gridList		= null;
			
			if(map.containsKey("WRHS_CRCT_DOC_NO")){
			    cfm_gridList = epce4705601Mapper.epce4705664_select2(map);		//입고 그리드쪽 조회
			}
			List<?> crct_gridList		= epce4705601Mapper.epce4705664_select3(map);		//수기입고정정 그리드쪽 조회
		 ...
		 * @throws Exception 
			 * @
			 */
			public String epce4705664_delete(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
				...
				try {
					 int stat = epce4705601Mapper.epce4705601_select5(inputMap); //상태 체크
					 if(stat>0){
							throw new Exception("A012"); //상태정보가 변경되어있습니다. 다시 한 번 확인하시기 바랍니다.
					 }
					 
					 inputMap.put("UPD_PRSN_ID", vo.getUSER_ID());
					 epce4705601Mapper.epce4705664_delete(inputMap); 	// 수기입고정정테이블 삭제
					 
					 if(inputMap.containsKey("WRHS_DOC_NO")){//문서번호가 없는건 수기등록임..
						 epce4705601Mapper.epce4705664_update(inputMap); 	// 연결정정문서번호 삭제
					 }
			...
			****************************************************************************************************************************************************************************************/
				 ...
				 * @
				 */
				  public ModelMap epce4705631_select(ModelMap model, HttpServletRequest request) {

					  	//파라메터 정보
						String reqParams = util.null2void(request.getParameter("INQ_PARAMS"),"{}");
						JSONObject jParams = JSONObject.fromObject(reqParams);
						String title = commonceService.getMenuTitle("EPCE4705631"); //타이틀
						...
						try {
							...
							List<?> iniList			= epce4705601Mapper.epce4705631_select(map);		//상세내역 조회
							List<?> cfm_gridList 	= epce4705601Mapper.epce4705631_select2(map);		//그리드쪽 조회 입고정정 데이터
					 ...
					 * @
					 */
					public HashMap epce4705631_select2(Map<String, String> inputMap, HttpServletRequest request) {
				    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
				    	try {
					...
					public String epce4705631_insert(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
							HttpSession session = request.getSession();
							UserVO vo = (UserVO) session.getAttribute("userSession");
							String errCd = "0000";
							//Map<String, String> map;
							...
							if (list != null) {
								try {
											...
											/*
											 int sel = epce4705601Mapper.epce4705631_select6(check); // 적용기간 체크
											 if(sel >0 ){
												 throw new Exception("A017"); 
											 }
											 */
											 
											// 190129 신영희 주임 - 반환수량과 확인수량이 일치하지 않아도 등록 가능해야 함
											/*
											int sel = epce4705601Mapper.epce4705631_select5(check); // 수량 체크
												if(sel != 1){
													 throw new Exception("A016"); 
											}
											*/
											
											List<?>  gridList	= util.mapToJson(epce4705601Mapper.epce4705631_select2(check));// 입고정보 초기 값 데이터 비교
											...
											for(int i=0; i<list.size(); i++){
												...
												/*
												sel = epce4705601Mapper.epce4705631_select4(map); //중복 체크 
												if(sel>0){
												...
												*/
												...
												epce4705601Mapper.epce4705631_insert(map); // 입고정정 등록
												
												
												if(flag){ //한번만 실행
													epce4705601Mapper.epce4705631_update(map);	//연결입고정정문서번호 업데이트
													flag = false;
												}
			...
			****************************************************************************************************************************************************************************************/
				 ...
				 * @
				 */
				  public ModelMap epce47056312_select(ModelMap model, HttpServletRequest request) {
						...
						String title = commonceService.getMenuTitle("EPCE47056312");	 
					 ...
					 */
				    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
					public String epce47056312_insert(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
							HttpSession session = request.getSession();
							UserVO vo = (UserVO) session.getAttribute("userSession");
							String errCd = "0000";
			...
			****************************************************************************************************************************************************************************************/
					 ...
					 * @
					 */
					  public ModelMap epce47056422_select(ModelMap model, HttpServletRequest request) {
							...
							String title = commonceService.getMenuTitle("EPCE47056422");	 

							List<?> ctnr_se_list		= commonceService.getCommonCdListNew("E005");		//빈용기구분 구/신
							List<?> ctnr_prps_list	= commonceService.ctnr_se_select(request, map);		//빈용기구분 유흥/가정/직접 조회
							List<?> rmk_list			= commonceService.getCommonCdListNew("D025");		//소매수수료 적용여부 비고
							List<?> crct_gridList 	= epce4705601Mapper.epce47056422_select(map);		//입고정정재등록 그리드쪽 조회 
					 ...
					 * @
					 */
					  public ModelMap epce4705642_select(ModelMap model, HttpServletRequest request) {
						 	
						  	//파라메터 정보
							String reqParams = util.null2void(request.getParameter("INQ_PARAMS"),"{}");
							JSONObject jParams = JSONObject.fromObject(reqParams);
							String title = commonceService.getMenuTitle("EPCE4705642"); //타이틀
							...
							List<?> ctnr_seList		= commonceService.ctnr_se_select(request, map);		//빈용기구분 유흥/가정/직접 조회
							List<?> iniList			= epce4705601Mapper.epce4705631_select(map);		//상세내역 조회
							List<?> cfm_gridList 	= epce4705601Mapper.epce4705631_select2(map);		//입고정정 그리드쪽 조회 
							List<?> crct_gridList 	= epce4705601Mapper.epce4705642_select(map);		//입고정정재등록 그리드쪽 조회 
							List<?> ctnr_nm			= commonceService.ctnr_cd_select(map);					//빈용기명 조회
							List<?> rmk_list			= commonceService.getCommonCdListNew("D025");		//소매수수료 적용여부 비고
					  ...
					  /**
						 ...
						 */
					    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
						public String epce47056422_update(Map<String, String> inputMap, HttpServletRequest request) throws Exception  {
								HttpSession session = request.getSession();
								UserVO vo = (UserVO) session.getAttribute("userSession");
								String errCd = "0000";
								//Map<String, String> map;
								...
								if (list != null) {
									try {
											...
											epce4705601Mapper.epce4705642_delete(check); // 입고정정 삭제
						 ...
						 */
					    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
						public String epce4705642_update(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
								HttpSession session = request.getSession();
								UserVO vo = (UserVO) session.getAttribute("userSession");
								String errCd = "0000";
								//Map<String, String> map;
								...
								if (list != null) {
									try {
												 ...
												 */
												 /*
												 sel = epce4705601Mapper.epce4705631_select6(check); // 적용기간 체크
												 if(sel >0 ){
													 throw new Exception("A017"); 
												 }
												 */
												List<?>  gridList	= util.mapToJson(epce4705601Mapper.epce4705642_select(check));// 입고정정 초기 값 데이터 비교
												//System.out.println("gridList :  " +gridList.size()  +  "  :  list_ori  :" +list_ori.size() );
												if(list_ori.size() ==gridList.size() ){
														 for(int k =0;k<list_ori.size();k++){
															 Map<String, String> list_ori_map = new HashMap<String, String>();
		...
		****************************************************************************************************************************************************************************************/
			 ...
			 * @
			 */
			  public ModelMap epce47056642_select(ModelMap model, HttpServletRequest request) {
					...
					String   title					= commonceService.getMenuTitle("EPCE47056642");		//타이틀
				 ...
				 * @
				 */
				public HashMap epce47056642_select2(Map<String, String> inputMap, HttpServletRequest request) {
			    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
				 ...
				 * @
				 */
				public HashMap epce47056642_select3(Map<String, String> inputMap, HttpServletRequest request) {
			    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
			    		try {
				 ...
				 * @
				 */
				public HashMap epce47056642_select4(Map<String, String> inputMap, HttpServletRequest request) {
			    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
				 ...
				 * @
				 */
				public HashMap epce47056642_select5(Map<String, Object> inputMap, HttpServletRequest request) {
			    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
			    		try {
							...
							rtnMap.put("selList", util.mapToJson(epce4705601Mapper.epce47056642_select(inputMap)));
							rtnMap.put("totalCnt", epce4705601Mapper.epce47056642_select_cnt(inputMap));
						} catch (Exception e) {
							// TODO Auto-generated catch block
							org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
						}	  
