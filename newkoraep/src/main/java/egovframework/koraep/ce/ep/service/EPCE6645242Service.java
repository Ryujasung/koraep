 ...
 *
 */
@Service("epce6645242Service")
public class EPCE6645242Service {
	
	@Resource(name="epce6645231Mapper")
	private EPCE6645231Mapper epce6645231Mapper;
	
	@Resource(name="epce6645242Mapper")
	private EPCE6645242Mapper epce6645242Mapper;
	 ...
	 * @
	 */
	public ModelMap epce6645242_select1(ModelMap model, HttpServletRequest request)  {
		...
		String  title = commonceService.getMenuTitle("EPCE6645242");		//타이틀
		...
		List<?> initList	= epce6645242Mapper.epce6645242_select4(map);	//그리드쪽 조회
    	...
    	try {
        	...
        	//빈용기명 조회
			model.addAttribute("ctnr_nm", util.mapToJson(epce6645242Mapper.epce6645242_select1(map)));
		} catch (Exception e) {
	...
	public String epce6645242_update(Map<String, Object> inputMap, HttpServletRequest request) throws Exception  {
		...
		/*상태 체크*/
		int stat = epce6645242Mapper.epce6645242_select5(jmap);
		if(stat>0){
		...
		if (list != null) {
			try {
				...
				for(int i=0; i<list.size(); i++){
					...
					for(int j=0 ;j<list2.size(); j++){
				 	...
				 	if(!keyCheck){
				 		if(cnt == 0){
				 			list2.add(map);
				 			epce6645242Mapper.epce6645242_delete(map); 		//모든 데이터 삭제
				 	...
				 	/*중복체크*/
				 	int sel = epce6645242Mapper.epce6645242_select6(map);
					if(sel>0){
					...
					/*중복체크*/
				 	...
				 	//detail
				 	epce6645242Mapper.epce6645242_insert(map); 		// 직접회수상세 등록
					 
				}//end of for
				
			 	for(int j=0 ;j<list2.size(); j++){
			 		Map<String, String> map = (Map<String, String>) list2.get(j);
			 		int result = epce6645242Mapper.epce6645242_update(map);
			 		if(result == 0){
				...
				if(e.getMessage().equals("A014") || e.getMessage().equals("A012") || e.getMessage().equals("A008") ){
	 ...
	 * @
	 */
	public HashMap<String, Object> epce6645242_select2(Map<String, String> data, HttpServletRequest request)  {
    	...
    	try {
	    	...
	    	if(null != data){
				 ...
				 * else { data.put("BIZR_TP_CD", ""); }
				 */
	    	//빈용기명 조회
	    	rtnMap.put("ctnr_nm", util.mapToJson(epce6645242Mapper.epce6645242_select1(data)));
	 ...
	 * @
	 */
	public HashMap epce6645242_select3(Map<String, String> inputMap, HttpServletRequest request) {
    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	 ...
	 * @
	 */
	public HashMap epce6645242_select4(Map<String, String> inputMap, HttpServletRequest request) {
    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
    	try {
			rtnMap.put("ctnr_nm", util.mapToJson(epce6645242Mapper.epce6645242_select1(inputMap)));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
		}    	 	//빈용기명 조회
	 ...
	 * @
	 */
	public String epce6645242_delete(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
		String errCd = "0000";
		Map<String, String> map;
		
		
			int stat = epce6645242Mapper.epce6645242_select5(inputMap); //상태 체크 반환등록 상태 아닌거면 삭제 안되게
			 if(stat>0){
