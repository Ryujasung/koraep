...
import egovframework.mapper.mf.ep.EPMF2983901Mapper;
 ...
 *
 */
@Service("epmf2983901Service")
public class EPMF2983901Service {  
	
	
	@Resource(name="epmf2983901Mapper")
	private EPMF2983901Mapper epmf2983901Mapper;  //입고관리 Mapper
	 ...
	 * @
	 */
	  public ModelMap epmf2983901_select(ModelMap model, HttpServletRequest request) {
			...
			String   title				= commonceService.getMenuTitle("EPMF2983901");		//타이틀
			List<?>	whsdlList		= commonceService.mfc_bizrnm_select4(request, map);    			// 생산자랑 거래중인 도매업자 업체명조회
			String   detail 				= "F";
			List<?> grid_info			= commonceService.GRID_INFO_SELECT("EPMF2983901",request);		//그리드 컬럼 정보
		 ...
		 * @
		 */
		public HashMap epmf2983901_select2(Map<String, String> inputMap, HttpServletRequest request) {
		 ...
		 * @
		 */
		public HashMap epmf2983901_select3(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    		try {
		 ...
		 * @
		 */
		public HashMap epmf2983901_select4(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
		 ...
		 * @
		 */
		public HashMap epmf2983901_select5(Map<String, String> inputMap, HttpServletRequest request) {
	    	...
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
    		try {
				rtnMap.put("selList", util.mapToJson(epmf2983901Mapper.epmf2983901_select4(inputMap)));
				rtnMap.put("totalList", util.mapToJson(epmf2983901Mapper.epmf2983901_select4_cnt(inputMap)));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
			}	  
		 ...
		 * @
		 */
		public String epmf2983901_excel(HashMap<String, String> data, HttpServletRequest request) {
			...
			try {
				...
				List<?> list = epmf2983901Mapper.epmf2983901_select4(data);
				//엑셀파일 저장
		 ...
		 * @
		 */
		public String epmf2983901_update(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
			...
			if (list != null) {  // 중복, 상태 확인
				for(int i=0; i<list.size(); i++){
					...
					int stat = epmf2983901Mapper.epmf2983901_select5(map); //상태 체크
					 if(stat>0){
				...
				try {
					for(int i=0; i<list.size(); i++){
						...
						if(map.get("RTN_STAT_CD").equals("RR")){
							...
							epmf2983901Mapper.epmf2983901_insert3(map); 		//실태조사요청정보 테이블에 등록
						}
						
							//실태조사중 반환등록 상태인경우 입고등록 이 안되있어서 반환내역서에만 반환상태 변경
							epmf2983901Mapper.epmf2983901_update(map); 	// 반환내역서 반환상태 변화
						if(map.get("CFM_CHECK").equals("T")){
							//입고조정 및 입고확인은 입고등록이 되어있어서 입고내역서도 반환상태 변경을 해야한다.
							epmf2983901Mapper.epmf2983901_update2(map); 	// 입고내역서 반환상태 변화
						}
						
						//취소요청 사유저장
						if(map.containsKey("CNL_REQ_YN") && map.get("CNL_REQ_YN").equals("Y")){
							map.put("REQ_RSN", reqRsn);
							epmf2983901Mapper.epmf29839884_insert(map);
		 ...
		 * @
		 */
		public String epmf2983901_update2(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
			HttpSession session = request.getSession();
			UserVO vo = (UserVO) session.getAttribute("userSession");
			...
			if (list != null) {
				try {
					...
					for(int i=0; i<list.size(); i++){
						...
						//상태 먼저 변경함.. 데이터 변조 대비
						epmf2983901Mapper.epmf2983901_update4(map); 		//반환 마스터  상태변경
						 
						epmf2983901Mapper.epmf2983901_insert(map); 			//	입고마스터	  등록

						//detail
				 		epmf2983901Mapper.epmf2983901_insert2(map); 		// 입고상세
				 	...
				 	for(int j=0 ;j<list2.size(); j++){
				 		...
				 		epmf2983901Mapper.epmf2983901_update3(map_list2);			//입고 마스터 총값 넣기
				 		//epmf2983901Mapper.epmf2983901_update4(map_list2); 			//반환 마스터  상태변경
				 	}
		 ...
		 * @
		 */
		  public ModelMap epmf2983964_select(ModelMap model, HttpServletRequest request) {
				...
				*/
				...
				String   title					= commonceService.getMenuTitle("EPMF2983964");		//타이틀
				List<?> iniList				= epmf2983901Mapper.epmf2983964_select(map);		//상세내역 조회
				List<?> rtn_gridList			= epmf2983901Mapper.epmf2983964_select2(map);		//반환 그리드쪽 조회
				List<?> cfm_gridList		= epmf2983901Mapper.epmf2983964_select3(map);		//입고 그리드쪽 조회
		  ...
		  /**
			 ...
			 * @
			 */
			public HashMap epmf2983964_select2(Map<String, String> inputMap, HttpServletRequest request) {
		    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
		    		try {
						rtnMap.put("iniList", util.mapToJson(epmf2983901Mapper.epmf2983964_select  (inputMap)));
					} catch (Exception e) {
						// TODO Auto-generated catch block
						org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
					}	  
			 ...
			 * @
			 */
			public String epmf2983964_update(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
				String errCd = "0000";
				Map<String, String> map;
				HttpSession session = request.getSession();
				UserVO vo = (UserVO) session.getAttribute("userSession");
					...
					try {
							int stat = epmf2983901Mapper.epmf2983901_select5(inputMap); //상태 체크
							 if(stat>0){
							...
							epmf2983901Mapper.epmf2983901_update2(inputMap); 	// 입고내역서 반환상태 변화
							epmf2983901Mapper.epmf2983901_update4(inputMap); 	// 반환내역서 반환상태 변화
			 ...
			 * @   
			 */    
			public String epmf2983964_update2(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
				String errCd = "0000";  
				Map<String, String> map;           
				HttpSession session = request.getSession();
				UserVO vo = (UserVO) session.getAttribute("userSession");
				...
				if (list != null) {                
						try {       
								for(int i=0; i<list.size(); i++){
										...
										inputMap.put("UPD_PRSN_ID", vo.getUSER_ID());  	//등록자
										epmf2983901Mapper.epmf2983964_update(map); 	//반환내역서 반환상태 변화
										epmf2983901Mapper.epmf2983964_delete(map); 	//입고증빙파일 테이블 삭제
										epmf2983901Mapper.epmf2983964_delete2(map); 	//입고상세 테이블 삭제
										epmf2983901Mapper.epmf2983964_delete3(map); 	//입고마스터 테이블 삭제
										if(map.get("RSRC_DOC_NO") !=null){
											epmf2983901Mapper.epmf2983964_delete4(map);	//실태조사증빙파일 테이블 삭제
											epmf2983901Mapper.epmf2983964_delete5(map);	//실태조사요청정보 테이블 삭제
										}
								}  
						}catch (Exception e) {
							 if(e.getMessage().equals("A012")){
	...
	//---------------------------------------------------------------------------------------------------------------------
		 ...
		 * @
		 */
		public HashMap epmf2983988_select(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    		String   title	= commonceService.getMenuTitle("EPMF2983988");		//타이틀
		 ...
		 * @
		 */
		public String epmf2983988_insert(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
			String errCd = "0000";
			Map<String, String> map;
			HttpSession session = request.getSession();
			UserVO vo = (UserVO) session.getAttribute("userSession");
				...
				try {
						...
						epmf2983901Mapper.epmf2983988_insert(inputMap);								// 실태조사요청 저장
						epmf2983901Mapper.epmf2983988_update(inputMap);							// 반환관리,입고관리 반환상태 조사요청으로 변경
	...
	//---------------------------------------------------------------------------------------------------------------------
		  ...
		  /**
			 ...
			 * @
			 */
			public HashMap epmf29839883_select(Map<String, String> inputMap, HttpServletRequest request) {
		    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
		    		String   title					= commonceService.getMenuTitle("EPMF29839883");		//타이틀
		    		try {
						rtnMap.put("selList", util.mapToJson(epmf2983901Mapper.epmf29839883_select (inputMap)));
					} catch (Exception e) {
						// TODO Auto-generated catch block
						org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
					}
