...
import egovframework.mapper.mf.ep.EPMF4738701Mapper;
 ...
 *
 */
@Service("epmf4738701Service")
public class EPMF4738701Service {
	
	@Resource(name="epmf4738701Mapper")
	private EPMF4738701Mapper epmf4738701Mapper;  //입고정정  Mapper
	 ...
	 * @
	 */
	  public ModelMap epmf4738701_select(ModelMap model, HttpServletRequest request) {
			...
			try {
					...
					model.addAttribute("titleSub", commonceService.getMenuTitle("EPMF4738701"));											//타이틀;
			} catch (Exception e) {
	  ...
	  /**
		 ...
		 * @
		 */
		public HashMap epmf4738701_select5(Map<String, String> inputMap, HttpServletRequest request) {
		 ...
		 * @
		 */
		public HashMap epmf4738701_select2(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    	System.out.println("paramMap"+request);
		 ...
		 * @
		 */
		public HashMap epmf4738701_select3(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    		try {
		 ...
		 * @
		 */
		public HashMap epmf4738701_select4(Map<String, Object> inputMap, HttpServletRequest request) {
	    	...
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    		try {
					...
					rtnMap.put("selList", util.mapToJson(epmf4738701Mapper.epmf4738701_select4(inputMap)));
					rtnMap.put("totalList", epmf4738701Mapper.epmf4738701_select4_cnt(inputMap));
				} catch (Exception e) {
					// TODO Auto-generated catch block
					org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
				}	  
		 ...
		 * @
		 */
		public String epmf4738701_excel(HashMap<String, Object> data, HttpServletRequest request) {
			...
			try {
				...
				List<?> list = epmf4738701Mapper.epmf4738701_select4(data);
		 ...
		 * @
		 */
		public String epmf4738701_update(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
			String errCd = "0000";
			...
			if (list != null) {
				try {
					for(int i=0; i<list.size(); i++){
						...
						int stat = epmf4738701Mapper.epmf4738701_select5(map); //상태 체크
						 if(stat>0){
							throw new Exception("A012"); //상태정보가 변경되어있습니다. 다시 한 번 확인하시기 바랍니다.
						 }
						 map.put("REG_PRSN_ID", vo.getUSER_ID());  		//등록자
						 epmf4738701Mapper.epmf4738701_update(map);//입고정정  정정확인,정정반려,확인취소 상태 변경
					}
				}catch (Exception e) {
		 ...
		 * @
		 */
		  public ModelMap epmf4738764_select(ModelMap model, HttpServletRequest request) {
				...
				String   title						= commonceService.getMenuTitle("EPMF4738764");		//타이틀
				List<?> iniList					= epmf4738701Mapper.epmf4738764_select(map);		//상세내역 조회
				List<?> cfm_gridList			= epmf4738701Mapper.epmf4738764_select2(map);		//입고 그리드쪽 조회
				List<?> crct_gridList			= epmf4738701Mapper.epmf4738764_select3(map);		//입고정정 그리드쪽 조회
		 ...
		 * @throws Exception 
			 * @
			 */
			public String epmf4738764_delete(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
				...
				try {
					...
					/*정산기간 진행 체크*/
					int sel2 = epmf4738701Mapper.epmf4738701_select6(inputMap);
					if(sel2 < 1){
						 throw new Exception("A017");
					}
					
					 int stat = epmf4738701Mapper.epmf4738701_select5(inputMap); //상태 체크
					 if(stat>0){
							throw new Exception("A012"); //상태정보가 변경되어있습니다. 다시 한 번 확인하시기 바랍니다.
					 }
					 
					 inputMap.put("UPD_PRSN_ID", vo.getUSER_ID());
					epmf4738701Mapper.epmf4738764_delete(inputMap); 	// 입고정정테이블 삭제
					epmf4738701Mapper.epmf4738764_update(inputMap); 	// 입고관리마스터 정정문서번호 삭제
				}catch(Exception e){
					if(e.getMessage().equals("A012") || e.getMessage().equals("A017") ){
						 throw new Exception(e.getMessage()); 
					 }else{
						 throw new Exception("A001"); // DB 처리중 오류가 발생하였습니다. 관리자에게 문의하세요.
					 }
