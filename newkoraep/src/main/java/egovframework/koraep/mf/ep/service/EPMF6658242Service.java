...
import egovframework.mapper.mf.ep.EPMF6658242Mapper;
 ...
 *
 */
@Service("epmf6658242Service")
public class EPMF6658242Service {
	
	@Resource(name="epmf6658242Mapper")
	private EPMF6658242Mapper epmf6658242Mapper;
	 ...
	 * @
	 */
	public ModelMap epmf6658242_select1(ModelMap model, HttpServletRequest request)  {
		...
		try {
			...
			String  title = commonceService.getMenuTitle("EPMF6658242");		// 타이틀
			...
			List<?> initList	= epmf6658242Mapper.epmf6658242_select4(map);		// 그리드쪽 조회
	...
	public String epmf6658242_update(Map<String, Object> inputMap,HttpServletRequest request) throws Exception  {
		...
		if (list != null) {
			for(int i=0; i<list.size(); i++){
				if(i == 0){
					...
					int stat = epmf6658242Mapper.epmf6658242_select5(map); //상태 체크
					if(stat>0){
		...
		if (list != null) {
			try {
				...
				for(int i=0; i<list.size(); i++){
					...
					//세션 사업자아이디 및 번호 체크
					if(!map.get("MFC_BIZRID").equals(vo.getBIZRID()) || !map.get("MFC_BIZRNO").equals(vo.getBIZRNO_ORI())){// 세션 사업자정보와 일치하지 않음
					...
					for(int j=0 ;j<list2.size(); j++){
				 	...
				 	if(!keyCheck){
				 		...
				 		if(cnt == 0){
				 			 list2.add(map);
				 			epmf6658242Mapper.epmf6658242_delete(map); 		//모든 데이터 삭제
				 	...
				 	/*중복체크*/
				 	int sel = epmf6658242Mapper.epmf6658242_select6(map);
					if(sel>0){
					...
					//detail
				 	epmf6658242Mapper.epmf6658242_insert(map); 		// 출고상세 등록
					 
				}//end of for
				
			 	for(int j=0 ;j<list2.size(); j++){
			 		Map<String, String> map = (Map<String, String>) list2.get(j);
			 		int result = epmf6658242Mapper.epmf6658242_update(map);
			 		if(result == 0){
	 ...
	 * @
	 */
	public HashMap<String, Object> epmf6658242_select2(Map<String, String> data, HttpServletRequest request)  {
    	...
    	//빈용기명 조회
    	try {
			rtnMap.put("ctnr_nm", util.mapToJson(epmf6658242Mapper.epmf6658242_select1(data)));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
		}
	 ...
	 * @
	 */
	public HashMap epmf6658242_select3(Map<String, String> inputMap, HttpServletRequest request) {
    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	 ...
	 * @
	 */
	public HashMap epmf6658242_select4(Map<String, String> inputMap, HttpServletRequest request) {
    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
    	try {
			rtnMap.put("ctnr_nm", util.mapToJson(epmf6658242Mapper.epmf6658242_select1(inputMap)));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
		}    	 	//빈용기명 조회
	 ...
	 * @
	 */
	public String epmf6658242_delete(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
		String errCd = "0000";
		Map<String, String> map;

			int stat = epmf6658242Mapper.epmf6658242_select5(inputMap); //상태 체크 반환등록 상태 아닌거면 삭제 안되게
			 if(stat>0){
				throw new Exception("A009"); 
			 }
