...
import egovframework.mapper.mf.ep.EPMF4738731Mapper;
 ...
 *
 */
@Service("epmf4738731Service")
public class EPMF4738731Service {
	
	@Resource(name="epmf4738731Mapper")
	private EPMF4738731Mapper epmf4738731Mapper;  //입고정정등록 Mapper
	
	@Resource(name="epmf2983931Mapper")
	private EPMF2983931Mapper epmf2983931Mapper;  //입고정정 등록 Mapper
	...
	****************************************************************************************************************************************************************************************/
		 ...
		 * @
		 */
		  public ModelMap epmf4738731_select(ModelMap model, HttpServletRequest request) {
			  
			 	
			  	//파라메터 정보
				String reqParams = util.null2void(request.getParameter("INQ_PARAMS"),"{}");
				JSONObject jParams = JSONObject.fromObject(reqParams);
				String   title					= commonceService.getMenuTitle("EPMF4738731");		//타이틀
				...
				try {
					...
					List<?> iniList			= epmf4738731Mapper.epmf4738731_select(map);		//상세내역 조회
					List<?> cfm_gridList 	= epmf4738731Mapper.epmf4738731_select2(map);		//그리드쪽 조회 입고정보 있을 경우 
			 ...
			 * @
			 */
			public HashMap epmf4738731_select2(Map<String, String> inputMap, HttpServletRequest request) {
			...
			public String epmf4738731_insert(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
					...
					if (list != null) {
						try {
									...
									Map<String, String> check = (Map<String, String>) list.get(0);
									 ...
									 int sel = epmf4738731Mapper.epmf4738731_select6(check); // 적용기간 체크
									 if(sel >0 ){
									 ...
									 sel = epmf4738731Mapper.epmf4738731_select5(check); // 수량 체크
										if(sel != 1){
											 throw new Exception("A016"); 
									}
									*/ 
									List<?>  gridList	= util.mapToJson(epmf4738731Mapper.epmf4738731_select2(check));// 입고정보 초기 값 데이터 비교
									...
									for(int i=0; i<list.size(); i++){
										...
										sel = epmf4738731Mapper.epmf4738731_select4(map); //중복 체크 
										if(sel>0){
										...
										epmf4738731Mapper.epmf4738731_insert(map); 							// 입고정정 등록
										if(flag){
											epmf4738731Mapper.epmf4738731_update(map);						//입고마스터 update
											flag = false;
										}
									}//end of for
	...
	****************************************************************************************************************************************************************************************/
			 ...
			 * @
			 */
			  public ModelMap epmf47387312_select(ModelMap model, HttpServletRequest request) {
					...
					String   title					= commonceService.getMenuTitle("EPMF47387312");		//타이틀
					try {
			  ...
			  /**
				 ...
				 * @
				 */
				public HashMap epmf47387312_select2(Map<String, String> inputMap, HttpServletRequest request) {
			    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
			    	
			    	try {
				      		rtnMap.put("selList", util.mapToJson(epmf4738731Mapper.epmf47387312_select(inputMap)));
					} catch (Exception e) {
						// TODO Auto-generated catch block
						org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
					}	 
				 ...
				 */
			    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
				public String epmf47387312_insert(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
						HttpSession session = request.getSession();
						UserVO vo = (UserVO) session.getAttribute("userSession");
						String errCd = "0000";
						//Map<String, String> map;
						...
						if (list != null) {
							try {
										...
										for(int j=0 ;j<list2.size(); j++){//list2에 있는 입고문서번호 수만큼 입고마스터에 입고정정문서번호 입력
									 		...
									 		/*
									 		 * 20180823, 이근표 : 사업관리팀 곽명진팀장 요청
									 		sel = epmf4738731Mapper.epmf4738731_select5(map3); 	// 수량 체크
											if(sel != 1){	
													 throw new Exception("A016"); 
											}
											*/
									 		epmf4738731Mapper.epmf4738731_update(map3);			//입고마스터 update
									 	}
	...
	****************************************************************************************************************************************************************************************/
			 ...
			 * @
			 */
			  public ModelMap epmf4738742_select(ModelMap model, HttpServletRequest request) {
				 	
				  	//파라메터 정보
					String reqParams 	 = util.null2void(request.getParameter("INQ_PARAMS"),"{}");
					JSONObject jParams = JSONObject.fromObject(reqParams);
					String   title				 = commonceService.getMenuTitle("EPMF4738742");		//타이틀
					...
					try {
						...
						List<?> ctnr_seList		= commonceService.ctnr_se_select(request, map);						//빈용기구분 유흥/가정/직접 조회
						List<?> iniList				= epmf4738731Mapper.epmf4738731_select(map);		//상세내역 조회
						List<?> cfm_gridList 	= epmf4738731Mapper.epmf4738731_select2(map);		// 입고정보	 그리드쪽 조회 
						List<?> crct_gridList 	= epmf4738731Mapper.epmf4738742_select(map);		// 입고정정  그리드쪽 조회 
						List<?> ctnr_nm			= commonceService.ctnr_cd_select(map);					//빈용기명 조회
						List<?> rmk_list			= commonceService.getCommonCdListNew("D025");	//소매수수료 적용여부 비고
				 ...
				 */
			    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
				public String epmf4738742_update(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
						HttpSession session = request.getSession();
						UserVO vo = (UserVO) session.getAttribute("userSession");
						String errCd = "0000";
						//Map<String, String> map;
						...
						if (list != null) {
							try {
										...
										/*
										 * 20180823, 이근표 : 사업관리팀 곽명진팀장 요청
										int sel = epmf4738731Mapper.epmf4738731_select5(check); // 수량 체크
										 if(sel != 1){
											 throw new Exception("A016"); 
										 }
										*/ 
										...
										List<?>  gridList	= util.mapToJson(epmf4738731Mapper.epmf4738742_select(check));// 입고정정 초기 값 데이터 비교
										//System.out.println("gridList :  " +gridList.size()  +  "  :  list_ori  :" +list_ori.size() );
										if(list_ori.size() ==gridList.size() ){
												 for(int k =0;k<list_ori.size();k++){
													 Map<String, String> list_ori_map = new HashMap<String, String>();
										...
										epmf4738731Mapper.epmf4738742_delete(check); // 입고정정 삭제
...
****************************************************************************************************************************************************************************************/
	 ...
	 * @
	 */
	  public ModelMap epmf47387642_select(ModelMap model, HttpServletRequest request) {
		...
		//	Map<String, String> map= new HashMap<String, String>();
			...
			String   title					= commonceService.getMenuTitle("EPMF47387642");		//타이틀
		 ...
		 * @
		 */
		public HashMap epmf47387642_select2(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
		 ...
		 * @
		 */
		public HashMap epmf47387642_select3(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    		try {
		 ...
		 * @
		 */
		public HashMap epmf47387642_select4(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
		 ...
		 * @
		 */
		public HashMap epmf47387642_select5(Map<String, Object> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    		try {
					...
					rtnMap.put("selList", util.mapToJson(epmf4738731Mapper.epmf47387642_select(inputMap)));
					rtnMap.put("totalCnt", epmf4738731Mapper.epmf47387642_select_cnt(inputMap));
				} catch (Exception e) {
					// TODO Auto-generated catch block
					org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
				}	  
