...
import egovframework.mapper.mf.ep.EPMF2916431Mapper;
 ...
 *
 */
@Service("epmf2916431Service")
public class EPMF2916431Service {
	
	
	@Resource(name="epmf2916431Mapper")
	private EPMF2916431Mapper epmf2916431Mapper;  //실태조사 입고내역서조정 Mapper
	...
	private CommonCeService commonceService;  //공통
		 ...
		 * @
		 */
		  public ModelMap epmf2916431_select(ModelMap model, HttpServletRequest request) {
				...
				try {
					String title	= commonceService.getMenuTitle("EPMF2916431");		//타이틀
					...
					List<?> iniList				= epmf2916431Mapper.epmf2916431_select(map);		//상세내역 조회
					selCnt 						= epmf2916431Mapper.epmf2916431_select6(map); 	
					if(selCnt>0){
						cfm_gridList			= epmf2916431Mapper.epmf2916431_select7(map);		//그리드쪽 조회 입고정보 있을 경우 
					}
					List<?>  gridList			= epmf2916431Mapper.epmf2916431_select2(map);		//그리드쪽 조회  
			 ...
			 * @
			 */
			public HashMap epmf2916431_select2(Map<String, String> inputMap, HttpServletRequest request) {
			...
			public String epmf2916431_insert(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
					...
					if (list != null) {
						try {
							...
							int stat = epmf2916431Mapper.epmf2916431_select4(check); //상태 체크
							 if(stat>0){
								throw new Exception("A009"); 
							 }
							 int sel = epmf2916431Mapper.epmf2916431_select5(check); // 수량 체크
							 if(sel != 1){
								 throw new Exception("A008"); 
							 }
			
							 List<?>  gridList	= util.mapToJson(epmf2916431Mapper.epmf2916431_select2(check));// 입고 정보 초기 값 데이터 비교
							...
							for(int i=0; i<list.size(); i++){
								...
								map.put("REG_PRSN_ID", vo.getUSER_ID());  											//등록자
							 	...
							 	if(!keyCheck){
										 ...
										 epmf2916431Mapper.epmf2916431_insert(map); 							//	입고마스터	  등록
										 epmf2916431Mapper.epmf2916431_update3(map);						// 실태조사확인상태
								 		 list2.add(map);
								 		 keyCheck = true;
							 	}	
							 	//detail  
							 	epmf2916431Mapper.epmf2916431_insert2(map); 			// 입고상세
						 	...
						 	for(int j=0 ;j<list2.size(); j++){
						 		...
						 		epmf2916431Mapper.epmf2916431_update(map);			//입고 마스터 총값 넣기
						 		epmf2916431Mapper.epmf2916431_update2(map); 		//반환 마스터  상태변경
			 ...
			 */
		    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
			public String epmf2916431_update(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
					HttpSession session = request.getSession();
					UserVO vo = (UserVO) session.getAttribute("userSession");
							...
							for(int i=0; i<list.size(); i++){
								...
								map.put("UPD_PRSN_ID", vo.getUSER_ID());  					//등록자
							 	if(!keyCheck){
								 		...
								 		epmf2916431Mapper.epmf2916431_delete(map);		// 입고상세 삭제
								 		epmf2916431Mapper.epmf2916431_update4(map); 	//입고 마스터  상태변경및 입고확인입력
								 		epmf2916431Mapper.epmf2916431_update2(map); 	//반환 마스터  상태변경및 입고확인입력
								 		epmf2916431Mapper.epmf2916431_update3(map);	// 실태조사확인상태
							 	}	 
							 	//detail
							 	epmf2916431Mapper.epmf2916431_insert2(map); 				// 입고상세
						 	...
						 	for(int j=0 ;j<list2.size(); j++){
						 		...
						 		epmf2916431Mapper.epmf2916431_update(last_map);			//입고 마스터 총값 넣기
