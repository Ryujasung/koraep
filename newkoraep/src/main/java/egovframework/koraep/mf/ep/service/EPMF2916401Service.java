...
import egovframework.mapper.mf.ep.EPMF2916401Mapper;
 ...
 *
 */
@Service("epmf2916401Service")
public class EPMF2916401Service {
	
	
	@Resource(name="epmf2916401Mapper")
	private EPMF2916401Mapper epmf2916401Mapper;  //실태조사 Mapper
	 ...
	 * @
	 */
	  public ModelMap epmf2916401_select(ModelMap model, HttpServletRequest request) {
			...
			List<?> stat_cdList		= epmf2916401Mapper.epmf2916401_select7();			//상태
			...
			List<?> sys_seList		= epmf2916401Mapper.epmf2916401_select8();			//실태조사구분
			List<?> areaList			= commonceService.getCommonCdListNew("B010");		//지역
			String   title					= commonceService.getMenuTitle("EPMF2916401");		//타이틀
			List<?>	whsdlList		= commonceService.mfc_bizrnm_select4(request, map);    			//도매업자 업체명조회
			String   detail 				= "F";
			List<?> grid_info			= commonceService.GRID_INFO_SELECT("EPMF2916401",request);		//그리드 컬럼 정보
		 ...
		 * @
		 */
		public HashMap epmf2916401_select2(Map<String, String> inputMap, HttpServletRequest request) {
		 ...
		 * @
		 */
		public HashMap epmf2916401_select3(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
	    		try {
		 ...
		 * @
		 */
		public HashMap epmf2916401_select4(Map<String, String> inputMap, HttpServletRequest request) {
	    	HashMap<String, Object> rtnMap = new HashMap<String, Object>();
		 ...
		 * @
		 */
		public HashMap epmf2916401_select5(Map<String, String> inputMap, HttpServletRequest request) {
			...
			if(vo != null){ //로그인 생산자 
	    		...
	    		try {
					rtnMap.put("selList", util.mapToJson(epmf2916401Mapper.epmf2916401_select4(inputMap)));
					rtnMap.put("totalList", util.mapToJson(epmf2916401Mapper.epmf2916401_select4_cnt(inputMap)));
				} catch (Exception e) {
					// TODO Auto-generated catch block
					org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
				}	  
		 ...
		 * @
		 */
		public String epmf2916401_excel(HashMap<String, String> data, HttpServletRequest request) {
			...
			try {
				...
				List<?> list = epmf2916401Mapper.epmf2916401_select4(data);
				//엑셀파일 저장
		 ...
		 * @
		 */
		public String epmf2916401_update(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
			String errCd = "0000";
			...
			if (list != null) {
				try {
					for(int i=0; i<list.size(); i++){
						...
						int stat = epmf2916401Mapper.epmf2916401_select5(map); //상태 체크
						 if(stat>0){
							throw new Exception("A010");  //실태조사 대상 설정 처리가 불가능한 자료가 선택되었습니다. 다시 한 번 확인하시기 바랍니다.
						 }
						 	map.put("UPD_PRSN_ID", vo.getUSER_ID());  				//등록자
							epmf2916401Mapper.epmf2916401_update(map); 		// 반환내역서  실태조사이전 상태로 변화
							if(map.get("WRHS_DOC_NO") !=null){
								epmf2916401Mapper.epmf2916401_update2(map); 	// 입고내역서 실태조사이전 상태로 변화
							}
							epmf2916401Mapper.epmf2916401_delete(map); 			// 실태조사파일테이블 삭제
							epmf2916401Mapper.epmf2916401_delete2(map); 		// 실태조사정보테이블 삭제
					}
				}catch (Exception e) {
	...
	//---------------------------------------------------------------------------------------------------------------------
		 ...
		 * @
		 */
		public HashMap epmf2916488_select(Map<String, String> inputMap, HttpServletRequest request) {
				HashMap<String, Object> rtnMap = new HashMap<String, Object>();
			  	//파라메터 정보
				String   title = commonceService.getMenuTitle("EPMF2916488");		//타이틀
				
				try {
					rtnMap.put("initList", util.mapToJson(epmf2916401Mapper.epmf2916488_select(inputMap)));  //입고조정인 애들 증빙파일  기초정보조회
				} catch (Exception e) {
					// TODO Auto-generated catch block
					org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
				}   
		...
		public String epmf2916488_insert(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
	    	HttpSession session = request.getSession();
			UserVO vo = (UserVO)session.getAttribute("userSession");
...
//			MultipartHttpServletRequest mptRequest = (MultipartHttpServletRequest)request;
			...
			try {	
					while (fileIter.hasNext()) {
						...
						if(fileName != null && !fileName.equals("")){
							...
							epmf2916401Mapper.epmf2916488_insert(inputMap);	
						}
					}
					...
					for(int i=0; i<delSeqList.length; i++) {
						...
						epmf2916401Mapper.epmf2916488_delete(inputMap);
					}
		 ...
		 */
	    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
		public String epmf2916488_delete(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
				HttpSession session = request.getSession();
				UserVO vo = (UserVO) session.getAttribute("userSession");
	...
	//---------------------------------------------------------------------------------------------------------------------
		 ...
		 * @
		 */
		public HashMap epmf29164882_select(Map<String, String> inputMap, HttpServletRequest request) {
			HashMap<String, Object> rtnMap = new HashMap<String, Object>();
		  	//파라메터 정보
			String   title					= commonceService.getMenuTitle("EPMF29164882");		//타이틀
			
			try {
				rtnMap.put("initList", util.mapToJson(epmf2916401Mapper.epmf29164882_select(inputMap))); //입고조정인 애들 증빙파일  기초정보조회
			} catch (Exception e) {
				// TODO Auto-generated catch block
				org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
			}    
	...
	//---------------------------------------------------------------------------------------------------------------------
		 ...
		 * @
		 */
		public HashMap epmf29164883_select(Map<String, String> inputMap, HttpServletRequest request) {
			HashMap<String, Object> rtnMap = new HashMap<String, Object>();
		  	//파라메터 정보
			String   title					= commonceService.getMenuTitle("EPMF29164883");		//타이틀
			
			try {
				rtnMap.put("initList", util.mapToJson(epmf2916401Mapper.epmf29164883_select(inputMap))); //입고조정인 애들 증빙파일  기초정보조회
			} catch (Exception e) {
				// TODO Auto-generated catch block
				org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
			}     
		 ...
		 */
	    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
		public String epmf29164883_update(Map<String, String> inputMap,HttpServletRequest request) throws Exception  {
				HttpSession session = request.getSession();
				UserVO vo = (UserVO) session.getAttribute("userSession");
				...
				try {
					inputMap.put("REG_PRSN_ID", vo.getUSER_ID());  		
					epmf2916401Mapper.epmf29164883_update(inputMap);		
		 ...
		 * @
		 */
		public ModelMap epmf29164313_select(ModelMap model, HttpServletRequest request) {
			...
			if(jParams.get("PARAMS") !=null){//반환상세 볼경우
				...
				rsrcList = epmf2916401Mapper.epmf29164313_select(map);
			}
		 ...
		 */
	    @Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
		public String EPMF29164313_update(Map<String, String> inputMap, HttpServletRequest request) throws Exception  {
			HttpSession session = request.getSession();
			UserVO vo = (UserVO) session.getAttribute("userSession");
			String errCd = "0000";
			...
			try {
				...
				if(vo != null){
					...
					inputMap.put("PR_PRSN_ID", ssUserId);
					inputMap.put("UPD_PRSN_ID", ssUserId);
				}

				epmf2916401Mapper.epmf29164313_update(inputMap); // 실태조사표
