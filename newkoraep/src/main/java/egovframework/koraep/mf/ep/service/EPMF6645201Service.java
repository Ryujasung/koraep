...
import egovframework.mapper.mf.ep.EPMF6645201Mapper; //직접회수정보조회
 ...
 *
 */
@Service("epmf6645201Service")
public class EPMF6645201Service {
	
	@Resource(name="epmf6645201Mapper")
	private EPMF6645201Mapper epmf6645201Mapper;
	 ...
	 * @return
	 */
	public ModelMap epmf6645201_select1(ModelMap model, HttpServletRequest request)  {
		//파라메터 정보
		...
		String   title					= commonceService.getMenuTitle("EPMF6645201");	//타이틀
		...
		List<?> grid_info			= commonceService.GRID_INFO_SELECT("EPMF6645201",request);		//그리드 컬럼 정보
	 ...
	 * @
	 */
	public HashMap<String, Object> epmf6645201_select2(Map<String, String> inputMap, HttpServletRequest request) {
		...
		try {
			rtnMap.put("selList", util.mapToJson(epmf6645201Mapper.epmf6645201_select(inputMap)));
			rtnMap.put("totalList", util.mapToJson(epmf6645201Mapper.epmf6645201_select_cnt(inputMap)));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			org.slf4j.LoggerFactory.getLogger(egovframework.common.AuthenticationFailHandlerImpl.class).debug("Exception Error");
		}
	 ...
	 * @
	 */
	public HashMap<String, Object> epmf6645201_select3(Map<String, String> data, HttpServletRequest request) {
	 ...
	 * @
	 */
	public String epmf6645201_delete(Map<String, String> inputMap, HttpServletRequest request) throws Exception {
		...
		if (list != null) {
			for(int i=0; i<list.size(); i++){
				...
				int stat = epmf6645201Mapper.epmf6645201_select3(map); //상태 체크
				if(stat>0){
		...
		if (list != null) {
			try {
				for(int i=0; i<list.size(); i++){
					map = (Map<String, String>) list.get(i);
					epmf6645201Mapper.epmf6645201_delete(map); // 직접회수정보 삭제
				}
			}catch (Exception e) {
	 ...
	 * @
	 */
	public ModelMap epmf6645201_selec4(ModelMap model, HttpServletRequest request) {
		Map<String, String> map = new HashMap<String, String>();
		...
		String   title		= commonceService.getMenuTitle("EPMF6645264");		//타이틀
		List<?> iniList	= epmf6645201Mapper.epmf6645264_select(map);		//상세내역 조회 표시
		List<?> gridList	= epmf6645201Mapper.epmf6645264_select2(map);		//그리드쪽 조회
	 ...
	 * @
	 */
	public String epmf6645201_excel(HashMap<String, String> data, HttpServletRequest request) {
		...
		try {
			...
			data.put("excelYn", "Y");
			List<?> list = epmf6645201Mapper.epmf6645201_select(data);
	 ...
	 * @
	 */
	public String epmf6645264_excel(HashMap<String, String> data, HttpServletRequest request) {
		
		String errCd = "0000";

		try {
					
			List<?> list = epmf6645201Mapper.epmf6645264_select2(data);
